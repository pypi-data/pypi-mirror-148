...
def set_beam_center(root: nx.NXroot, x0: float = None, y0: float = None) -> None:
    """
    Set the beam center position (in pixels) within the "entry0/detector/beam_center_x" and  "entry0/detector/beam_center_y" fields.
    Args:
        root:
        x0 (float): horizontal position of the beam center in pixels
        y0 (float): vertical position of the beam center in pixels
...
@nxlib.treatment_function(new_entry=False)
def set_transmission(root: nx.NXroot, transmission: float = None) -> None:
    """
    Set the transmission of the sample within the "entry0/sample/transmission" field
    Args:
        root:
        transmission (float): sample transmission
...
def azimutal_integration(root: nx.NXroot, mask: str = None, x0: float = None, y0: float = None, bins: int = 900,
    ...
    """
    Performe azimutal averaging of the data
    Args:
        root:
        mask (str): mask file path
        x0 (float): horizontal position of the beam center in pixels
        y0 (float): vertical position of the beam center in pixels
        bins (int): number of equal width bins in the given range of the radial coordinate (radius or Q)
        x_pixel_size (float): pixel width in mm
        y_pixel_size: (float): ppixel height in mm
...
def resu2D(root: nx.NXroot, dark_file: str = None, ec_file: str = None, eb_file: str = None,
    ...
    i_sample.nxerrors = np.sqrt(np.abs(i_sample.data.nxdata))
    last_key = nxlib.get_last_entry_key(root)
    i_sample /= entry['sample/count_time']
    ...
    x_pixel_size = root[nxlib.get_last_entry_key(root)].instrument.detector.x_pixel_size.nxdata
    y_pixel_size = root[nxlib.get_last_entry_key(root)].instrument.detector.y_pixel_size.nxdata
    y = (y - y0) * y_pixel_size
    x = (x - x0) * x_pixel_size
    r = np.sqrt(x**2 + y**2)
    theta = np.arctan(r / distance)
...
def q_scale(root: nx.NXroot, distance: float = None) -> None:
    ...
    i_errors = entry.data.nxerrors #entry.data[entry.data.attrs['uncertainties']].nxdata
    data = nx.NXdata()
    data.nxsignal = i
...
def azimutal_integration2D(root: nx.NXroot, mask: str = None, x0: float = None, y0: float = None, distance: float = None,
                           q_bins: int = 900, chi_bins: int = 360, pixel_size: float = None) -> None:
    last_key = nxlib.get_last_entry_key(root)
    entry = root[last_key]
    if x0 is None:
        x0 = entry.instrument.detector.beam_center_x.nxdata
    ...
    entry.instrument.detector.pixel_mask = mask_data
    del entry['data']
    q, chi, masked_data = flib.xy2polarQ(m, mask_data, x0, y0, pixel_size, pixel_size, q_bins, chi_bins,
                                         distance, wavelength)
    # q = 4*np.pi*np.sin(np.arctan(r_grid/distance)/2)/wavelength

    # new_entry.data = nx.NXdata(attrs={'interpretation': 'spectrum',
    #                                   'signal': 'I', 'axes': ['Q']})
    y = nx.NXfield(q, name='Q', attrs={'units': r'$\AA^{-1}$'})
    # y = nx.NXfield(r_grid, name='r', attrs={'units': r'$\AA^{-1}$'})
...
@nxlib.treatment_function(new_entry=True)
def sector_cut(root: nx.NXroot, chi_min: float = None, chi_max: float = None):
    last_key = nxlib.get_last_entry_key(root)
    entry = root[last_key]
    i = entry.data.nxsignal.nxdata
    if np.ndim(i) == 2 and 'chi' in entry.data:
        chi = entry.data.chi.nxdata
        if "Q" in entry.data:
            radial = entry.data.Q.nxdata
            axe_name = "Q"
        elif "r" in entry.data:
            radial = entry.data.r.nxdata
            axe_name = "r"
        else:
            raise NameError("no Q nor r field within data")
        if chi_min is None:
            chi_min = np.min(chi)
        if chi_max is None:
            chi_max = np.max(chi)
        indexes = np.argwhere(np.logical_and(chi > chi_min, chi < chi_max))
        i_masked = np.ma.masked_less(i[:, indexes], 0, copy=True)
        i_cut = np.mean(i_masked, axis=1)[:,0]#.compressed()
        # x = np.ma.masked_array(radial, mask=i_masked).compressed()

        del entry['data']
        data = nx.NXdata(nx.NXfield(value=i_cut, name='i'), [nx.NXfield(radial, name=axe_name)])
        entry.data = data
        return
...
if __name__ == '__main__':
    ...
    # file = 'AgBe.edf'
    ############################################################################
    # Test with fabrice files
    # Comparision PASINET /PYGDATAX : OK
    # Test resu2D at small angle : OK
    ############################################################################
    ...
    ############################################################################
    # données Annies TTGQ
    # test polar cut and res2D
    ############################################################################
    # test 2D resu
    import shutil
    file = '/home/achennev/Documents/xeuss/Test_PolarCut/2022-04-19_AB_0_113477.edf'
    dark_file = '/home/achennev/Documents/xeuss/Test_PolarCut/2022-04-19_AB_0_113471.edf'
    eb_file = '/home/achennev/Documents/xeuss/Test_PolarCut/2022-04-19_AB_0_113473.edf'

    nxlib.build_nexus_from_edf(file)
    nxlib.build_nexus_from_edf(dark_file)
    nxlib.build_nexus_from_edf(eb_file)

    file = file.replace('.edf','.nxs')
    dark_file = dark_file.replace('.edf','.nxs')
    eb_file = eb_file.replace('.edf','.nxs')

    file2 = '/home/achennev/Documents/xeuss/Test_PolarCut/2022-04-19_AB_0_113477_0.nxs'
    shutil.copyfile(file, file2)

    azimutal_integration(file, mask='/home/achennev/Documents/xeuss/Test_PolarCut/MASQ_TTGQ.edf')
    azimutal_integration(dark_file, mask='/home/achennev/Documents/xeuss/Test_PolarCut/MASQ_TTGQ.edf')
    azimutal_integration(eb_file, mask='/home/achennev/Documents/xeuss/Test_PolarCut/MASQ_TTGQ.edf')

    resu(file)#, dark_file=dark_file,
         # eb_file=eb_file)
    # resu(file2)
    resu2D(file2)#, dark_file=dark_file,
           # eb_file=eb_file)
    # azimutal_integration(file2, mask='/home/achennev/Documents/xeuss/Test_PolarCut/MASQ_TTGQ.edf')
    # q_scale(file2)
    azimutal_integration2D(file2, mask='/home/achennev/Documents/xeuss/Test_PolarCut/MASQ_TTGQ.edf')
    polar_cut(file2, q=1.3, pixel_width=1)
    # sector_cut(file2)
