...
def getTargetFileParamsList(targetFileName):
    with open(targetFileName, 'r', encoding='utf8') as readF:
        fileData = readF.read()
        fileJsonData = correctJsonData(fileData.split("=")[1].replace(";", ""))
        jsonData = json.loads(fileJsonData)
        jsonKeyList = []
...
""".format(moduleName, getManagerClassName(moduleName), mapListCreater(emptyParams)))
    ...
    if (os.path.exists('i18n.json') == False):
        ...
        with open(tdfIntlDir + '/i18n.json', 'r', encoding='utf8') as originF:
            ...
            for targetFileNameSuffix in i18nList:
                ...
                targetFile = open(targetFileName, 'r', encoding='utf8')
                fileContent = targetFile.read()
                keyNode = fileContent.find("}")

                addStr = ""
                for item in json_data:
                    if item in paramsResultList:
                        continue
                    else:
                        if keyNode != -1:
                            if (targetFileNameSuffix.find('zh-ch') != -1):
                                ...
                                addStr = addStr + "\n  \"{0}\": \"{1}\",\n".format(
                                    item, tdf_translate(json_data[item], targetFileNameSuffix))
                            # 其他语言不再进行翻译，自行填写
                            # else:
