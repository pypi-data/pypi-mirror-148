...
class _BaseOperationAdditionalFunctionDecorator:  # noqa
    ...
    def __getattr__(self, operation_name):
        ...
        class _BaseOperationAdditionalFunctionDecoratorWrapper:
            def __init__(self, method=None, *, priority: int = -1):
                self.method = method
            ...
            def __call__(self, method):
                return _BaseOperationAdditionalFunctionDecoratorWrapper(method, priority=self.priority)
    ...
    def _register_operation_func(
        self, operation_name: str, original_function_owner: Type, original_function: Callable, priority: int
    ):
...
if TYPE_CHECKING:
    ...
    class operation:
        ...
        def __set_name__(self, owner, name):
            ...
            if inspect.iscoroutinefunction(self.method):
                self.operation = _AsyncOperation(
                    method=self.method, method_class=owner, exception_handlers_mapping=self.exception_handlers_mapping
                )
            else:
        ...
        def __call__(self, func):
            return operation(func, exception_handlers_mapping=self.exception_handlers_mapping)
