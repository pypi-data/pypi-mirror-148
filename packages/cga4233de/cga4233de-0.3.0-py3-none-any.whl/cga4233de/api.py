...
class Api:
    ...
    def get(self, endpoint, timestamp=False):
        if timestamp:
            ...
            return self.session.get(
                self.addr + endpoint + "?_=" + str(now), headers=self.headers
            ).json()
        else:
    ...
    def login(self):
        ...
        a = hashlib.pbkdf2_hmac(
            ...
            bytes(response["salt"], "utf-8"),
            1000,
        ).hex()[:32]
        b = hashlib.pbkdf2_hmac(
            "sha256", bytes(a, "utf-8"), bytes(response["saltwebui"], "utf-8"), 1000
        ).hex()[:32]

        response = self.post(
            LOGIN, data={"username": self.username, "password": b}, csrf=False
        ).json()
        if response["error"] == "error":
