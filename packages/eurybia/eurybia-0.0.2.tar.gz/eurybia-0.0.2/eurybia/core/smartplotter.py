...
class SmartPlotter:
    ...
    def scatter_feature_importance(
        ...
        maxx = 1.1 * data["datadrift_classifier"].max()
    ...
    def generate_auc_historical(
        ...
        if auc_historical is not None:
            auc_historical = auc_historical[["date", "auc"]]

            auc_historical = auc_historical.groupby("date")["auc"].mean().reset_index()
    ...
    def generate_modeldrift_data(
        ...
        data_modeldrift[metric] = data_modeldrift[metric].apply(
            lambda row: round(row, len([char for char in str(row).split(".")[1] if char == "0"]) + 1)
        )
    ...
    def generate_fig_auc(self) -> plotly.graph_objs._figure.Figure:
        """
        Displays auc in a colorbar
        """
        color = sns.blend_palette(["green", "yellow", "orange", "red"], 100)
        color = color.as_hex()
        list_color_glob = list()
        threshold = [i for i in np.arange(0.5, 1, 0.5 / len(color))]
        ...
        fig = go.Figure(
            go.Indicator(
                mode="gauge+number",
                value=self.smartdrift.auc,
                domain={"x": [0, 1], "y": [0, 1]},
                title={"text": "<b>AUC</b>", "align": "center", "font": {"size": 50}},
                gauge={
                    "axis": {"range": [0.5, 1], "ticktext": ["No Drift", "High Drift"], "tickwidth": 1},
                    "bar": {"color": "black"},
                    "borderwidth": 0,
