...
class TestSmartPlotter(unittest.TestCase):
    ...
    def test_generate_auc_historical(self):
        """
        Unit test for generate_auc_historical()'s method
        """
        auc = {"date": ["23/09/2021", "23/08/2021", "23/07/2021"], "auc": [0.528107, 0.532106, 0.510653]}
        auc_historical = pd.DataFrame(auc)
        fig = self.smartdrift.plot.generate_auc_historical(auc_historical)
        assert isinstance(fig, plotly.graph_objs._figure.Figure)
    ...
    def test_generate_fig_auc(self):
        """
        Unit test for generate_fig_auc()'s method
        """
        fig = self.smartdrift.plot.generate_fig_auc()
        assert isinstance(fig, plotly.graph_objs._figure.Figure)
