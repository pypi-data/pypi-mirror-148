...
def reshape_wit_data_list(original_wit_data_list, default_value=np.nan):
    ...
    for index in presend_wit_indexes:
        for index in range(len(wit_data_list[index].stock_codes)):
            non_nan_indexex = np.where(np.logical_not(np.isnan(wit_data_list[index].data[:, index])))[0]
            if non_nan_indexex > 0: break
