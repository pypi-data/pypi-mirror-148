...
from .lib import _extract_def_header
...
class RepositoryFunctionArgument:
    ...
    def __post_init__(self):
        ...
        if self.is_entity:
            self.converter = f'{self.name}_converter'
...
def extract_repo_info_from_ao(ao_cls):
    ...
    for func_str in cls_funcs_str:
        ...
        code = getsource(func)
        def_header = _extract_def_header(code)
        ...
        for arg_tuple in arg_tuples:
            ...
            if default_value is None and not is_list and not is_optional:
                ...
                element = (arg_name, True, var_to_value_type(arg_name), default_value)
            elif not is_optional:
                element = (arg_name, True, var_to_value_type(arg_name), default_value, True)
            else:
                element = (arg_name, True, var_to_value_type(arg_name), default_value, True, True)
            args_info.append(element)
        func_info[func_str]['args'] = args_info
        ret_is_list = _extract_ret_from_code(code)
        arg_name = 'your_ret'
        func_info[func_str]['ret'] = (
            arg_name, True, var_to_value_type(arg_name,),None, ret_is_list, True)
    return func_info

def save_repo_info(ao_classes, save_dir, save_name='repo_info_temp.json'):
    save_fn = os.path.join(save_dir, save_name )
    repo_info = {}
    for cls in ao_classes:
        repo_info[cls.__name__] = extract_repo_info_from_ao(cls)
    json_str = '{\n'
    for class_name in repo_info:
        json_str += f'  "{class_name}":{{\n'
        for method_name in repo_info[class_name]:
            json_str += f'    "{method_name}":{{\n'
            json_str += f'      "args":'
