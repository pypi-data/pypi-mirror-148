...
def replace_header(target_header, source_header):
    ...
    target_entities, entity_header = extract_object_from_import_code(target_entity_code)
    target_obj_values, obj_value_header = extract_object_from_import_code(target_obj_value_code)
    target_converters, converter_header = extract_object_from_import_code(target_converter_code)
    target_repositories, repository_header = extract_object_from_import_code(target_repository_code)
    target_assemblers, assember_header = extract_object_from_import_code(target_assembler_code)
    target_dos, do_header = extract_object_from_import_code(target_do_code)
    target_dtos, dto_header = extract_object_from_import_code(target_dto_code)
    # extract import code
    ...
    # extract objects and subheader
    source_entities, _ = extract_object_from_import_code(source_entity_code)
    source_obj_values, _ = extract_object_from_import_code(source_obj_value_code)
    source_converters, _ = extract_object_from_import_code(source_converter_code)
    source_repositories, _ = extract_object_from_import_code(source_repository_code)
    source_assemblers, _ = extract_object_from_import_code(source_assembler_code)
    source_dos, _ = extract_object_from_import_code(source_do_code)
    source_dtos, _ = extract_object_from_import_code(source_dto_code)
    # replace objects
    ...
    object_code_pairs = [
        (target_entities, entity_header),
        (target_obj_values, obj_value_header),
        (target_converters, converter_header),
        (target_repositories, repository_header),
        (target_assemblers, assember_header),
        (target_dos, do_header),
        (target_dtos, dto_header)
    ]
...
def replace_class_functions(source_dir, fn, target=None, exclude_classes=[]):
    ...
    # source_converter_code, source_rest = extract_import_converter_code(source_rest)
    # source_repository_code, source_rest = extract_import_repository_code(source_rest)
    # source_entities, entity_header = extract_object_from_import_code(source_entity_code)
    # source_obj_values, obj_value_header = extract_object_from_import_code(source_obj_value_code)
    # source_converters, converter_header = extract_object_from_import_code(source_converter_code)
    # source_repositories, repository_header = extract_object_from_import_code(source_repository_code)
    # if target_entities:
    #     target_entities = list(set(target_entities+source_entities))
    # else:
    #     target_entities = source_entities
    # if target_obj_values:
    #     target_obj_values = list(set(target_obj_values+source_obj_values))
    # else:
    #     target_obj_values = source_obj_values
    # if target_converters:
    #     target_converters = list(set(target_converters+source_converters))
    # else:
    #     target_converters = source_converters
