...
class RepositoryImplGenerator:
    ...
    def gen(repo_info, save_dir, save_name='repository_impl_tmp.py'):
        ...
        with open(save_fn, 'w') as f:
            ...
            f.write('from ddd_object.infrastructure.repository_impl import RepositoryImpl\n')
            f.write('from .converter import (\n')
            for converter in converters:
                f.write(f'    {var_to_value_type(converter)},\n')
            f.write(')\n')
            ...
            if value_types:
                ...
                f.write(')\n')
            for converter in converters:
                f.write(f'{converter} = {var_to_value_type(converter)}()\n')
            for repo in repos:
    ...
    def _extract_converters(repos: List[RepositoryInfo]):
        ...
        for repo in repos:
            for method in repo.methods:
                ...
                for arg in all_args:
                    if not isinstance(arg.converter, bool):
                        converters[arg.converter] = 0
    ...
    def _gen_class_block(repo: RepositoryInfo):
        ...
        for method in repo.methods:
            ...
            for arg in args:
                ...
                if isinstance(arg.converter, bool) and arg.converter:
                    arg_do_str.append(f'{arg.name} = {arg.name}.get_value()')
                elif isinstance(arg.converter, str):
            ...
            if method.key is not None:
                method_block = f"""
    def _{method.name}(self, {args_str})->{ret_type}:
        {arg_do_str}
...
class RepositoryGenerator:
    ...
    def gen(repo_info, save_dir, save_name='repository_tmp.py'):
        ...
        with open(save_fn, 'w') as f:
            f.write('from typing import Optional, List\n')
            f.write('from ddd_object.domain.repository import Repository\n')
            if entity_types:
                f.write('from ..domain.entity import (\n')
                for t in entity_types:
                    f.write(f'    {t},\n')
