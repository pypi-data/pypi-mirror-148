    ...
    def post(self):
        ...
        for i in range(0, len(image_coll)):
            try:
                ...
                with open("dms_app/fingerprint_img/" + str(person_id) + "_" + str(i + 1) + "_" + ".tif", 'wb') as f:
...
class MatchFingerprint(Resource):
    ...
    def post(self):
        ...
        for i in range(len(args["base64"])):
            ...
            with open("dms_app/sample.tif", 'wb') as f:
                f.write(image_binary)
            test_original = cv2.imread("dms_app/sample.tif")
            for file in [file for file in os.listdir("dms_app/fingerprint_img")]:
                _image = cv2.imread("dms_app/fingerprint_img/" + file)
