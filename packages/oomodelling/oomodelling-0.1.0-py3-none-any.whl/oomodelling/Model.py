...
class Model:
    ...
    def _new_var(self, name, fun, override=False):
        assert self._under_construction, "Not under construction."
        assert callable(fun), "Fun must be callable."
        assert name.isidentifier(), "Name must be valid identifier."
        if not override:
            ...
            assert hasattr(self, name), f"Name {name} must already defined if it's to be overridden."
            assert name in self._vars, f"Name {name} must already defined if it's to be overridden."
            assert name in self.signals, f"Name {name} must already defined if it's to be overridden."
        return self._get_signal_function(name, fun)

    def var(self, fun):
        assert self._under_construction, "Not under construction."
        return lambda name: self._new_var(name, fun, override=False)

    def ovar(self, fun):
        assert self._under_construction, "Not under construction."
        return lambda name: self._new_var(name, fun, override=True)
    ...
    def edit(self):
        assert not self._under_construction, "Already under construction."
        self._bypass_set_attr = True
        self._under_construction = True
        self._bypass_set_attr = False
