...
def build_filter_functions_str(node_name: str, conversion_map: T.Dict[str, dict]):
    ...
    filter_by_str = f"""
def filter_by(self, {params_fields_str}) -> {node_name}Resolver:
    return super().filter_by(**{{{dict_fields_str}}})
    """
    # now for filter in
    ...
    filter_in_str = f"""
def filter_in(self, {params_fields_lst_str}) -> {node_name}Resolver:
    return super().filter_in(**{{{dict_fields_str}}})
    """
