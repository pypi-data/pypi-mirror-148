...
LastEditTime: 2022-04-26 14:43:50
LastEditors: BDFD
Description: 
...
def gumbel(pq, unitt, unitx, i1, i2, i3, meanx, sdx, n, dataolist):
    ...
    if n<nmin:
        ...
        note_1_2 = ''    
        note_1_3 = '说明：由于样本容量小于',nmin,'，频率分析未进行。'
        note_1_4 = ''
        note.append(note_1_1)
        note.append(note_1_2)
        note.append(note_1_3)
        note.append(note_1_4)
    else:
        if i2==1:
        ...
        if i2==2:
            ...
            note2.append(note_2_1)        
        note_2_2 = ' ','均值 = ',round(meanx,2),'('+str(unitx)+')'
        note_2_3 =' ','均方差 = ',round(sdx,2),'('+str(unitx)+')'
        note2.append(note_2_2)
        note2.append(note_2_3)   
        if i2==2:
            note_2_4 = ' ','偏态系数 = ',round(Csx,2)
        ...
        print(' ')
        if i1==1:
            note_2_6 = '------ Gumbel分布频率分析（最大值模式）------'
        if i1==2:
            note_2_6 = '------ Gumbel分布频率分析（最小值模式）------'    
        print(' ')
        print('                               ','极限理论Gumbel分布','    基于样本容量的理论Gumbel分布')

        print(' ')
        print('1) 不同回归期下的取值')
        print('回归期('+str(unitt)+')','                     ',str(pq),'('+str(unitx)+')')
        for i in range(1,len(Tt)+1):
            print(Tt[i-1],'                                 ',round(xTt1[i-1],2),'                         ',round(xTt2[i-1],2))
        print(' ')
        
        print('2) 分布参数') 
        print('位置系数:                ',round(mode1,3),'                        ',round(mode2,3))
        print('尺度系数:                   ',round(scale1,3),'                        ',round(scale2,3))
        print(' ') 

        if i2==2:  
            print('3) 拟合检验') 
            print('Kolmogorov-Smirnov 检验值:',round(KS1,3),'                        ',round(KS2,3))
            if n>=nlC2:
                print('Chi-Squared 检验值:       ',round(C21,3),'                        ',round(C22,3))
            else:
                print('Chi-Squared 检验值: 无 (因为样本容量小于',nlC2,')') 

        print('')#20220310
        print('---结果展示结束---')#20220310
        # data=()
        # for i in range(1,len(Tt)+1):
    ...
    plt.close("all")
    return plot_url
    # return plot_url, data, note, note2