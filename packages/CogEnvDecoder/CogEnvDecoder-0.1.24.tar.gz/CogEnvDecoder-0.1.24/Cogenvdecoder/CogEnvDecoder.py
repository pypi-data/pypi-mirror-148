    ...
    def _thread_step(self):
        ...
        while True:
            ...
            if len(terminal_step) != 0:
                # The agent is done
                self._env.game_over = True
                self._obs, self._reward, self._done, self._info =  self._env._single_step(terminal_step)
            else:
                self._obs, self._reward, self._done, self._info =  self._env._single_step(decision_step)
            #time.sleep(0.02)
            '''
            time.sleep(0.02)
