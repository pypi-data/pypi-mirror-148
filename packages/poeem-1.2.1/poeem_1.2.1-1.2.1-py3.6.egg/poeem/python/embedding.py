...
class PoeemEmbed(object):
  ...
  def metrics(self, x, x_tau, coarse_code, code):
    ...
    def full_summary(dim, code, name):
      ...
      tally_var = tf.Variable(tf.zeros([dim], dtype=tf.float32),
        name=name + '_cumulative_count', dtype=tf.float32, trainable=False,
        synchronization=tf.VariableSynchronization.ON_READ,
        aggregation=tf.VariableAggregation.SUM)
      update_ops.append(tf.scatter_add(tally_var, v, tf.to_float(counts)))
