...
class MessageProcessor(threading.Thread):
    ...
    def process_client_message(self, message, client):
        ...
        if ACTION in message and message[ACTION] == PRESENCE and TIME in message and USER in message:
            # Если сообщение о присутствии - вызываем функцию авторизации.
    ...
    def autorize_user(self, message, sock):
        ...
        if message[USER][ACCOUNT_NAME] in self.names.keys():
            response = RESPONSE_400
            response[ERROR] = 'К сожалению, имя пользователя уже занято!'
            try:
                SERVER_LOGGER.debug(f'Имя пользователя занято, отправка {response}')
                send_message(sock, response)
            except OSError:
                SERVER_LOGGER.debug('Ошибка операционной системы.')
                pass
        ...
        # Проверяем что пользователь зарегистрирован на сервере.
            ...
            response[ERROR] = 'Пользователь не зарегистрирован на сервере.'
            try:
                SERVER_LOGGER.debug(f'Неизвестное имя пользователя, отправлен ответ {response}')
                send_message(sock, response)
            except OSError:
                pass
            self.clients.remove(sock)
            ...
            SERVER_LOGGER.debug('Имя пользователя корректно, запущена проверка пароля.')
            ...
            digest = hash.digest()
            SERVER_LOGGER.debug(f'Сообщение об авторизации = {message_auth}')
            try:
                # Обмен с клиентом
                send_message(sock, message_auth)
                ans = get_message(sock)
            except OSError as err:
                SERVER_LOGGER.debug('Ошибка авторизации, сообщение:', exc_info=err)
                sock.close()
                return
