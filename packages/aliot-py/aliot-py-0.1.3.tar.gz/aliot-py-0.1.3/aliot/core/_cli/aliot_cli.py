...
@click.option("--obj-id", "-id", default=None, type=click.STRING, prompt=f"Paste the id of the object here"
                                                                         f" (or let it blank to do it later in"
                                                                         f" the {CONFIG_FILE_NAME!r} file)",
              prompt_required=False)
# @click.option("-o", "mode", is_flag=True, help="Specify what you want to make")
def new(object_name: str, obj_id: str):
    success = print_result(
        f"Object {object_name!r} config created successfully", *service.make_obj_config(object_name, obj_id)
    )
    if success is None:
        return
    if success is False:
        override = click.confirm(f"Do you want to override the configuration of {object_name!r}?", default=False)
        if override:
            print_result(
                f"Object {object_name!r} config updated successfully",
                *service.make_obj_config(object_name, obj_id, force=True)
            )
