...
from castor_extractor import file_check  # type: ignore
from castor_extractor.utils import LocalStorage  # type: ignore

logging.basicConfig(level=logging.INFO, format="%(levelname)s - %(message)s")
logger = logging.getLogger(__name__)

WarehouseTemplate = file_check.GenericWarehouseFileTemplate
...
def process(directory: str, verbose: bool):
    ...
    def _check(
        file_name: str,
        template: file_check.FileTemplate,
        *,
        with_ids: bool = True,
    ) -> Ids:
        content = storage.get(file_name)
        checker = file_check.FileCheckerRun(
            content,
            template,
            file_name,
            verbose,
