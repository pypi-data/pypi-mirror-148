...
def golden_section(A, C, x, delta, tau_decimal, function, tol):
    iter_num = 0
    score_dict = {}
    diff = 1e9
    opt_score = None
...
def two_d_golden_section(a, c, A, C, delta, function,
                         tol, bw_decimal, tau_decimal, verbose=False):
    b = a + delta * np.abs(c - a)
    d = c - delta * np.abs(c - a)
    opt_bw = None
    opt_tau = None
    ...
    while np.abs(diff) > tol and iter_num < 200:
        ...
        if b in score_dict:
            ...
            tau_b, score_b = golden_section(A, C, b, delta, tau_decimal, function, tol)
            score_dict[b] = [tau_b, score_b]
        if d in score_dict:
            tau_d, score_d = score_dict[d]
        else:
            tau_d, score_d = golden_section(A, C, d, delta, tau_decimal, function, tol)
        ...
        diff = score_b - score_d
        if verbose:
            print('----------\n bw:%s, tau:%s, score:%s' % (opt_bw, opt_tau, opt_score))
