...
def get_openid_config_url(http_exception, url: str, app_id: Optional[str], tenant_id: Optional[str]) -> str:
    if tenant_id:
        return url.replace("common", settings.internal_tenant_id)
    elif app_id:
        return f"{url}?appid={settings.internal_app_id}"
    else:
...
@contextlib.asynccontextmanager
async def openid_config(base_url: str, http_exception, status_code: int):
