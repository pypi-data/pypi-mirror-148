...
import nlu_bf.train
from nlu_bf.cli.shell import shell
from nlu_bf.cli.utils import create_output_path, print_success, print_error_and_exit
...
def print_train_or_instructions(args: argparse.Namespace, path: Text) -> None:
    ...
    if should_train:
        ...
        args.model = nlu_bf.train(domain, config, training_files, output)
        ...
        print_success(
            ...
            "to the project directory and running 'nlu_bf train'."
            "".format(path)
        )
    ...
    if should_run:
        ...
        if args.no_prompt:
            print(
                ...
                "run 'nlu_bf shell' at any time inside "
...
def print_cancel() -> None:
    print_success("Ok. You can continue setting up by running 'nlu_bf init' 🙋🏽‍♀️")
    exit(0)
