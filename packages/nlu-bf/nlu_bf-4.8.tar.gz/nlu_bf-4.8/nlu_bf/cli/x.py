...
import nlu_bf.cli.utils as cli_utils
import nlu_bf.utils.io as io_utils
from nlu_bf.cli.arguments import x as arguments
...
def _rasa_service(
    ...
    from nlu_bf.core.run import serve_application
    import nlu_bf.utils.common

    # needs separate logging configuration as it is started in its own process
    nlu_bf.utils.common.set_log_level(args.loglevel)
...
def _prepare_credentials_for_rasa_x(
    ...
    if rasa_x_url:
        credentials["nlu_bf"] = {"url": rasa_x_url}
...
def generate_rasa_x_token(length: int = 16):
    ...
    A new token is generated on every `nlu_bf x` command.
    """
...
def _configure_logging(args: argparse.Namespace):
    from nlu_bf.core.utils import configure_file_logging
    from nlu_bf.utils.common import set_log_level
...
def _validate_rasa_x_start(args: argparse.Namespace, project_path: Text):
    ...
    if args.port == args.rasa_x_port:
        cli_utils.print_error_and_exit(
            ...
            "different ports using the arguments '--port' and '--nlu_bf-x-port'.".format(
    ...
    if not is_rasa_project_setup(args, project_path):
        cli_utils.print_error_and_exit(
            "This directory is not a valid Rasa project. Use 'nlu_bf init' "
            "to create a new Rasa project or switch to a valid Rasa project "
            "directory (see http://nlu_bf.com/docs/nlu_bf/user-guide/"
            "nlu_bf-tutorial/#create-a-new-project)."
        )
...
def _validate_domain(domain_path: Text):
    from nlu_bf.core.domain import Domain, InvalidDomain
...
def rasa_x(args: argparse.Namespace):
    from nlu_bf.cli.utils import signal_handler
...
def run_in_production(args: argparse.Namespace):
    from nlu_bf.cli.utils import print_success
...
def run_locally(args: argparse.Namespace):
    ...
    try:
        ...
        cli_utils.print_error(
            ...
            "the issue visit our forum: https://forum.nlu_bf.com/."
        )
    finally:
        process.terminate()
