...
import nlu_bf.cli.train as train
from nlu_bf.cli.arguments import interactive as arguments
from nlu_bf import model

from nlu_bf.constants import DEFAULT_MODELS_PATH, DEFAULT_ENDPOINTS_PATH
    ...
    if args.model is None:
        ...
        if not zipped_model:
            utils.print_error_and_exit(
                ...
                "or use 'nlu_bf train' to train a model."
            )
    else:
        ...
        if not (zipped_model and os.path.exists(zipped_model)):
            utils.print_error_and_exit(
                ...
                f"found at path '{args.model}'.  Use 'nlu_bf train' to train a model."
            )
...
def perform_interactive_learning(
    ...
    from nlu_bf.core.train import do_interactive_learning
