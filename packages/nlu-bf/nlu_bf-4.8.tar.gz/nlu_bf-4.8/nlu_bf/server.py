...
from nlu_bf.utils.common import raise_warning, arguments_of
import nlu_bf.utils.endpoints
import nlu_bf.utils.io
from nlu_bf import model
...
from nlu_bf.core.agent import Agent, load_agent
...
from nlu_bf.core.domain import InvalidDomain
from nlu_bf.core.events import Event
...
from nlu_bf.core.trackers import DialogueStateTracker, EventVerbosity
from nlu_bf.core.utils import AvailableEndpoints
from nlu_bf.nlu.emulators.no_emulator import NoEmulator
...
def _create_emulator(mode: Optional[Text]) -> NoEmulator:
    ...
    if mode is None:
        ...
        from nlu_bf.nlu.emulators.wit import WitEmulator

        return WitEmulator()
    elif mode.lower() == "luis":
        from nlu_bf.nlu.emulators.luis import LUISEmulator

        return LUISEmulator()
    elif mode.lower() == "dialogflow":
        from nlu_bf.nlu.emulators.dialogflow import DialogflowEmulator
...
def add_root_route(app: Sanic):
    ...
    async def hello(request: Request):
        ...
        return response.text("Hello from Rasa: " + nlu_bf.__version__)
...
def create_app(
    ...
    async def retrieve_tracker(request: Request, conversation_id: Text):
        ...
        until_time = nlu_bf.utils.endpoints.float_arg(request, "until")
    ...
    async def train(request: Request) -> HTTPResponse:
        ...
        nlu_bf.utils.io.write_text_file(rjs["config"], config_path)

        if "nlu" in rjs:
            nlu_path = os.path.join(temp_dir, "nlu.md")
            nlu_bf.utils.io.write_text_file(rjs["nlu"], nlu_path)

        if "stories" in rjs:
            stories_path = os.path.join(temp_dir, "stories.md")
            nlu_bf.utils.io.write_text_file(rjs["stories"], stories_path)

        domain_path = DEFAULT_DOMAIN_PATH
        if "domain" in rjs:
            domain_path = os.path.join(temp_dir, "domain.yml")
            nlu_bf.utils.io.write_text_file(rjs["domain"], domain_path)
        ...
        try:
            ...
            from nlu_bf import train as train_model
    ...
    async def evaluate_stories(request: Request):
        ...
        stories = nlu_bf.utils.io.create_temporary_file(request.body, mode="w+b")
        use_e2e = nlu_bf.utils.endpoints.bool_arg(request, "e2e", default=False)
    ...
    async def evaluate_intents(request: Request):
        ...
        nlu_data = nlu_bf.utils.io.create_temporary_file(request.body, mode="w+b")
