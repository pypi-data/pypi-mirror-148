...
from nlu_bf.constants import DOCS_URL_MIGRATE_GOOGLE
import nlu_bf.utils.io
from nlu_bf.nlu import utils
from nlu_bf.nlu.training_data.formats.readerwriter import TrainingDataReader
from nlu_bf.nlu.training_data.util import transform_entity_synonyms
...
class DialogflowReader(TrainingDataReader):
    def read(self, fn: Text, **kwargs: Any) -> "TrainingData":
        ...
        root_js = nlu_bf.utils.io.read_json_file(fn)
    ...
    def _read_examples_js(fn: Text, language: Text, fformat: Text) -> Any:
        ...
        if os.path.isfile(examples_fn):
            return nlu_bf.utils.io.read_json_file(examples_fn)
        else:
            return None
