...
import nlu_bf.nlu.utils
from nlu_bf.utils.common import raise_warning, lazy_property
from nlu_bf.nlu.constants import RESPONSE, RESPONSE_KEY_ATTRIBUTE
from nlu_bf.nlu.training_data.message import Message
from nlu_bf.nlu.training_data.util import check_duplicate_synonym
from nlu_bf.nlu.utils import list_to_str
...
class TrainingData:
    ...
    def __hash__(self) -> int:
        from nlu_bf.core import utils as core_utils
    ...
    def nlu_as_json(self, **kwargs: Any) -> Text:
        ...
        from nlu_bf.nlu.training_data.formats import (  # pytype: disable=pyi-error
            RasaWriter,
        )
    ...
    def persist_nlu(self, filename: Text = DEFAULT_TRAINING_DATA_OUTPUT_PATH):
        ...
        if filename.endswith("json"):
            nlu_bf.nlu.utils.write_to_file(filename, self.nlu_as_json(indent=2))
        elif filename.endswith("md"):
            nlu_bf.nlu.utils.write_to_file(filename, self.nlu_as_markdown())
        else:
            ValueError(
    ...
    def persist_nlg(self, filename: Text) -> None:
        ...
        nlu_bf.nlu.utils.write_to_file(filename, self.nlg_as_markdown())
