...
from nlu_bf.constants import TEST_DATA_FILE, TRAIN_DATA_FILE, NLG_DATA_FILE
...
from nlu_bf.nlu import config, training_data, utils
from nlu_bf.nlu.utils import write_to_file
from nlu_bf.nlu.components import ComponentBuilder
from nlu_bf.nlu.config import RasaNLUModelConfig
from nlu_bf.nlu.model import Interpreter, Trainer, TrainingData
...
def compare_nlu(
    ...
    from nlu_bf.train import train_nlu
...
if __name__ == "__main__":
    raise RuntimeError(
        "Calling `nlu_bf.nlu.test` directly is no longer supported. Please use "
        "`nlu_bf test` to test a combined Core and NLU model or `nlu_bf test nlu` "
