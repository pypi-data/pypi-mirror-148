...
import nlu_bf
import nlu_bf.utils.io
import nlu_bf.core.utils
...
from nlu_bf.core import constants, jobs, training
from nlu_bf.core.channels.channel import InputChannel, OutputChannel, UserMessage
from nlu_bf.core.constants import DEFAULT_REQUEST_TIMEOUT
from nlu_bf.core.domain import Domain
from nlu_bf.core.exceptions import AgentNotReady
from nlu_bf.core.interpreter import NaturalLanguageInterpreter, RegexInterpreter
from nlu_bf.core.lock_store import LockStore, InMemoryLockStore
from nlu_bf.core.nlg import NaturalLanguageGenerator
from nlu_bf.core.policies.ensemble import PolicyEnsemble, SimplePolicyEnsemble
from nlu_bf.core.policies.memoization import MemoizationPolicy
from nlu_bf.core.policies.policy import Policy
from nlu_bf.core.processor import MessageProcessor
from nlu_bf.core.tracker_store import (
...
from nlu_bf.model import (
    get_model_subdirectories,
    get_latest_model,
    unpack_model,
    get_model,
)
from nlu_bf.nlu.utils import is_url
from nlu_bf.utils.common import raise_warning, update_sanic_log_level
...
def _load_and_set_updated_model(
    ...
    if nlu_path:
        from nlu_bf.core.interpreter import RasaNLUInterpreter
    ...
    async with model_server.session() as session:
        try:
            ...
            async with session.request(
                ...
                nlu_bf.utils.io.unarchive(await resp.read(), model_directory)
                logger.debug(
...
class Agent:
    ...
    def handle_channels(
        ...
        from nlu_bf.core import run

        raise_warning(
            "Using `handle_channels` is deprecated. "
            "Please use `nlu_bf.run(...)` or see "
            "`nlu_bf.core.run.configure_app(...)` if you want to implement "
        ...
        app.run(
            ...
            workers=nlu_bf.core.utils.number_of_sanic_workers(self.lock_store),
        )
    ...
    async def visualize(
        ...
        from nlu_bf.core.training.visualization import visualize_stories
    ...
    def load_from_remote_storage(
        ...
        from nlu_bf.nlu.persistor import get_persistor
