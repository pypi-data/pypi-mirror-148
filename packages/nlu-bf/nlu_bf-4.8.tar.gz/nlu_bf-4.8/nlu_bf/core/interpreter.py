...
from nlu_bf.constants import DOCS_URL_STORIES
from nlu_bf.core import constants
from nlu_bf.core.trackers import DialogueStateTracker
from nlu_bf.core.constants import INTENT_MESSAGE_PREFIX
from nlu_bf.utils.common import raise_warning, class_from_module_path
...
class RasaNLUHttpInterpreter(NaturalLanguageInterpreter):
    ...
    async def _rasa_http_parse(
        ...
        """Send a text message to a running nlu_bf NLU http server.
        Return `None` on failure."""

        if not self.endpoint_config:
            logger.error(
                f"Failed to parse text '{text}' using nlu_bf NLU over http. "
                f"No nlu_bf NLU server specified!"
            )
            return None
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=params) as resp:
                    if resp.status == 200:
                        ...
                        logger.error(
                            f"Failed to parse text '{text}' using nlu_bf NLU over "
                            f"http. Error: {response_text}"
                        )
                        return None
        except Exception:
            logger.exception(f"Failed to parse text '{text}' using nlu_bf NLU over http.")
            return None
