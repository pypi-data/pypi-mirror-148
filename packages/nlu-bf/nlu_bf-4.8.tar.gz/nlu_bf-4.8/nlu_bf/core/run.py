...
from nlu_bf import model, server
from nlu_bf.constants import ENV_SANIC_BACKLOG
from nlu_bf.core import agent, channels, constants
...
from nlu_bf.core.channels.channel import InputChannel
...
def create_http_input_channels(
    ...
    if credentials_file:
        all_credentials = nlu_bf.utils.io.read_config_file(credentials_file)
    else:
        all_credentials = {}
...
def _create_single_channel(channel, credentials) -> Any:
    from nlu_bf.core.channels import BUILTIN_CHANNELS
    ...
    if channel in BUILTIN_CHANNELS:
        ...
        try:
            input_channel_class = nlu_bf.utils.common.class_from_module_path(channel)
...
def configure_app(
    ...
    """Run the agent."""
    from nlu_bf import server

    nlu_bf.core.utils.configure_file_logging(logger, log_file)
    ...
    if logger.isEnabledFor(logging.DEBUG):
        nlu_bf.core.utils.list_routes(app)

    # configure async loop logging
    async def configure_async_logging():
        if logger.isEnabledFor(logging.DEBUG):
            nlu_bf.utils.io.enable_async_loop_debugging(asyncio.get_event_loop())
...
def serve_application(
    ...
    nlu_bf.utils.common.update_sanic_log_level(log_file)
    ...
    app.run(
        ...
        workers=nlu_bf.core.utils.number_of_sanic_workers(
...
if __name__ == "__main__":
    raise RuntimeError(
        "Calling `nlu_bf.core.run` directly is no longer supported. "
        "Please use `nlu_bf run` to start a Rasa server or `nlu_bf shell` to chat with "
