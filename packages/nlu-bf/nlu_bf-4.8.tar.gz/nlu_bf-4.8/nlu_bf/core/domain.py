...
import nlu_bf.core.constants
from nlu_bf.utils.common import (
    raise_warning,
    lazy_property,
    sort_list_of_dicts_by_first_key,
)
import nlu_bf.utils.io
from nlu_bf.cli.utils import bcolors, wrap_with_color
...
from nlu_bf.core.actions import action  # pytype: disable=pyi-error
from nlu_bf.core.actions.action import Action  # pytype: disable=pyi-error
...
from nlu_bf.core.events import SlotSet, UserUttered
from nlu_bf.core.slots import Slot, UnfeaturizedSlot, CategoricalSlot
from nlu_bf.utils.endpoints import EndpointConfig
from nlu_bf.utils.validation import InvalidYamlFileError, validate_yaml_schema
...
class Domain:
    ...
    def from_file(cls, path: Text) -> "Domain":
        return cls.from_yaml(nlu_bf.utils.io.read_file(path))
    ...
    def from_yaml(cls, yaml: Text) -> "Domain":
        ...
        data = nlu_bf.utils.io.read_yaml(yaml)
    ...
    def persist_specification(self, model_path: Text) -> None:
        ...
        nlu_bf.utils.io.create_directory_for_file(domain_spec_path)

        metadata = {"states": self.input_states}
        nlu_bf.utils.io.dump_obj_as_json_to_file(domain_spec_path, metadata)
    ...
    def load_specification(cls, path: Text) -> Dict[Text, Any]:
        ...
        specification = json.loads(nlu_bf.utils.io.read_file(metadata_path))
    ...
    def cleaned_domain(self) -> Dict[Text, Any]:
        ...
        for slot in domain_data["slots"].values():  # pytype: disable=attribute-error
            ...
            if slot["auto_fill"]:
                del slot["auto_fill"]
            if slot["type"].startswith("nlu_bf.core.slots"):
    ...
    def _actions_for_domain_warnings(self) -> List[Text]:
        ...
        from nlu_bf.core.actions.action import (  # pytype: disable=pyi-error
    ...
    def check_missing_templates(self) -> None:
        ...
        utterances = [
            ...
            if a.startswith(nlu_bf.core.constants.UTTER_PREFIX)
        ]
