...
from nlu_bf.constants import MINIMUM_COMPATIBLE_VERSION, DOCS_BASE_URL, DOCS_URL_POLICIES

from nlu_bf.core import utils
from nlu_bf.core.constants import USER_INTENT_BACK, USER_INTENT_RESTART
from nlu_bf.core.actions.action import (
    ACTION_LISTEN_NAME,
    ACTION_BACK_NAME,
    ACTION_RESTART_NAME,
)
from nlu_bf.core.domain import Domain
from nlu_bf.core.events import SlotSet, ActionExecuted, ActionExecutionRejected, Event
from nlu_bf.core.exceptions import UnsupportedDialogueModelError
from nlu_bf.core.featurizers import MaxHistoryTrackerFeaturizer
...
from nlu_bf.core import registry
...
class PolicyEnsemble:
    versioned_packages = ["nlu_bf", "tensorflow", "sklearn"]
    ...
    def load_metadata(cls, path) -> Any:
        ...
        metadata = json.loads(nlu_bf.utils.io.read_file(os.path.abspath(metadata_path)))
        return metadata
    ...
    def ensure_model_compatibility(metadata, version_to_check=None) -> None:
        ...
        model_version = metadata.get("nlu_bf", "0.0.0")
        if version.parse(model_version) < version.parse(version_to_check):
            raise UnsupportedDialogueModelError(
                ...
                "".format(model_version, nlu_bf.__version__, version_to_check),
                model_version,
            )
