...
import nlu_bf.utils.common as common_utils
...
class KerasPolicy(Policy):
    ...
    def persist(self, path: Text) -> None:
        ...
        if self.model:
            ...
            nlu_bf.utils.io.dump_obj_as_json_to_file(meta_file, meta)

            model_file = os.path.join(path, meta["model"])
            # makes sure the model directory exists
            nlu_bf.utils.io.create_directory_for_file(model_file)
    ...
    def load(cls, path: Text) -> "KerasPolicy":
        ...
        if os.path.exists(path):
            ...
            if os.path.isfile(meta_file):
                meta = json.loads(nlu_bf.utils.io.read_file(meta_file))
