...
from nlu_bf.constants import NUMBER_OF_TRAINING_STORIES_FILE, PERCENTAGE_KEY
from nlu_bf.core.domain import Domain
from nlu_bf.importers.importer import TrainingDataImporter
from nlu_bf.utils.common import TempDirectoryPath
...
async def train(
    ...
    from nlu_bf.core import config, utils
    from nlu_bf.core.utils import AvailableEndpoints
...
async def get_no_of_stories(story_file: Text, domain: Text) -> int:
    ...
    from nlu_bf.core.domain import TemplateDomain
...
async def do_compare_training(
    ...
    nlu_bf.utils.io.dump_obj_as_json_to_file(training_stories_per_model_file, story_range)
...
if __name__ == "__main__":
    raise RuntimeError(
        "Calling `nlu_bf.core.train` directly is no longer supported. Please use "
        "`nlu_bf train` to train a combined Core and NLU model or `nlu_bf train core` "
