...
from nlu_bf.core.brokers.kafka import KafkaEventBroker
from nlu_bf.core.brokers.pika import PikaEventBroker
from nlu_bf.core.brokers.sql import SQLEventBroker
from nlu_bf.core.events import Event, Restarted, SlotSet, UserUttered
...
def test_load_custom_broker_name():
    config = EndpointConfig(**{"type": "nlu_bf.core.brokers.file.FileEventBroker"})
    assert EventBroker.create(config)


def test_load_non_existent_custom_broker_name():
    config = EndpointConfig(**{"type": "nlu_bf.core.brokers.my.MyProducer"})
