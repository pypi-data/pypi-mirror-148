...
from nlu_bf.core.actions import action
from nlu_bf.core.channels import UserMessage
from nlu_bf.core.domain import Domain
from nlu_bf.core.events import BotUttered
from nlu_bf.core.trackers import DialogueStateTracker
from nlu_bf.core.training import interactive
from nlu_bf.importers.nlu_bf import TrainingDataImporter
from nlu_bf.nlu.training_data import Message
from nlu_bf.nlu.training_data.loading import RASA, MARKDOWN
...
def test_latest_user_message():
    ...
    tracker_json = json.loads(nlu_bf.utils.io.read_file(tracker_dump))
...
async def test_print_history(mock_endpoint):
    tracker_dump = nlu_bf.utils.io.read_file("data/test_trackers/tracker_moodbot.json")
...
def test_splitting_conversation_at_restarts():
    tracker_dump = "data/test_trackers/tracker_moodbot.json"
    evts = json.loads(nlu_bf.utils.io.read_file(tracker_dump)).get("events")
...
def test_as_md_message():
    parse_data = {
        "text": "Hello there nlu_bf.",
        "entities": [{"start": 12, "end": 16, "entity": "name", "value": "nlu_bf"}],
        "intent": {"name": "greeting", "confidence": 0.9},
    }
    md = interactive._as_md_message(parse_data)
    assert md == "Hello there [nlu_bf](name)."


def test_entity_annotation_merge_with_original():
    parse_original = {
        "text": "Hello there nlu_bf, it's me, paula.",
        "entities": [
            {
                "start": 12,
                "end": 16,
    ...
    parse_annotated = {
        "text": "Hello there nlu_bf, it's me, paula.",
        "entities": [
            {"start": 12, "end": 16, "entity": "name1", "value": "nlu_bf"},
...
async def test_interactive_domain_persistence(mock_endpoint, tmpdir):
    ...
    tracker_dump = "data/test_trackers/tracker_moodbot.json"
    tracker_json = nlu_bf.utils.io.read_json_file(tracker_dump)
    ...
    saved_domain = nlu_bf.utils.io.read_config_file(domain_path)
