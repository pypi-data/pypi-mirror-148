...
from nlu_bf.core import jobs, utils
...
from nlu_bf.core.interpreter import INTENT_MESSAGE_PREFIX
from nlu_bf.core.policies.ensemble import PolicyEnsemble, SimplePolicyEnsemble
from nlu_bf.core.policies.memoization import AugmentedMemoizationPolicy, MemoizationPolicy
...
async def test_agent_with_model_server_in_thread(
    ...
    agent = Agent()
    agent = await nlu_bf.core.agent.load_from_server(
...
async def test_wait_time_between_pulls_without_interval(model_server, monkeypatch):
    monkeypatch.setattr(
        "nlu_bf.core.agent.schedule_model_pulling", lambda *args: 1 / 0
    ...
    await nlu_bf.core.agent.load_from_server(agent, model_server=model_endpoint_config)
    jobs.kill_scheduler()
