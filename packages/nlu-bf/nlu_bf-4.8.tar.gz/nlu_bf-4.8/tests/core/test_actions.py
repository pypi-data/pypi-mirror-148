...
from nlu_bf.core.channels import CollectingOutputChannel
from nlu_bf.core.domain import Domain, SessionConfig
...
from nlu_bf.core.nlg.template import TemplatedNaturalLanguageGenerator
from nlu_bf.core.constants import USER_INTENT_SESSION_START
from nlu_bf.core.trackers import DialogueStateTracker
from nlu_bf.utils.endpoints import ClientResponseError, EndpointConfig
...
async def test_remote_action_logs_events(
    ...
    response = {
        "events": [{"event": "slot", "value": "nlu_bf", "name": "name"}],
        "responses": [
            {
                "text": "test text",
                "template": None,
    ...
    assert events[2] == SlotSet("name", "nlu_bf")
