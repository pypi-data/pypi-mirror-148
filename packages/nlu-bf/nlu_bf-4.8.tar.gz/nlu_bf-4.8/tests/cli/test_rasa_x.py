...
def test_x_help(run: Callable[..., RunResult]):
    ...
    help_text = """usage: nlu_bf x [-h] [-v] [-vv] [--quiet] [-m MODEL] [--data DATA] [-c CONFIG]
              [--no-prompt] [--production] [--nlu_bf-x-port RASA_X_PORT]
...
def test_prepare_credentials_for_rasa_x_if_rasa_channel_not_given(tmpdir: Path):
    ...
    assert actual["nlu_bf"]["url"] == "http://localhost:5002"
...
async def test_pull_runtime_config_from_server():
    ...
    credentials = "nlu_bf: http://example.com:5002/api"
    endpoint_config = """
    event_broker:
