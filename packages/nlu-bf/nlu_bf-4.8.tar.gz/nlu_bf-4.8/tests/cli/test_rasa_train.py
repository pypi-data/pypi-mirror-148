...
from nlu_bf.nlu.training_data import training_data
from nlu_bf.cli.train import _get_valid_config
...
def test_train_core(run_in_default_project_without_models: Callable[..., RunResult]):
    ...
    assert os.path.exists("train_rasa_models/nlu_bf-model.tar.gz")
    assert os.path.isfile("train_rasa_models/nlu_bf-model.tar.gz")
...
def test_train_core_no_domain_exists(
    ...
    assert not os.path.exists("train_rasa_models_no_domain/nlu_bf-model.tar.gz")
    assert not os.path.isfile("train_rasa_models_no_domain/nlu_bf-model.tar.gz")
...
def test_train_help(run):
    ...
    help_text = """usage: nlu_bf train [-h] [-v] [-vv] [--quiet] [--data DATA [DATA ...]]
...
def test_train_nlu_help(run: Callable[..., RunResult]):
    ...
    help_text = """usage: nlu_bf train nlu [-h] [-v] [-vv] [--quiet] [-c CONFIG] [--out OUT]
...
def test_train_core_help(run: Callable[..., RunResult]):
    ...
    help_text = """usage: nlu_bf train core [-h] [-v] [-vv] [--quiet] [-s STORIES] [-d DOMAIN]
...
def test_get_valid_config(parameters):
    ...
    if parameters["config_data"] is not None:
        ...
        nlu_bf.utils.io.write_yaml_file(parameters["config_data"], config_path)

    default_config_path = None
    if parameters["default_config"] is not None:
        default_config_path = os.path.join(tempfile.mkdtemp(), "default-config.yml")
        nlu_bf.utils.io.write_yaml_file(parameters["default_config"], default_config_path)
    ...
    if parameters["error"]:
        ...
        config_data = nlu_bf.utils.io.read_yaml_file(config_path)
