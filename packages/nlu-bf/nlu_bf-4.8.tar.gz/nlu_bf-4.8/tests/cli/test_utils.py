...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["nlu_bf", "run", "actions"],
        ["nlu_bf", "run", "core"],
        ["nlu_bf", "interactive", "nlu", "--param", "xy"],
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["nlu_bf", "run", "actions"],
        ["nlu_bf", "run", "core"],
        ["nlu_bf", "test", "nlu", "--param", "xy", "--model", "test"],
    ],
)
...
def test_validate_with_none_if_default_is_valid(caplog: LogCaptureFixture):
    ...
    with caplog.at_level(logging.WARNING, nlu_bf.cli.utils.logger.name):
...
def test_print_error_and_exit():
    with pytest.raises(SystemExit):
        nlu_bf.cli.utils.print_error_and_exit("")
