...
from nlu_bf.utils.io import write_yaml_file


@pytest.fixture
def run(testdir: Testdir) -> Callable[..., RunResult]:
    def do_run(*args):
        args = ["nlu_bf"] + list(args)
...
def init_default_project(tmpdir_factory: TempdirFactory) -> str:
    ...
    check_call(["nlu_bf", "init", "--no-prompt"], cwd=path)
...
def _set_up_initial_project(testdir: Testdir):
    ...
    testdir.copy_example("nlu_bf/cli/initial_project/actions.py")
    testdir.copy_example("nlu_bf/cli/initial_project/credentials.yml")
    testdir.copy_example("nlu_bf/cli/initial_project/domain.yml")
    testdir.copy_example("nlu_bf/cli/initial_project/endpoints.yml")
    testdir.mkdir("data")
    testdir.copy_example("nlu_bf/cli/initial_project/data")
