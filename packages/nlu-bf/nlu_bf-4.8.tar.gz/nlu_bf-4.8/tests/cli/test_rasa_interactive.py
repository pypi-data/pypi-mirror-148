...
import nlu_bf
from nlu_bf.cli import interactive, train


def test_interactive_help(run: Callable[..., RunResult]):
    output = run("interactive", "--help")

    help_text = """usage: nlu_bf interactive [-h] [-v] [-vv] [--quiet] [--e2e] [-m MODEL]
...
def test_interactive_core_help(run: Callable[..., RunResult]):
    ...
    help_text = """usage: nlu_bf interactive core [-h] [-v] [-vv] [--quiet] [-m MODEL] [-s STORIES]
                             [--skip-visualization]
                             [--conversation-id CONVERSATION_ID]
                             [--endpoints ENDPOINTS] [-c CONFIG] [-d DOMAIN]
                             [--out OUT] [--augmentation AUGMENTATION]
...
def test_pass_arguments_to_rasa_train(
    ...
    mock = Mock()
    monkeypatch.setattr(nlu_bf, "train", mock.method)
...
def test_pass_conversation_id_to_interactive_learning(monkeypatch: MonkeyPatch):
    ...
    from nlu_bf.core.training import interactive as interactive_learning
