...
from nlu_bf.core.interpreter import (
    NaturalLanguageInterpreter,
    RasaNLUHttpInterpreter,
    RasaNLUInterpreter,
    RegexInterpreter,
)
from nlu_bf.model import get_model_subdirectories, get_model
from nlu_bf.nlu import registry, training_data
...
async def test_interpreter_on_pipeline_templates(
    ...
    test_data = "data/examples/nlu_bf/demo-nlu_bf.json"
    ...
    interpreter = await utilities.interpreter_for(
        component_builder, "data/examples/nlu_bf/demo-nlu_bf.json", tmpdir.strpath, config
    )
...
def test_model_is_not_compatible(metadata):
    with pytest.raises(nlu_bf.nlu.model.UnsupportedModelError):
        Interpreter.ensure_model_compatibility(metadata)


@pytest.mark.parametrize("metadata", [{"rasa_version": nlu_bf.__version__}])
