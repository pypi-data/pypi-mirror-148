...
def confirm_list_columns(
    ...
    for dim in dimensions:
        if dim not in columns:
            ...
            if naming._cleanse_sql_name(dim) in columns:
                consider.append(naming._cleanse_sql_name(dim))
        else:
    ...
    for ft in list(features):
        if ft not in columns:
            ...
            if naming._cleanse_sql_name(ft) in columns:
                consider.append(naming._cleanse_sql_name(ft))
        else:
...
def _cleanse_sql_dataframe(df: pd.DataFrame):
    """
    Renames all columns in a pandas dataframe to Snowflake compliant names in place
    """
    df.rename(columns={r: naming._cleanse_sql_name(r) for r in build_schema(df)}, inplace=True)
