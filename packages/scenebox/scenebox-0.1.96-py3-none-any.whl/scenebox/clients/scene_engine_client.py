...
class SceneEngineClient(object):
    ...
    def add_annotations(self,
        ...
        for i in range(0, index_rounds):
            ...
            annotations_chunks = chunk_list(annotations[i * asset_count_per_round:(i + 1) * asset_count_per_round],
                                            CHUNK_SIZE)
            raw_annotations_chunks = chunk_list(raw_annotations[i * asset_count_per_round:(i + 1) * asset_count_per_round],
    ...
    def add_images(
        ...
        def process_image_chunk_add(iterable):
            ...
            # Process Image object information to lists
            for image in images_chunk:
                image_ids.append(image.id)
                if image.path is not None:
                    image_paths_list.append(image.path)
                if image.url is not None:
                    image_urls_list.append(image.url)
                if image.uri is not None:
                    image_uris_list.append(image.uri)
                    uri_object_access_list.append(ObjectAccess(uri=image.uri).to_dic())
                if image.bytes is not None:
                    image_bytes_list.append(image.bytes)

            # Ensure all images have only one of these available
            if not (len(image_paths_list) == chunk_length or len(image_urls_list) == chunk_length \
                    or len(image_uris_list) == chunk_length or len(image_bytes_list) == chunk_length):
                raise InvalidArgumentsError(
            ...
            if uri_object_access_list:
                url = images_amc.get_assets_url("url_from_object_access_in_batch/")
                params = images_amc.add_asset_manager_params()
                resp = requests.post(url, params=params, json={"ids": image_ids,
            ...
            def process_image_metadata(image):
                ...
                if image.bytes:
                    image_attributes = get_image_attributes(image_bytes=image.bytes)
                    filename = image.id
                elif image.path:
                    filename, _, _, image_format = parse_file_path(image.path)
                    image_attributes = get_image_attributes(image_path=image.path)
                    with open(image.path, 'rb') as f:
                        image_id_bytes_map[image.id] = io.BytesIO(f.read())
                elif image.url:
                    filename = image.url.split("/")[-1]
                    image_attributes = get_image_attributes(image_url=image.url)
                elif image.uri:
                    filename = UriParser(uri=image.uri).key.split("/")[-1]
                    image_url = id_url_map.get(image.id)
                    if not image_url:
                        AssetError("could not get url for image {}".format(image.id))
                    image_attributes = get_image_attributes(image_url=image_url)
                else:
                    raise ValueError(
                        "Either image file path, image bytes, uri or url must be provided")
            ...
            for image in images_chunk:
                ...
                if image_id_bytes_map:
                    image_bytes_list.append(image_id_bytes_map[image.id])
                if image_id_content_type_map:
                    image_content_types_list.append(image_id_content_type_map[image.id])

            ids = images_amc.put_assets_batch(
                metadata=image_metadata_list,
                filenames=image_filenames_list,
                file_objects=image_bytes_list or None,
                urls=image_urls_list or None,
                ids=image_ids,
                uris=image_uris_list or None,
                geo_field=geo_field,
                shape_group_field=shape_group_field,
                nested_fields=nested_fields,
                wait_for_completion=True,
                content_type=image_content_types_list or None,
                threading=True,
                disable_tqdm=True,
                add_to_redis_cache=add_to_cache)

            if add_to_session and image_id_entity_map:
        ...
        run_threaded(func=process_image_chunk_add,
                     ...
                     disable_threading=False,
                     disable_tqdm=False,
                     unit=f" {CHUNK_SIZE} images"
                     )

        added_image_ids = []
        for chunk_id in range(len(images_list_chunked)):
            added_image_ids.extend(chunk_id_added_image_ids_map[chunk_id])
