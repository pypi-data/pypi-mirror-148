...
class BreezeConnect():
    ...
    def get_data_from_stock_token_value(self, input_stock_token):
        ...
        # exchange_code_name = NSE
        exchange_code_name = exchange_code_list.get(exchange_type, False)
        if exchange_code_name == False:
            raise GetStockTokenError(message = "Stock-Token cannot be found due to wrong exchange-code.")
        elif exchange_code_name.lower() == "bse":
            stock_data = self.token_script_dict_list[0].get(stock_token,False)
            if stock_data == False:
                raise GetStockTokenError(message = "Stock-Data does not exist in exchange-code BSE for Stock-Token "+input_stock_token+".")
        elif exchange_code_name.lower() == "nse":
            stock_data = self.token_script_dict_list[1].get(stock_token,False) # CHecing data present in NSE
            if stock_data == False:
                stock_data = self.token_script_dict_list[4].get(stock_token,False)
                if stock_data == False:
                    raise GetStockTokenError(message = "Stock_Token does not exist in both exchange-code i.e. NSE or NFO.")
                else:
                    exchange_code_name = "NFO"
        elif exchange_code_name.lower() == "ndx":
            stock_data = self.token_script_dict_list[2].get(stock_token,False)
            if stock_data == False:
                raise GetStockTokenError(message = "Stock-Data does not exist in exchange-code NDX for Stock-Token "+input_stock_token+".")
        elif exchange_code_name.lower() == "mcx":
            stock_data = self.token_script_dict_list[3].get(stock_token,False)
            if stock_data == False:
                raise GetStockTokenError(message = "Stock-Data does not exist in exchange-code MCX for Stock-Token "+input_stock_token+".")
    ...
    def parse_data(self, data):
        ...
        if exchange == '6':
            ...
            data_dict["LastTradeTime"] = datetime.fromtimestamp(data[7]).strftime('%c')
