...
# sources: Constants.proto, Datawatch.proto, FeatureProviders.proto, Forecast.proto, ModelService.proto
...
class DataNodeType(betterproto.Enum):
    ...
    DATA_NODE_TYPE_REPORT_TABLEAU = 2
...
class AsyncBatchRunMetricsRequest(betterproto.Message):
    ...
    workflow_id: int = betterproto.int64_field(2)
    workflow_process_id: int = betterproto.int64_field(3)
    workflow_processing_type: "WorkflowProcessingType" = betterproto.enum_field(4)
...
@dataclass
class RunTestScriptJob(betterproto.Message):
    test_script_id: int = betterproto.int32_field(1)
...
class JobDefinition(betterproto.Message):
    ...
    run_test_script_job: "RunTestScriptJob" = betterproto.message_field(4, group="job")
...
class WhereClause(betterproto.Message):
    ...
    column_id: int = betterproto.int32_field(7)
...
class CatalogIndexRequest(betterproto.Message):
    id: int = betterproto.int32_field(1)
    workflow_type: "WorkflowProcessingType" = betterproto.enum_field(2)
    workflow_id: int = betterproto.int64_field(3)
...
@dataclass
class MonocleModelInput(betterproto.Message):
...
class MetricServiceStub(betterproto.ServiceStub):
    ...
    async def get_metric_run_history(
        self,
        *,
        metric_identifier: Optional["MetricIdentifier"] = None,
        days_of_history: int = 0,
        include_failed_runs: bool = False,
        max_number_of_points: int = 0,
    ) -> MetricRunHistory:
        """Get metric run history"""

        request = GetMetricRunHistoryRequest()
        if metric_identifier is not None:
            request.metric_identifier = metric_identifier
        request.days_of_history = days_of_history
        request.include_failed_runs = include_failed_runs
        request.max_number_of_points = max_number_of_points

        return await self._unary_unary(
            "/com.torodata.models.generated.MetricService/GetMetricRunHistory",
            request,
            MetricRunHistory,
        )
...
class CatalogRebuildsServiceStub(betterproto.ServiceStub):
    ...
    async def update_source(self, *, source_id: int = 0) -> WorkflowResponse:
        request = SinglePathParamSourceIdRequest()
        request.source_id = source_id
    ...
    async def update_schema(self, *, schema_id: int = 0) -> WorkflowResponse:
        request = SinglePathParamSchemaIdRequest()
        request.schema_id = schema_id
