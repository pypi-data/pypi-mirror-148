...
prj_file = load_config("pyproject.toml")
...
if environ.get("MPB_CONFIG_HOME"):
    _user_config_file = path.join(
        environ.get("MPB_CONFIG_HOME"), user_config_file_name
    )
elif environ.get("XDG_CONFIG_HOME"):
    _user_config_file = path.join(
        environ.get("XDG_CONFIG_HOME"), prj_file['tool']['poetry']['name'], user_config_file_name
    )
elif environ.get("APPDATA"):
    _user_config_file = path.join(
        environ.get("APPDATA"), prj_file['tool']['poetry']['name'], user_config_file_name
    )
else:
    _user_config_file = path.join(
        environ["HOME"], ".config", prj_file['tool']['poetry']['name'], user_config_file_name
    )
