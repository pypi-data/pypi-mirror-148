...
DC_COMMANDS = ['install', 'uninstall', 'upgrade', 'enable', 'disable']
...
TOOLS={
  ...
  "dl-model-training-services":['cnvrg','kubeflow','one-api','redhat-data-science','sonoma-creek'],
  ...
  "edge-to-cloud-services":['aws-solutions','azure-solutions','dev-catalog','market-ready-solutions','redhat-marketplace','select-solutions','seo-exp-kits'],
...
class DC(cmd2.Cmd):
    ...
    def __init__(self):
        ...
        self.allow_cli_args = True  # disable parsing of command-line args by cmd2
        ...
        if begidx>4:
              for t in TOOLS:                
                if line.__contains__(t):
                    if t == "app-services":
                        SUB_TOOLS=TOOLS[t]
                        CMD_TOOLS = APP_COMMANDS
                        TM_TOOLS = TM_SERVICE_TOOLS
                    else:
                        SUB_TOOLS=TOOLS[t]
