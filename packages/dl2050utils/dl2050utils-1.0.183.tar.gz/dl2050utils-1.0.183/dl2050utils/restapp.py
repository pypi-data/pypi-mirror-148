...
from dl2050utils.restutils import HTTPException, rest_ok, rest_error, get_meta, get_required_args,\
                                  get_required, get_optional_args, mk_key, get_hash
...
class App():
    ...
    def get_routes(self):
        BASE_ROUTES = [
            ...
            Route('/api/upload/{size}', endpoint=self.upload, methods=['GET']),
    ...
    async def upload(self, request):
        ...
        self.perm(self.d, u, request.url.path, {'size':size})
        res = {'key':mk_key(16), 'size':size}
        secret = oget(self.cfg,['fs','secret'])
        if not secret:
            return rest_error(self.LOG, 'APP', 'upload', 'Upload secret not defined')
        h = get_hash(res, secret)
        res['h'] = h
        return rest_ok(res)
