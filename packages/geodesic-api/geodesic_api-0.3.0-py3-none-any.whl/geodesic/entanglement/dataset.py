...
class Dataset(Object):
    ...
    def from_arcgis_service(name: str, url: str, credentials=None) -> 'Dataset':
        ...
        if dataset_alias is None:
            ...
            if item_id is None:
                ...
                return Dataset.from_arcgis_item(name=name, item_id=item_id, credentials=credentials)
    ...
    @staticmethod
    def from_stac_collection(name: str, url: str, credentials=None, item_type: str = 'raster') -> 'Dataset':
        if url.endswith('/'):
            url = url[:-1]
