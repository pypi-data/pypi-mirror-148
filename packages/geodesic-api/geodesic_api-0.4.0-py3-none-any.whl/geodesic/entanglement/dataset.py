...
class Dataset(Object):
    ...
    def warp(self, *,
        ...
        res = boson_client.post(f'raster/{self.name}.{self.project.name}/warp', **req)
    ...
    @staticmethod
    def from_arcgis_service(name: str, url: str, credential=None) -> 'Dataset':
        if url.endswith('/'):
            url = url[:-1]
        ...
        if dataset_alias is None:
            ...
            if item_id is None:
                ...
                return Dataset.from_arcgis_item(name=name, item_id=item_id, credential=credential)
    ...
    @staticmethod
    def from_stac_collection(name: str, url: str, credential=None, item_type: str = 'raster') -> 'Dataset':
        if url.endswith('/'):
            url = url[:-1]
        ...
        stac_cfg = {}
        if item_assets:
            stac_cfg['item_assets'] = {}
        for key, asset in item_assets.items():
            stac_cfg['item_assets'][key] = asset
