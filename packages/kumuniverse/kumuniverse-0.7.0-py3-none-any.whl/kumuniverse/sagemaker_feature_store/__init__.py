...
class SagemakerFeatureStore:
    def __init__(
        ...
        if redis_host and redis_password and redis_port:
            # Store credentials
            ...
            self.redis_client = self._init_redis(redis_host, redis_password, redis_port)
    ...
    def create_feature_group(
        ...
        if not enable_online_store:
            ...
            if "dynamodb" not in online_store_sources:
                online_store_sources.append("dynamodb")
        ...
        config_data = {
            ...
            "enable_offline_store": enable_online_store,
    ...
    def delete_feature_group(self, feature_group_name: str):
        ...
        try:
            ...
            for key in self.redis_client.scan_iter(f"{feature_group_name}:*"):
    ...
    def ingest_spark_df(
        ...
        def ingest_parallelize_spark_df_multithread(
            feature_group_name, rows, record_identifier, redis_config, thread_num=10
        ):
            if redis_config:
                redis_client = Redis(
            ...
            def ingest_worker(rows, feature_group_name, ops_limit=3200):
                ...
                for row in rows:
                    # Ingest to DynamoDB online store
                    record = [
                    ...
                    if is_redis:
                        ...
                        mapping = {column: row[column] for column in row.__fields__}
    ...
    def describe_feature_group(self, feature_group_name: str, features=None):
        ...
        print("\nDESCRIPTION:\n", json.dumps(custom_feature, indent=2, default=str))
    ...
    def _init_redis(self, redis_host: str, redis_password: str, redis_port: int):
        """
