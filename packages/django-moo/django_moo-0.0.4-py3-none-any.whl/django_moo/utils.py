import warnings, datetime, copy, os
...
def get_user_all_apps_name():
    app_ls = [app for app in settings.INSTALLED_APPS if '.' not in app]
    apps_name = set(os.listdir(settings.BASE_DIR)) & set(app_ls)
    return apps_name

def get_user_all_apps_name_path():
...
class QuerySetPaginateObject:  # per_page再settings中可以设置
    def __init__(self, query, per_page, index_page, orphans=0, allow_empty_first_page=True) -> None:
...
class QToDict:
    ...
    def get_query_attrs(self, query):
        from django_moo.models import SampleModel
        u = set(dir(query.__class__))
        s = set(dir(SampleModel))
        s.remove('id')
        attrs = u - s
