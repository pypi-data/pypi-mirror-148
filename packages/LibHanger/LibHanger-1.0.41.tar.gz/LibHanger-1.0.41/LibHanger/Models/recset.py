...
class recset(Generic[T]):
    ...
    def __checkSessionOpen(self, __autoCommit = True, __beginTransaction = False):
        ...
        if self.__session is None:
            ...
            if self.__da.autoCommit and \
               self.session.transaction is None and \
               __beginTransaction:
                self.__da.beginTransaction()
    ...
    def __checkSessionClose(self):
        ...
        if not self.__session is None and self.__tempSessionFlg:
    ...
    def filterExpr(self, w, s):
        ...
        if not type(w) is saWhere:
            q = self.__session.query(self.modelType).filter(w)
        else:
            q = self.__session.query(self.modelType)
    ...
    def update(self, upsert = False) -> upsertResult:
        ...
        try:
            ...
            # Commit
            if self.__da.autoCommit and not self.session.transaction is None:
