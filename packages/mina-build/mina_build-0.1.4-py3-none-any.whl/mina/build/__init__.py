...
def _get_project_conf() -> Dict[str, Any]:
    cwd = Path.cwd()
...
def prepare_metadata_for_build_wheel(
    ...
    project = _get_project_conf()
    use_mina_structure = project.get("mina-enabled")
    _patched_meta = None
    if use_mina_structure:
        mina_target: Optional[str] = (
            (config_settings or {}).get("--package")
            or os.environ.get("MINA_BUILD_TARGET")
            or project.get("mina-build-target")
        )
