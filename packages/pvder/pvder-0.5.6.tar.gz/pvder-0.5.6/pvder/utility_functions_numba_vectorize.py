...
def clear_numba_cache(cache_folder):
    ...
    print("Clearning {} *.nbi,*.nbc files from Cache".format(len(files)))
    for f in files:
          os.remove(f)
    files = glob.glob(os.path.join(cache_folder,'*.nbi')) + glob.glob(os.path.join(cache_folder,'*.nbc'))
...
@numba.vectorize(cache=cache_flag)
...
@numba.vectorize(cache=cache_flag) #complex128(float32, complex128 ,complex128),
def m_calc(Kp_GCC,u,x):
	"""Duty cycle for a single phase."""
	return Kp_GCC*u + x #PI controller equation  

@numba.vectorize(cache=cache_flag) #float32(complex128,complex128,complex128),
def ia_ref_Vdc_Q_control_calc(xDC,xQ,Vdc,Q_PCC,Vdc_ref,Q_ref,Kp_DC,Kp_Q):
	"""Phase A current reference"""
	return xDC + Kp_DC*(Vdc_ref - Vdc) + 1j*(xQ  - Kp_Q*(Q_ref - Q_PCC)) #PI controller equation
...
@numba.vectorize(cache=cache_flag) #complex128(complex128,float32),
def vt_calc(m,Vdc):
...
def phasor_to_time(upha,uphb,uphc,w,t):
