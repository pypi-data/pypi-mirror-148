...
class Supervisor(Process):
    ...
    def update(self, name=''):
        """ 更新supervisor的进程列表
        """
        self.host.ssh(self.update_command.format(name), sudo=True, warn=False)

    def start(self, name):
        """ 启动进程
        """
        self.host.ssh(self.start_command.format(name), sudo=True, warn=False)

    def restart(self, name):
        """ 重启进程
        """
        self.host.ssh(self.restart_command.format(name), sudo=True, warn=False)

    def stop(self, name):
        """ 停止进程
        """
        self.host.ssh(self.stop_command.format(name), sudo=True, warn=False)
