...
@pytest.mark.skipif(
    'appdynamics_bindeps.zmq.backend.cython' not in sys.modules, reason="Requires cython backend"
)
@pytest.mark.skipif(
...
def test_cython(language_level, request, tmpdir):
    ...
    assert 'appdynamics_bindeps.zmq.tests.cython_ext' not in sys.modules
    ...
    def unimport():
        ...
        sys.modules.pop('appdynamics_bindeps.zmq.tests.cython_ext', None)
