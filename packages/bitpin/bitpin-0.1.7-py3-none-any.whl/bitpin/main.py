...
class Bitpin:
    ...
    def recent_trades(self, market_id: t.Optional[int]):
        """
        Get recent trades.
        ...
        processed_reponse = self._process_response(f_name, response, f_args)

        if market_id is not None:
            filtered_results = [r for r in processed_reponse.get('results') if r['market']['id'] == market_id]

            processed_reponse['results'] = filtered_results
            
        return processed_reponse

    def balances(self, currency_id: t.Optional[int]):
        """
        Get balances.
        ...
        if currency_id is not None:
            filtered_results = [r for r in processed_reponse.get('results') if r['currency']['id'] == currency_id]

            processed_reponse['results'] = filtered_results
            
        return processed_reponse

    def open_orders(self, market_id: t.Optional[int]):
        """
        Get open orders.
