...
from .omni_stat import emission_prob, mixture_tools, trans
from .omni_stat.utils import get_mem_usage
...
def run_omniCLIP(args):
    ...
    if args.gene_anno_file.split(".")[-1] == "db":
        GeneAnnotation = gffutils.FeatureDB(
            args.gene_anno_file, keep_order=True
        )
    ...
    while iter_cond:
        ...
        if CurrIter < max(3, EmissionParameters["max_it"]):
            ...
            iter_cond = (
                ...
                and (
                    abs(CurrLogLikelihood - OldLogLikelihood) > args.tol_lg_lik
                )
            )
    ...
    if not isinstance(
        EmissionParameters["ExpressionParameters"][0], np.ndarray
    ):
        print("Emmision parameters have not been fit yet")
        return

    OutFile = os.path.join(
        EmissionParameters["out_dir"],
    ...
    tools.GeneratePred(
        ...
        fg_state,
        bg_state,
        pv_cutoff=EmissionParameters["pv_cutoff"],
        verbosity=EmissionParameters["verbosity"],
    )
    print("Done")
...
def FitEmissionParameters(
    Sequences, Background, NewPaths, OldEmissionParameters, First, verbosity=1
):
    ...
    CorrPriorMatrix[CorrPriorMatrix == 0] = (
        np.min(CorrPriorMatrix[CorrPriorMatrix > 0]) / 10
    )
...
def add_pseudo_gene(Sequences, Background, NewPaths, PriorMatrix):
    ...
    gen_lens = [
        Sequences[gene]["Coverage"]["0"][()].shape[1] for gene in Sequences
    ]

    random_gen = np.random.choice(
        np.arange(len(gen_lens)),
        1,
    ...
    new_path = np.random.choice(
        zero_states, size=NewPaths[gene_name].shape, replace=True
    )
...
def main():
    ...
    parser_generateDB = subparsers.add_parser(
        "generateDB",
        help="generateDB help",
        description="Preprocessing of a GFF annotation file into an SQL database.",
    )
    parser_generateDB_reqNamed = parser_generateDB.add_argument_group(
        "required arguments"
    )
    parser_generateDB_reqNamed.add_argument(
        "--gff-file",
        dest="gff_file",
    ...
    parser_generateDB_reqNamed.add_argument(
        "--db-file",
        dest="db_file",
        help="Path to the output .GFF.DB file",
        required=True,
    )
    # Optional args for the generateDB command
    parser_generateDB.add_argument(
        "--gene_features",
        nargs="+",
        dest="gene_features",
        help="Features in the annotation file to be considered as genes",
        required=False,
    )

    # Shared optional arguments from parsingBG and parsingCLIP command
    parent_parsing = argparse.ArgumentParser(add_help=False)
    parent_parsing.add_argument(
        "--rev_strand",
        action="store",
        dest="rev_strand",
        choices=[0, 1],
        help="Only consider reads on the forward (0) or reverse strand (1) relative to the gene orientation",
        type=int,
        default=None,
    )
    parent_parsing.add_argument(
        ...
        default=False,
        dest="collapsed",
        help="Reads are collapsed",
    )
    parent_parsing.add_argument(
        "--ign_out_rds",
        action="store_true",
        dest="ign_out_rds",
        help="ignore reads where the ends map ouside of the genes",
        default=False,
    )
    parent_parsing.add_argument(
        "--max-mismatch",
        action="store",
        dest="max_mm",
        help="Maximal number of mismatches that is allowed per read (default: 2)",
        type=int,
        default=2,
    )
    parent_parsing.add_argument(
        "--mask_flank_mm",
        action="store",
        dest="mask_flank_variants",
        help="Do not consider mismatches in the N bp at the ends of reads for diagnostic event modelling (default: 3)",
        type=int,
        default=3,
    )

    # Create the parser for the parsingBG command
    parser_parsingBG = subparsers.add_parser(
        "parsingBG",
        help="parsingBG help",
        description="Parsing the background files.",
        parents=[parent_parsing],
    )
    parser_parsingBG_reqNamed = parser_parsingBG.add_argument_group(
        "required arguments"
    )
    parser_parsingBG_reqNamed.add_argument(
        "--bg-files",
        action="append",
        dest="bg_libs",
        help="BAM files for background libraries",
        required=True,
    )
    parser_parsingBG_reqNamed.add_argument(
        "--db-file",
    ...
    parser_parsingBG.add_argument(
        "--bck-var",
        action="store_false",
        default=True,
        dest="only_coverage",
        help="Parse variants for background reads",
    )

    # Create the parser for the parsingCLIP command
    parser_parsingCLIP = subparsers.add_parser(
        "parsingCLIP",
        help="parsingCLIP help",
        description="Parsing the CLIP files.",
        parents=[parent_parsing],
    )
    parser_parsingCLIP_reqNamed = parser_parsingCLIP.add_argument_group(
        "required arguments"
    )
    parser_parsingCLIP_reqNamed.add_argument(
        "--clip-files",
        action="append",
        dest="clip_libs",
        help="BAM files for CLIP libraries",
        required=True,
    )
    parser_parsingCLIP_reqNamed.add_argument(
        "--db-file",
    ...
    parser_parsingCLIP.add_argument(
        "--mask-miRNA",
        action="store_true",
        dest="mask_miRNA",
        help="Mask miRNA positions",
        default=False,
    )
    parser_parsingCLIP.add_argument(
        "--mask-ovrlp",
        action="store_true",
        dest="mask_ovrlp",
        help="Ignore overlapping gene regions for diagnostic event model fitting",
        default=False,
    )
    parser_parsingCLIP.add_argument(
        "--min-coverage",
        action="store",
        dest="min_coverage",
        help="Minimum summed coverage on a whole gene to be considered in the analysis",
        default=100,
    )
    parser_parsingCLIP.add_argument(
        "--min-peak",
        action="store",
        dest="min_peak",
        help="Minimum maximum peak heigth on a gene to be considered in the analysis",
        default=5,
    )

    # Create the parser for the run_omniCLIP command
    parser_run_omniCLIP = subparsers.add_parser(
        "run_omniCLIP",
        help="run_omniCLIP help",
        description="running the main omniCLIP program.",
    )
    parser_run_omniCLIP_reqNamed = parser_run_omniCLIP.add_argument_group(
        "required arguments"
    )
    parser_run_omniCLIP_reqNamed.add_argument(
        "--bg-dat",
        action="store",
        dest="dat_file_bg",
        help="Path to the parsed background .dat file",
        required=True,
    )
    parser_run_omniCLIP_reqNamed.add_argument(
        "--clip-dat",
        action="store",
        dest="dat_file_clip",
        help="Path to the parsed CLIP .dat file",
        required=True,
    )
    parser_run_omniCLIP_reqNamed.add_argument(
        "--out-dir",
    ...
    parser_run_omniCLIP_reqNamed.add_argument(
        ...
        action="store",
        dest="gene_anno_file",
        help="File where gene annotation is stored",
    )
    # Optional args for the run_omniCLIP command
    parser_run_omniCLIP.add_argument(
        "--bg-type",
        action="store",
        dest="bg_type",
        help="Background type",
        choices=["None", "Coverage", "Coverage_bck"],
        default="Coverage_bck",
    )
    parser_run_omniCLIP.add_argument(
        "--max-it",
        action="store",
    ...
    parser_run_omniCLIP.add_argument(
        ...
        action="store",
        dest="tol_lg_lik",
        help="tolerance for lok-likelihood",
        type=float,
        default=10000.0,
    )
    parser_run_omniCLIP.add_argument(
        ...
        action="store",
        dest="nr_mix_comp",
        help="Number of diagnostic events mixture components",
        type=int,
        default=1,
    )
    parser_run_omniCLIP.add_argument(
        ...
        action="store_true",
        dest="ign_diag",
        help="Ignore diagnostic event model for scoring",
        default=False,
    )
    parser_run_omniCLIP.add_argument(
        "--ign-GLM",
        action="store_true",
        dest="ign_GLM",
        help="Ignore GLM model for scoring",
        default=False,
    )
    parser_run_omniCLIP.add_argument(
        "--snp-ratio",
        action="store",
        dest="snps_thresh",
        help="Ratio of reads showing the SNP",
        type=float,
        default=0.2,
    )
    parser_run_omniCLIP.add_argument(
        ...
        action="store",
        dest="snps_min_cov",
        help="Absolute number of reads covering the SNP position",
        type=float,
        default=10,
    )
    parser_run_omniCLIP.add_argument(
        ...
        action="store_true",
        dest="norm_class",
        help="Normalize class weights during glm fit",
        default=False,
    )
    parser_run_omniCLIP.add_argument(
        "--seed",
    ...
    parser_run_omniCLIP.add_argument(
        ...
        action="store",
        dest="diag_event_mod",
        help="Diagnostic event model",
        choices=["DirchMult", "DirchMultK"],
        default="DirchMultK",
    )
    parser_run_omniCLIP.add_argument(
        "--glm_weight",
        action="store",
        dest="glm_weight",
        help="weight of the glm score with respect to the diagnostic event score",
        type=float,
        default=-1.0,
    )
    parser_run_omniCLIP.add_argument(
        "--skip_diag_event_mdl",
        action="store_true",
        dest="skip_diag_event_mdl",
        help="Do not model the diagnostic events",
        default=False,
    )
    parser_run_omniCLIP.add_argument(
        "--pv",
        action="store",
        dest="pv_cutoff",
        help="bonferroni corrected p-value cutoff for peaks in bed-file",
        type=float,
        default=0.05,
    )
    parser_run_omniCLIP.add_argument(
        ...
        action="store_true",
        dest="emp_var",
        help="use the empirical variance if it larger than the expected variance",
        default=False,
    )
    parser_run_omniCLIP.add_argument(
        "--diag-bg",
        action="store_true",
        dest="diag_bg",
        help="estimate diagnostic events for the background states on the background",
        default=False,
    )
    parser_run_omniCLIP.add_argument(
        "--fg_pen",
        action="store",
        dest="fg_pen",
        help="Penalty for fg during scoring",
        type=float,
        default=0.0,
    )
    parser_run_omniCLIP.add_argument(
        ...
        action="store_true",
        dest="filter_snps",
        help="Do not fit diagnostic events at SNP-positions",
        default=False,
    )
    parser_run_omniCLIP.add_argument(
        "--no-subsample",
        ...
        default=True,
        dest="subs",
        help="Disable subsampling for parameter estimations (Warning: Leads to slow estimation)",
    )
    parser_run_omniCLIP.add_argument(
        "--ign_out_rds",
        action="store_true",
    ...
    parser_run_omniCLIP.add_argument(
        ...
        action="store",
        dest="tmp_dir",
        help="Output directory for temporary results",
        default=None,
    )
    parser_run_omniCLIP.add_argument(
        "--verbosity",
        action="store",
        dest="verbosity",
        help="verbosity: 0 (default) gives information of current state of site prediction, 1 gives aditional output on runtime and meomry consupmtiona and 2 shows selected internal variables",
        type=int,
        default=0,
    )
    parser_run_omniCLIP.add_argument(
