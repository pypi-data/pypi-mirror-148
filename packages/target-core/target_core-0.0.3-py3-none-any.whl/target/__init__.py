...
from target.file import config_file, config_compression, set_schema_file, save_jsonl_file
...
async def writelines(lines: TextIO,
                     config: Dict,
                     set_schemas: Callable = set_schema_file,
                     writeline: Callable = save_jsonl_file) -> Tuple[Optional[Any], Dict[Any, Any]]:
    ...
    for raw_line in lines:
        ...
        if message_type == 'SCHEMA':
            ...
            schemas[stream] = add_metadata_columns_to_schema(line) if config.get('add_metadata_columns') else float_to_decimal(line['schema'])
            ...
            set_schemas(stream, config, file_data)
            ...
            record_to_load = add_metadata_values_to_record(line, {}, config['date_time']) \
                if config.get('add_metadata_columns') \
                else remove_metadata_values_from_record(line)
            file_data[stream]['file_data'].append(record_to_load)

            await writeline(message_type, config, file_data[stream])
            ...
            LOGGER.debug('Setting state to {}'.format(line['value']))
            state = line['value']
    ...
    for _, file_info in file_data.items():
        await writeline(None, config, file_info)
