...
from mcli.utils.utils_kube import PodEvent, StopWatching, connect_to_pod, wait_for_job_pods, watch_pod_events
...
class InteractiveEpilog():
    """Callbacks for the interactive epilog
    """
    ...
    try:
        ...
        with MCLIPlatform.use(chosen_platform.platform_information) as mcli_platform:
            ...
            with console.status('Waiting for workload to be scheduled...') as status:
                epilog = InteractiveEpilog(status)
                ...
                if connect:
                    ...
                    if not connected:
                        ...
                        logger.warning(f'Please double-check that [bold]{context.cluster}[/] is your context and '
                                       '[bold]{context.namespace}[/] is your correct namespace.')
