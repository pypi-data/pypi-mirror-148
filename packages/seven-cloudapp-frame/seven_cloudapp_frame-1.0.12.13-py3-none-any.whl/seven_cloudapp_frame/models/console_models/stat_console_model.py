...
@LastEditTime: 2022-04-25 11:51:16
                ...
                if len(stat_queue_list) > 0:
                    for stat_queue in stat_queue_list:
                        try:
                            ...
                            stat_log_model = StatLogModel(sub_table=SevenHelper.get_sub_table(stat_queue.act_id, sub_table_config.get("stat_log_num", 0)), db_transaction=db_transaction)
                            stat_report_model = StatReportModel(sub_table=SevenHelper.get_sub_table(stat_queue.act_id, sub_table_config.get("stat_report_num", 0)), db_transaction=db_transaction)
...
class ClearDataWork(TimingWork):
    ...
    def execute(self):
        ...
        page_index = 0
        while True:
            stat_orm_dict_list,total_count =  stat_orm_model.get_dict_page_list(field="id,repeat_type",page_index=page_index,page_size=50,order_by="id asc")
            if len(stat_orm_dict_list) <=0:
                break
            for stat_orm_dict in stat_orm_dict_list:
                if stat_orm_dict["repeat_type"] != 2:
                    stat_log_model.del_entity("orm_id=%s and create_day<%s",params=[stat_orm_dict["id"],SevenHelper.get_now_day_int(-24*15)])
            page_index += 1
