...
@LastEditTime: 2022-04-27 15:13:48
...
class TaskBaseModel(FrameBaseModel):
    ...
    def get_client_task_list(self,app_id,act_id,module_id,user_id,task_types="",app_key="",app_secret="",is_log=False,daily_repeat_browse=False):
        ...
        task_count_model = TaskCountModel(sub_table=frame_base_model.get_business_sub_table("task_count_tb",{"act_id":act_id,"user_id":user_id}),context=self.context)
        ...
        try:
            ...
            if invoke_result_data.success == True:
                ...
                if task_invoke_result_data.success == True:
                    ...
                    frame_base_model = FrameBaseModel(context=self.context)
                    task_count_model = TaskCountModel(sub_table=frame_base_model.get_business_sub_table("task_count_tb",{"act_id":act_id,"user_id":user_id}), db_transaction=db_transaction, context=self.context)
                    ...
                    if reward_value <= 0 or task_count.now_count <=0:
                        ...
                        if task_type == int(TaskType.invite_new_user.value) or task_type == int(TaskType.invite_join_member.value):
                            ...
                            invite_log_model = InviteLogModel(sub_table=frame_base_model.get_business_sub_table("invite_log_tb",{"act_id":act_id,"user_id":user_id}),db_transaction=db_transaction,context=self.context) if task_type == int(TaskType.invite_new_user.value) else InviteLogModel(sub_table="member" if not invite_sub_table else "member_" + str(invite_sub_table),db_transaction=db_transaction,context=self.context)
                            #获取邀请用户数
                            ...
                            if task_type == int(TaskType.collect_goods.value):
                                collect_log_model = CollectLogModel(sub_table=frame_base_model.get_business_sub_table("collect_log_tb",{"act_id":act_id,"user_id":user_id}),db_transaction=db_transaction,context=self.context)
                                if task_info_dict["complete_type"] == 1:
                                ...
                                browse_log_model = BrowseLogModel(sub_table=frame_base_model.get_business_sub_table("browse_log_tb",{"act_id":act_id,"user_id":user_id}),db_transaction=db_transaction,context=self.context)
                                if task_info_dict["complete_type"] == 1:
    ...
    def process_invite_new_user(self,app_id,act_id,module_id,user_id,login_token,from_user_id,handler_name,check_user_nick=True,check_new_user=True,continue_request_expire=5,close_invite_limit=False):
        ...
        try:
            ...
            if invoke_result_data.success == True:
                ...
                frame_base_model = FrameBaseModel(context=self.context)   
                invite_log_model = InviteLogModel(sub_table=frame_base_model.get_business_sub_table("invite_log_tb",{"act_id":act_id,"user_id":user_id}),context=self.context)
                ...
                if is_invite > 0:
                    ...
                    if task_invoke_result_data.success == True:
                        ...
                        limit_num = int(config_json["limit_num"]) if config_json.__contains__("limit_num") else 1
                        satisfy_num = int(config_json["satisfy_num"]) if config_json.__contains__("satisfy_num") else 1
                        task_count_model = TaskCountModel(context=self.context,sub_table=frame_base_model.get_business_sub_table("task_count_tb",{"act_id":act_id,"user_id":user_id}))
        ...
        try:
            ...
            if invoke_result_data.success == True:
                ...
                if top_base_model.check_is_member("",user_info_dict["user_nick"],access_token,app_key, app_secret) == False:
                    ...
                    from_user_info_dict = user_base_model.get_user_info_dict(app_id,act_id,from_user_id)
                    frame_base_model = FrameBaseModel(context=self.context)   
                    invite_sub_table = frame_base_model.get_business_sub_table("invite_log_member_tb",{"act_id":act_id,"user_id":user_id})
    ...
    def process_collect_goods(self,app_id,act_id,module_id,user_id,login_token,goods_id,handler_name,check_user_nick=True,continue_request_expire=5):
        ...
        try:
            ...
            if invoke_result_data.success == True:
                ...
                request_queue_name = invoke_result_data.data["request_queue_name"]
                frame_base_model = FrameBaseModel(context=self.context)   
                collect_log_model = CollectLogModel(context=self.context,sub_table=frame_base_model.get_business_sub_table("collect_log_tb",{"act_id":act_id,"user_id":user_id}))
    ...
    def process_browse_goods(self,app_id,act_id,module_id,user_id,login_token,goods_id,handler_name,daily_repeat_browse=True,check_user_nick=True,continue_request_expire=5):
        ...
        try:
            ...
            if invoke_result_data.success == True:
                ...
                request_queue_name = invoke_result_data.data["request_queue_name"]
                frame_base_model = FrameBaseModel(context=self.context)   
                browse_log_model = BrowseLogModel(sub_table=frame_base_model.get_business_sub_table("browse_log_tb",{"act_id":act_id,"user_id":user_id}),context=self.context)
