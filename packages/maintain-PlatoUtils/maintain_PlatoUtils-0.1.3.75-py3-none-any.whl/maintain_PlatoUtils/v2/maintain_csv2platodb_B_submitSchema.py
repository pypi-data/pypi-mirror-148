...
def buildVertex(vertexJson,graphClient,nodeSysIDUUID=True):
    ...
    for vertexInfoItem in vertexJson:
        ...
        if gNodeAttrDf.shape[0]>0:
            ...
            vertexItemJson={
                        "path": "./tmp/{}".format(vertexFileName),
                        "failDataPath": "./tmp/err/{}Fail.csv".format(vertexFileName.split(".")[0]),
                        "batchSize": 10,
                        "type": "csv",
                        "csv": {
                            "withHeader": False,
...
def buildEdge(edgeJson,graphClient,srcSysIDUUID=True,tgtSysIDUUID=True):
    ...
    for edgeInfoItem in edgeJson:
        ...
        edgeItemJson={
                        "path": "./tmp/{}".format(edgeFileName),
                        "failDataPath": "./tmp/err/{}Fail.csv".format(edgeFileName.split(".")[0]),
                        "batchSize": 10,
                        "type": "csv",
                        "csv": {
                            "withHeader": False,
