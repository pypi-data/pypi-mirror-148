...
def wrapNebula2Df(nebulaObj):
    ...
    start=time.time()
    if nebulaObj.keys() is not None:
        columnList = [colItem for colItem in nebulaObj.keys()]
    else:
        return pd.DataFrame([])
    
    dataDict = {}
    if nebulaObj is not None:
        gc.disable()
        for colItem in columnList:
            colListItem = []
            for rowItem in nebulaObj.column_values(colItem):
                colListItem.append(rowItem)
            dataDict[colItem]=colListItem
        gc.enable()
    else:
        return pd.DataFrame([])
    
    return pd.DataFrame(dataDict, columns=columnList).drop_duplicates()
            ...
            for tgtVertexTypeItem_tail in srcVertexKeynameDict:
                edgeLookupReq=gClient.execute("LOOKUP ON {srcNodeType} WHERE {srcNodeType}.{srcNodeIdAttr}!='不可能的名字'|\
                                                        GO FROM $-.VertexID OVER {edgeType} YIELD   {edgeType}._src AS srcSysID,\
                                                                                                    ...
                                                                                                    $$.{tgtNodeType}.{tgtNodeIdAttr} AS {tgtNodeType}{tgtNodeIdAttr}_tail".format(
