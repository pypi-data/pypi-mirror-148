...
def update_results_chunks(data_paths, new_path, old_rc_data=None):
    """
    ...
    for path in data_paths:
        ...
        stn_id = results_chunk_dict['station_id']
        rc_chunk = orjson.loads(tdm.dataset.ResultChunk(**results_chunk_dict).json(exclude_none=True))
        if stn_id in rc_dict:
            rc_dict[stn_id][chunk_id] = rc_chunk
        else:
            rc_dict.update({stn_id: {chunk_id: rc_chunk}})
    ...
    _ = write_json_zstd(rc_list, rc_file_path)

    return rc_file_path


def update_stations(data_paths, rc_path, new_path, old_stns_data=None):
    """
    The data_paths should contain all of the paths for a single dataset.
    """
    ds_id, version_date_key, _, _, _, _ = os.path.split(data_paths[0])[1].split('_')
    ...
    _ = write_json_zstd(stns_list, stns_file_path)
...
def update_versions(version_data, new_path, old_versions=None):
    ...
    _ = write_json_zstd(version_list, v_file_path)
...
def update_dataset_agg(ds_paths, new_path, old_datasets=None):
    ...
    _ = write_json_zstd(ds_list, dss_path)
