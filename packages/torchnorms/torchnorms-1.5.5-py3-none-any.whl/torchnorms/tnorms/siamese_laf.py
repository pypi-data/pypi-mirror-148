...
class LearnableLatentTNorm(BaseTNorm):
    ...
    def __call__(self, a: Tensor,b: Tensor) -> Tensor:
        ...
        self.params.data = torch.exp(self.params)
        
        latent_1 = self.mlp_1(a)
        latent_2 = self.mlp_1(b)
        
        latent_1 = torch.clamp(self.relu(latent_1), max=1)
        latent_2 = torch.clamp(self.relu(latent_2), max=1)
        ...
        res = self.mlp_2(LAF)
        res = torch.clamp(self.relu(res), max=1)
