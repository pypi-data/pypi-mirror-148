...
from lbgcli.module_impl import Module, append_format_to_parser, TableResult
...
class JobGroupModule(Module):
    ...
    def func_ls(self, args):
        ...
        def mapper_func(each_value):
            ...
            each_value['spend_time_format'] = str(timedelta(seconds=each_value['spend_time']))
            return each_value
    ...
    def func_download(self, args):
        ...
        for each in ids:
            ...
            self.download_jobs(id_in_job_group, group_path, sub_bar)
            sub_bar.close()
            parent_bar.update(1)
    ...
    def download_jobs(self, job_ids, target, sub_bar):
        for each in job_ids:
            ...
            if result.get('result_url'):
                ...
                self.cli.print(f'{result["job_id"]} does not have result skipped.')
            sub_bar.update(1)
