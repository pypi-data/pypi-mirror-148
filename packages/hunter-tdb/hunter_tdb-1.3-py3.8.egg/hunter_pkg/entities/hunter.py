...
class Hunter(base_entity.IntelligentEntity):
    def __init__(self, engine, x: int, y: int):
        ...
        self.hidden = False
        self.attacked = False
        self.max_hunger = stats.get("hunter.max-hunger")
        self.max_health = stats.get("hunter.max-health")
        self.max_energy = stats.get("hunter.max-energy")
        self.curr_hunger = stats.get("hunter.starting-hunger")
        self.curr_health = stats.get("hunter.starting-health")
        self.curr_energy = stats.get("hunter.starting-energy")
        self.vision_distance = stats.get("hunter.vision-distance")
        self.attk_dmg = stats.get("hunter.attack-damage")
        self.bandage_threshold = stats.get("hunter.bandage-threshold")
        self.bandage_heal_amount = stats.get("hunter.bandage-heal-amount")
        self.memory = HunterMemory()
        self.max_recent_actions = stats.get("hunter.max-recent-actions")
        self.min_recent_actions = stats.get("hunter.min-recent-actions")
        self.days_survived = 1
        self.rt_spawn_time = time()
        self.bow = bw.Bow()
        self.bed = None
        self.accuracy = stats.get("hunter.accuracy")
    ...
    def is_hungry(self):
        return self.curr_hunger < stats.get("hunter.hunger-threshold-low")

    def is_still_hungry(self):
        return self.curr_hunger < stats.get("hunter.hunger-threshold-high")

    def is_tired(self):
        return self.curr_energy < stats.get("hunter.tired-threshold") or self.engine.time_of_day == tod.NIGHT
    ...
    def should_wake_up(self):
        if self.attacked:
            ...
            if self.bed == None:
                chance_to_wake_up += stats.get("ground.wake-chance")
            else:
    ...
    def progress(self):
        if self.alive:
            if self.is_affected_by(stfx.Starvation):
                self.curr_health -= stats.get("hunter.starvation-health-loss")
            if self.is_affected_by(stfx.SleepDeprivation):
                self.curr_health -= stats.get("hunter.sleep-deprivation-health-loss")
                chance_to_pass_out = self.rand_health_alert() * stats.get("hunter.sleep-deprivation-chance-to-pass-out")
                roll = rng.rand()
            ...
            self.curr_hunger -= stats.get("hunter.hunger-loss")
            self.curr_energy -= stats.get("hunter.energy-loss")
...
class HunterAI():
    def __init__(self, hunter):
        ...
        self.default_cooldown = stats.get("hunter.action-cooldowns.default")
    ...
    def roam(self):
        dist = stats.get("hunter.roam-distance")
        ...
        while(True):
            ...
            while(len(candidates) < stats.get("hunter.roam-candidates")):
            ...
            for dest in candidates:
                ...
                if len(path) < stats.get("hunter.max-path-distance"):
...
class MovementAction():
    def __init__(self, hunter, dest, path=None, final_action=None):
        ...
        self.cooldown = stats.get("hunter.action-cooldowns.movement-action")
        ...
        for y in range(y_start, y_end+1):
            for x in range(x_start, x_end+1):
                ...
                if curr_visible or prev_visible:
                    ...
                    self.hunter.engine.renderer.remove(layers.FOG, clmp_x, clmp_y)
...
class SearchAreaAction(enta.SearchAreaActionBase):
    def __init__(self, hunter, search_for_classes):
        ...
        self.cooldown = stats.get("hunter.action-cooldowns.search-area-action")
...
class SleepAction():
    ...
    def perform(self):
        if (self.hunter.curr_energy >= (self.hunter.max_energy - stats.get("hunter.energy-loss")) and self.hunter.engine.time_of_day != tod.NIGHT) or self.hunter.should_wake_up():
            self.hunter.wake_up()
            ...
            if self.bed != None:
                ...
                comfort = stats.get("ground.sleep-comfort")

            self.hunter.curr_energy += comfort
            self.hunter.curr_health += stats.get("hunter.sleep-health-gain") if not self.bed == None else stats.get("ground.sleep-health-gain")
