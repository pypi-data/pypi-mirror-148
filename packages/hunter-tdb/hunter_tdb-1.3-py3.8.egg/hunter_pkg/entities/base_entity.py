...
class Entity:
    ...
    # Moves entity by adding `dx` and `dy` to entity's current x and y
    def move(self, dx, dy):
        dest_x = self.x + dx
        dest_y = self.y + dy
        ...
        self.engine.renderer.remove(layers.CREATURES, self.x, self.y)

        self.engine.game_map.tiles[dest_y][dest_x].add_entities([self])
        self.engine.renderer.place(layers.CREATURES, self, self.x, self.y) # Note: `self` is not right here, but it works temporarily

        self.x += dx
        self.y += dy

    # Moves entity to `dest` x and y, which must be adjacent
    ...
    def select(self):
        self.selected = True
        self.color = colors.dark_gray_morning
...
class IntelligentEntity(Entity):
    def __init__(self, engine, x, y, char, color, bg_color, ai, name, article="a"):
        ...
        self.recent_actions = []
        self.max_recent_actions = stats.get("entity.max-recent-actions")
        self.min_recent_actions = stats.get("entity.min-recent-actions")
        self.attacker = None
