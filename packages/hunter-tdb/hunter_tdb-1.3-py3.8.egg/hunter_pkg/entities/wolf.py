...
class Wolf(base_entity.IntelligentEntity):
    def __init__(self, engine, x: int, y: int):
        ...
        self.alive = True
        self.asleep = False
        self.max_hunger = stats.get("wolf.max-hunger")
        self.max_health = stats.get("wolf.max-health")
        self.max_energy = stats.get("wolf.max-energy")
        self.curr_hunger = stats.get("wolf.starting-hunger")
        self.curr_health = stats.get("wolf.starting-health")
        self.curr_energy = stats.get("wolf.starting-energy")
        self.curr_health = stats.get("wolf.starting-health")
        self.critical_health_threshold = stats.get("wolf.critical-health-threshold")
        self.hunger_loss = stats.get("wolf.hunger-loss")
        self.energy_loss = stats.get("wolf.energy-loss")
        self.vision_distance = stats.get("wolf.vision-distance")
        self.attk_dmg = stats.get("wolf.attack-damage")
        self.hunger_threshold = stats.get("wolf.hunger-threshold")
        self.tired_threshold = stats.get("wolf.tired-threshold")
        self.need_a_snack_chance = stats.get("wolf.need-a-snack-chance")
        self.attacker = None
        self.recent_actions = []
        self.hidden = False
    ...
    def is_hungry(self):
        return self.curr_hunger < stats.get("wolf.hunger-threshold") or rng.rand() < self.need_a_snack_chance

    def is_tired(self):
        return self.curr_energy < stats.get("wolf.tired-threshold")
    ...
    def progress(self):
        if self.alive:
            if self.is_affected_by(stfx.Starvation):
                self.curr_health -= stats.get("wolf.starvation-health-loss")
                if not self.alive:
                    self.recent_actions.append("Wolf died of starvation!")
            if self.is_affected_by(stfx.SleepDeprivation):
                self.curr_health -= stats.get("wolf.sleep-deprivation-health-loss")
                chance_to_pass_out = self.rand_health_alert() * stats.get("wolf.sleep-deprivation-chance-to-pass-out")
                roll = rng.rand()
            ...
            self.curr_hunger -= stats.get("wolf.hunger-loss")
            self.curr_energy -= stats.get("wolf.energy-loss")
...
class WolfAI():
    def __init__(self, wolf):
        ...
        self.default_cooldown = stats.get("wolf.action-cooldowns.default")
    ...
    def decide_what_to_do(self):
        ...
        if self.wolf.is_hungry():
            ...
            if rng.rand() < stats.get("wolf.nap-chance"):
    ...
    def roam(self):
        ...
        dist = stats.get("wolf.roam-distance")
...
class MovementAction():
    def __init__(self, wolf, dest, path=None, final_action=None):
        ...
        self.cooldown = stats.get("wolf.action-cooldowns.movement-action")
...
class SearchAreaAction(enta.SearchAreaActionBase):
    def __init__(self, wolf):
        ...
        self.cooldown = stats.get("wolf.action-cooldowns.search-area-action")
...
class PursueAction():
    def __init__(self, wolf, target):
        self.wolf = wolf
        self.target = target
        self.cooldown = stats.get("wolf.action-cooldowns.pursue-action")
...
class FleeAction():
    def __init__(self, wolf, threat, path=None):
        ...
        self.threat = threat
        self.path = path
        self.cooldown = stats.get("wolf.action-cooldowns.flee-action")
        self.flee_search_attempts = stats.get("wolf.flee-search-attempts")
        self.flee_jitter_multiplier = stats.get("wolf.flee-jitter-multiplier")
...
class SleepAction():
    ...
    def perform(self):
        self.wolf.curr_energy += stats.get("wolf.sleep-energy-gain")
        self.wolf.curr_health += stats.get("wolf.sleep-health-gain")
        sleep_in = rng.rand() < stats.get("wolf.sleep-in-chance")
        ...
        elif self.wolf.curr_energy < (self.wolf.max_energy - stats.get("wolf.energy-loss")):
