...
class TestImportCallable(UnitCase):
    def test(self) -> None:
        actual = typeutil.import_callable('tests.custom_classes.Class', location='')
    ...
    def test__no_existing(self) -> None:
        ...
        self.assertRaises(ObjectNotFoundError, typeutil.import_callable, name, location='')

    def test__no_callable(self) -> None:
        name = 'tests.custom_classes.CONSTANT'

        self.assertRaises(ObjectNotCallableError, typeutil.import_callable, name, location='')
...
class TestConvertToPrimitiveTypes(UnitCase):
    def test(self) -> None:
        ...
        actual = typeutil.convert_to_primitive_types(data, missing_value='???')
