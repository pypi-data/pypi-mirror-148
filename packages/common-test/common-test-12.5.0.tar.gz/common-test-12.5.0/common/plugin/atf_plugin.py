...
class ATFPlugin(object):
    ...
    def sendResult(self, report_html_path: str = TEST_TARGET_REPORT_PATH):
        if get_system_key(Constant.RUN_TYPE) is not None:
            ...
            _result = 'success'
            act_passrate = (int(str(_passed)) / int(str(_total)))*100
            act_passrate_str= f'{act_passrate}%'
            if get_system_key(Constant.PASS_RATE) is not None:
                ...
                if act_passrate>passrate:
                    ...
                    _result = 'fail'
            if get_system_key(Constant.TEST_TYPE) is not None:
                _TestType = get_system_key(Constant.TEST_TYPE)
            if buildurl.lower().find('smo') != -1:
                _TestType = '自动化冒烟测试'
            if get_system_key(Constant.PROJECT_NAME) is not None:
                _projectName = get_system_key(Constant.PROJECT_NAME)
            if _total > 0 and _passed > 0:
                if get_system_key(Constant.SEND_URSER_LIST) is not None:
                    _list = get_system_key(Constant.SEND_URSER_LIST).split(',')
                    if get_system_key(Constant.ISSUE_KEY) is not None:
                        _jira_info = f'JIRA编号：{Constant.JIRA_URL}/browse/{Constant.ISSUE_KEY}'
...
if __name__ == '__main__':
    _passed='30%'
    _total='50'
    passrate = int(str(_passed).replace('%', '').strip())
    print(passrate)
