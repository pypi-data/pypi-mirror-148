...
class DataProcess:
    ...
    @classmethod
    def handle_data_fromat(cls, _template, data=None, _no_content=0):
        if isinstance(_template, str):
            if isinstance(data, list):
                ...
                return req_expr(content= _template, data= data, _no_content = _no_content)
        else:
            if isinstance(_template, list):
                for _index in range(len(_template)):
                    _template[_index] = cls.handle_data_fromat(_template[_index], data, _no_content)
                return _template
            else:
                if isinstance(_template, dict):
                    for key in _template.keys():
                        _template[key] = cls.handle_data_fromat(_template[key], data, _no_content)
                return _template
