...
def 帮助():
        ...
        if res_v != v:
            ...
            if sys:
                ...
                if fupdata:
                    ...
                    tool.缓存(1, config)
                tool.缓存(1, config)
                logs.系统日志(f"Fishconsole》help》强制更新启动")
                exit()
            else:
    ...
    help_mod = logs.输入("如果您使用termux安装的话，请按1，否则按2")
    if help_mod == "1":
        print(
                    ...
                    for a in res:
                        if a == "密码模块":
                            window.弹窗(
                                ...
                                "------------------------\n"
                                "加密示例：\n"
                                "tool.密码(1，原文，密码（默认为字符串类型的空）)\n\n"
                                "解密示例：\n"
                                "tool.密码(2，原文，密码（默认为字符串类型的空）)\n\n"
                                , "password加密模块", "ok")
                        if a == "网易云音乐模块":
                            window.弹窗(
                        ...
                        if a == "缓存模块":
                            window.弹窗(
                                ...
                                "\n-----------------------------\n\n"
                                "语法就是\n\n"
                                "tool.缓存(模式, '字典（请注意，字符串用单引号括起来，千万不要用双引号，否则将过不了加密',缓存文件的名字)\n\n"
