...
def setup_test(
    *,
    module: Callable[[], Tuple[str, 'Module']] = None,
    ...
    if module:
        module_name, _module = module()

        config_module(module_name=module_name, module=_module)
        Config.modules[module_name] = _module

        if mock_callables:
            for func in _module.funcs.values():
