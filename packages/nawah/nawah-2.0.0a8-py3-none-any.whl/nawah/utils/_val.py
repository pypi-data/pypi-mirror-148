...
from nawah.enums import NawahValues, VarType
    ...
    try:
        match var.type:
            ...
            case VarType.SESSION:
                ...
                return extract_val(scope=cast(Any, env['session']), path=var.var)
            ...
            case VarType.DOC:
                ...
                return extract_val(scope=cast(Any, doc), path=var.var)

            case VarType.CONFIG:
                return extract_val(scope=Config.vars, path=var.var)

            case VarType.L10N:
                if locale not in Config.locales:
                    raise InvalidLocaleException(locale=locale)
                return extract_val(scope=Config.l10n, path=var.var)
...
def test_call_arg(
    *, args: MutableMapping[str, Any], arg: str, value: Any = NawahValues.NONE_VALUE
):
    '''Provides shorthand check for 'arg' in 'args'. If 'value' provided, it would be matched'''
    ...
    if value not in (NawahValues.NONE_VALUE, args[arg]):
        return False
