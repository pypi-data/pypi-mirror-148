...
def _set_lims(xs, ys, ax):
    ...
    ax.set_xlim(xlim_low - marginx, xlim_high + marginx)
    ax.set_ylim(ylim_low - marginy, ylim_high + marginy)
...
def _set_new_ticks(xs, ys, ax):
    ...
    yticks = get_ticks(ys)
    assert all(
        min(xs) <= x <= max(xs) for x in xticks
    ), f"x ticks out of range: {xticks}"
    assert all(
        min(ys) <= y <= max(ys) for y in yticks
    ), f"y ticks out of range: {yticks}"
    assert min(xs) in xticks
    assert max(xs) in xticks
    assert min(ys) in yticks
    assert max(ys) in yticks
...
def adjust(xs, ys, ax=None, same_ratio=False, **kwargs):
