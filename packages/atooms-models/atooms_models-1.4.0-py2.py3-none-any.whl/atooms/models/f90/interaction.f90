...
contains
  ...
  subroutine forces(box,pos,ids,rad,for,epot,virial)
    double precision, intent(in)    :: box(:)
    double precision, intent(in)    :: pos(:,:)
    integer,          intent(in)    :: ids(:)
  ...
  subroutine hessian(box,pos,ids,rad,hes)
    double precision, intent(in)    :: box(:)
    double precision, intent(in)    :: pos(:,:)
    integer,          intent(in)    :: ids(:)
    double precision, intent(in)    :: rad(:)
  ...
  subroutine gradw(box,pos,ids,rad,grad_w)
    double precision, intent(in)    :: box(:)
    double precision, intent(in)    :: pos(:,:)
    integer,          intent(in)    :: ids(:)
    double precision, intent(in)    :: rad(:)
    ...
    logical                         :: zero_ij
    call tailor(compute)
    call forces(box,pos,ids,rad,forc,epot,virial)
    grad_w = 0.d0
    hbox = box / 2
    loop_i: do i = 1,size(pos,2)
       isp = ids(i)
