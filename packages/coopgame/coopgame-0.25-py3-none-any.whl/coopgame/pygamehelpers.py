...
def draw_text(text: str, hud: pygame.Surface, font: pygame.font.Font = None, offset_rect: Rectangle = None,
    ...
    # Handle rectangle outside of surface area
    x = min(offset_rect.x, hud.get_width())
    y = min(offset_rect.y, hud.get_height())
    width = min(offset_rect.width, hud.get_width() - x)
    height = min(offset_rect.height, hud.get_height() - y)

    if x >= 0 and y >= 0:
        try:
            display1 = hud.subsurface(x, y, width, height)
            display1.blit(rendered_txt, display1.get_rect())
        except:
            logging.error(f"Subsurface for text at {offset_rect} is outside the bounds of the hud: {hud.get_width()}x{hud.get_height()}")