...
class LearnableVQ(nn.Module):
    ...
    def forward(self,
        ...
        if self.ivf is not None:
            ...
            quantized_doc = self.pq.quantization(self.pq.rotate_vec(residual_doc_vecs)) + dc_emb
            quantized_neg = self.pq.quantization(self.pq.rotate_vec(residual_neg_vecs)) + nc_emb
        ...
        if world_size > 1 and cross_device_sample:
            ...
            query_vecs = self.dist_gather_tensor(query_vecs)
        ...
        dense_score = self.compute_score(query_vecs, rotate_doc_vecs, rotate_neg_vecs, temperature)
