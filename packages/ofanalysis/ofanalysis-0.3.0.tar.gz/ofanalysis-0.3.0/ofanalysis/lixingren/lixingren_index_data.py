...
class LixingrenIndexData:
    ...
    def __init__(self, lxr_token: str, code: str, start_date: str = None, end_date: str = None, force_update_db: bool = False):
        self.__v_init(lxr_token=lxr_token, code=code,
        ...
        if (len(item_list) == 0) or force_update_db:  # 需要从API获取至db
            logger.info('选择强制更新或者mongodb中不存在code：%s数据，需要从lxr全量获取...' %
                        self.code)
            ...
            self.__dict_data_raw = self.__save_index_data_to_db(
                self.__retrieve_index_data())  # 重新获取
            ...
            if today_period > existed_period:  # 差异超过周期，更新数据！
                start_date = pd.to_datetime(
                    existed_latest_date) + datetime.timedelta(days=1)
                logger.info('code：%s数据存在，db中最新日期是%s，增量更新....' %
                            (self.code, existed_latest_date))
                self.__save_index_data_to_db(self.__retrieve_index_data(
                    start_date=start_date.strftime('%Y-%m-%d')))
            else:
                logger.info('code：%s数据存在，db中最新日期是%s，无需更新....' %
                            (self.code, existed_latest_date))
    ...
    def __populate_item_raw_to_object(self):
        ...
        if self.__dict_data_raw is None:  # 从db中获取相应数据
            ...
            self.__dict_data_raw = [x for x in self.__dict_data_raw if (
                self.start_date <= x['date'] <= self.end_date)]
