...
class DatabaseIntrospection(BaseDatabaseIntrospection):
    ...
    def get_table_list(self, cursor):
        ...
        cursor.execute("select name, SUBTYPE$ from SYS.SYSOBJECTS WHERE TYPE$ = 'SCHOBJ' AND SUBTYPE$ IN('UTAB', 'VIEW') AND SCHID=CURRENT_SCHID() AND name not like 'SREF_CON_TAB%';")        
            
        return [TableInfo(self.identifier_converter(row[0]), {'UTAB': 't', 'VIEW': 'v'}.get(row[1]))
    ...
    def get_table_description(self, cursor, table_name):
        ...
        cursor.execute("""
            ...
            where col.id=tab.id and tab.name = UPPER(?) 
            )
            SELECT
                column_name,
    ...
    def get_key_columns(self, cursor, table_name):
        ...
        sql = """
        select REF_COLS.NAME, REFED_TABS.NAME, REFED_COLS.NAME 
    ...
    def get_indexes(self, cursor, table_name):
        ...
        from    
              (select ID from SYS.SYSOBJECTS WHERE NAME = '%s')TAB,
        ...
        indexes = {}
        for row in rows:            
            indexes[self.identifier_converter(row[0])] = {'primary_key': bool(row[1]), 'unique': bool(row[2])}
        return indexes
