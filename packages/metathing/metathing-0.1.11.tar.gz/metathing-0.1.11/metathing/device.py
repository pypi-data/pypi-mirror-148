...
class Device():
    ...
    def __init__(self, srv, model) -> None:
        ...
        if 'id' in model:
            ...
            self.alarmer.connect(self.srv_name, self.dev_id, self.label, self.srv_type)
    ...
    def selector(self, func_name:str, content = None):
        ...
        if (content == None or content == {}):
            ...
            if isinstance(content, str):
                ...
                self.alarmer.alarm(Err.SRV_REQ_INVALID, 1, "f:%{0} - {1}".format(func_name, content))
    ...
    def flow(self, op:str, srv_name='', sub_topic:str=None, pub_topic:str=None, qos=1 ):
        ...
        if (op == "add"):
            ...
            # Sub input
            print("MQTT Service Subscribe: {0}, {1}, {2}, {3}, Added service: {4}".format(self.dev_id, op, sub_topic, pub_topic, srv_name))

            # Save
            flow_models_JSON = self.sql.query(Flow).filter_by(id=self.model['id']).first()
            ...
            for m in models:
                ...
                if len(flows) > 0:
                    for fs in flows:
                        ...
                        for f in flowModels:
                            if f['dev_id'] == m.id:
                                self.srv.devs[f['dev_id']].flow('remove', srv_name=f['srv_name'], sub_topic=f['sub_topic'], pub_topic=f['pub_topic'], qos=f['qos'] )
            self.sql.commit()
    ...
    def getAllFlows(self):
        ...
        return json.dumps(self.sql.query(Flow).all()[0].content)
...
class DevMqttOp(CustomHandler):
    ...
    def get(self, op: str):
        if op == "all":
            self.write(self.dev.getAllFlows())
