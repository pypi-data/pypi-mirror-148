...
import pandas as pd
from src.GeneVecTools.library import reader
from src.GeneVecTools import encoder
...
class VecSS:
    ...
    def readq(
        ...
        R = reader.Reader()
        mp, count, total_len, quality = R.read_fastq(self.__dir)
    ...
    encoder = encoder.Encoder(bits=8, encoding="one-hot-encoding")

    def embed(self, s):
        embeddings = []
        for seq in s:
            embeddings.append(encoder.encode_sequence(seq))
        return embeddings

    def unembed(self, embeddings):
        original = []
        for embedding in embeddings:
            original.append(encoder.decode_sequence(embedding))
        return original
