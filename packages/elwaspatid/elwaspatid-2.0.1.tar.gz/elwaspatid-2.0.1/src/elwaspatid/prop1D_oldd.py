...
#            Force[0,indV] = .5*bar.Z[0]*Vinit # since Z=A*rho*co et F=A* 1/2*rho*co*Vinit
        ...
        self.Strain = 0 # @elements
        self.Stress = self.Force[:,:-1]/Z # left stress, @elements
...
class Barhete(Barhomo):
    ...
    def changeSection(self, iseg=0, l=None, d=None):
        ...
        """
        self.seg[iseg].changeSection(l=l, d=d)
...
class Segment(object):
    ...
    """
    def __init__(self, nel, z, l, dx, dt, xo, left='plain', right='plain'):
        """
...
if __name__ == '__main__':
    ...
    if True:
        ...
        if True:
            L = 1  # [m]
            doubleWave = np.zeros(100)
            doubleWave[:20] = -1
            #doubleWave[50:60] = -2
            bar = Barhete([E, E], [rho, rho], [L, 1.5*L], [d, d], nmin=6)
            testk = WP2(bar, doubleWave, nstep=100, left='free', right='free', Vinit=0,
                        contactLoss=None)
            testk.plot()
