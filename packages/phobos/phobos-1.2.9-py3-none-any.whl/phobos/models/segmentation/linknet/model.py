...
class Linknet(SegmentationModel):
    ...
    def __init__(
        ...
        assert len(input.keys()) == 1, logging.error("This PSPNet implementation only supports single input head")
        assert len(input[list(input.keys())[0]].dates) == 1, logging.error("This PSPNet implementation only supports single date input")
        assert len(output.keys()) == 1, logging.error("This PSPNet implementation only supports single output head")
        super().__init__()
        self.input = input 
        self.output = output
        ...
        self.segmentation_head = SegmentationHead(
            in_channels=32, out_channels=self.num_classes, activation=activation, kernel_size=1
        )
