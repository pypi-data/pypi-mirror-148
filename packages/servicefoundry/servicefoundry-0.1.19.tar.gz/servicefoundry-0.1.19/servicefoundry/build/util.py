...
def set_cluster_in_context(client, cluster):
    client.session.set_cluster(format_cluster(cluster))
    client.session.save_session()


def set_workspace_in_context(client, workspace):
    set_cluster_in_context(client, cluster={"id": workspace["clusterId"]})
...
def manage_file_diff(source_lines, target_lines, entity, callback: OutputCallBack):
    diffs = get_file_diff(source_lines,
                          target_lines,
                          f"Original {entity}",
                          f"Updated {entity}")
