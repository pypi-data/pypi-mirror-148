...
def rep_table(rep):
    ...
    #df = pd.DataFrame(table, index=['Instance ID', 'Zone'])
...
def aws(nick:str,   
    ...
    #typer.run(aws_main)
    rep=aws_main(company,nick,config_file,result_dir,cost)
    rep_table(rep)
...
def do(nick:str):
    ...
    nick_ok(company, nick)
    typer.echo(f'Go to {company.upper()} data:')
    rep= do_main(company, nick,config_file,result_dir)
    rep_table(rep)
