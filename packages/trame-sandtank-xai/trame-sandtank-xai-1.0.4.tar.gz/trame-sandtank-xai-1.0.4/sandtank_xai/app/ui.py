...
from trame.html import vuetify, Div, Element
from sandtank_xai import html as xai

# Create single page layout type
TITLE = "SandTank ML - XAI"
layout = SinglePage(
    TITLE,
    on_ready=ctrl.on_ready,
)
layout.logo.click = "step = ''"
layout.logo.children = [vuetify.VIcon("mdi-wizard-hat")]
            ...
            with vuetify.VStepperHeader():
                with vuetify.Template(v_for="label, idx in steps"):
                    ...
                    vuetify.VStepperStep(
                        "{{ label }}", editable=True, step=("idx + 1",), key=("idx",)
                    )
...
with layout.content as container:
    with vuetify.VContainer(
        ...
        with vuetify.VRow(classes="ma-3", justify="space-between", align="center"):
            xai.XaiImageOverlay(
                style="height: 225px",
                id="perm",
            ...
            with vuetify.VStepperItems():
                with vuetify.VStepperContent(step="1", classes="py-0"):
                    with vuetify.VRow(
                        ...
                        Div("Convolution 2D", classes="subtitle-1 font-weight-medium")
        ...
        with vuetify.VRow(
            ...
            with xai.XaiImageOverlay(
                ...
                with vuetify.VRow(classes="py-1"):
                    Element(
                        "img",
                        src=("`o-xai-${xaiModifier}-${step}-${idx}` | imageSrc",),
                        style="width: 100%; height: 15px",
                        __properties=["src"],
                    )
                with vuetify.VRow():
                    Div("{{ array.range[0].toFixed(4) }}")
                    vuetify.VSpacer()
                    Div("{{ array.range[1].toFixed(4) }}")

    xai.XaiFullPipeline(
        v_else=True,
        # __properties=["v_else"],
        pipeline=("aiPipeline", []),
        labels=("['Inputs', 'Convolution 2D', 'Relu', 'Max Pool', 'Dense / Output']",),
