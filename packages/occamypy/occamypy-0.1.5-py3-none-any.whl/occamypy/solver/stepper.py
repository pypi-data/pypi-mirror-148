...
class Stepper:
    ...
    def estimate_initial_guess(self, problem, modl, dmodl, logger):
        ...
        try:
            ...
            dres = problem.get_pert_res(modl, dmodl)
...
class CvSrchStep(Stepper):
    ...
    THE PURPOSE OF CVSRCH IS TO FIND op STEP WHICH SATISFIES
    op SUFFICIENT DECREASE CONDITION AND op CURVATURE CONDITION.

    AT EACH STAGE THE SUBROUTINE UPDATES AN INTERVAL OF
    UNCERTAINTY WITH ENDPOINTS STX AND STY. THE INTERVAL OF
    UNCERTAINTY IS INITIALLY CHOSEN SO THAT IT CONTAINS op
    MINIMIZER OF THE MODIFIED FUNCTION

        F(X+STP*S) - F(X) - FTOL*STP*(GRADF(X)'S).

    IF op STEP IS OBTAINED FOR WHICH THE MODIFIED FUNCTION
    HAS op NONPOSITIVE FUNCTION VALUE AND NONNEGATIVE DERIVATIVE,
    THEN THE INTERVAL OF UNCERTAINTY IS CHOSEN SO THAT IT
    CONTAINS op MINIMIZER OF F(X+STP*S).

    THE ALGORITHM IS DESIGNED TO FIND op STEP WHICH SATISFIES
    ...
    IS BOUNDED BELOW, THEN THERE IS ALWAYS op STEP WHICH SATISFIES
