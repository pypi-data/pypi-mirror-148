    ...
    def run(self, problem, verbose=False, restart=False):
        ...
        while True:
            ...
            if prblm_mdl.isDifferent(cg_mdl):
                ...
                if self.logger:
                    self.logger.addToLog(msg)
                # Recomputing m_current = m_new - dmodel
                prblm_mdl.scaleAdd(cg_dmodl, 1.0, -1.0)
                # Finding the projected dmodel = m_new_clipped - m_current
...
class LSQR(Solver):
    """
       ...
       If op is symmetric, LSQR should not be used! Use SymLCGsolver
       If initial domain different than zero the solver will perform the following:
         1. Compute initial residual vector ``r0 = b - op*x0``.
    ...
    def __init__(self, stopper, estimate_cond=False, estimate_var=False, logger=None):
        ...
        :param stopper          : Stopper, object to terminate inversion
        :param estimate_cond    : Boolean, whether the condition number of op is estimated
                                  access self.acond after solver run [False]
        :param estimate_var     : Boolean, whether the diagonal of op'op^-1 is estimated or not;
    ...
    def run(self, problem, verbose=False, restart=False):
        ...
        while True:
            ...
            if prblm_mdl.isDifferent(cg_mdl):
                ...
                if self.logger:
                    self.logger.addToLog(msg)
                # Recomputing m_current = m_new - alpha * dmodel
                prblm_mdl.scaleAdd(cg_dmodl, 1.0, -alpha)
                # Finding the projected dmodel = m_new_clipped - m_current
                cg_dmodl.copy(cg_mdl)
                cg_dmodl.scaleAdd(prblm_mdl, 1.0, -1.0)
                # dmodel is scaled by the inverse of the step length
                ...
                # Setting residual vector to avoid its unnecessary computation (if domain was not clipped)
                # res  = res + alpha * dres =  res + alpha * op * dmodel
