...
class ADMM(Solver):
    ...
    def __init__(self, stopper, logger=None, niter_linear=5, niter_lasso=5, rho=None,
        ...
        self.primal = None  # primal residual vector (r = op x + B z - c)
        self.dual = None  # dual residual vector (s = rho op.H B r)
    ...
    def run(self, problem, verbose=False, inner_verbose=False, restart=False, initial_guess=None):
        ...
        # op is the Vstack of the L1 reg operators, scaled by their respective eps
        ...
        while True:
            ...
            linear_problem = ProblemL2LinearReg(
                ...
                data=problem.csg,
                op=problem.op,
                reg_op=reg_op,  # pyOp.Vstack(problem.regL2_op, op),
                epsilon=1.,  # problem.epsL2 + [self.rho / 2] * op.N,
                prior_model=prior,
            ...
            # 2) update y
            # lasso problem: rho/2 | op x - z + u|_2^2 + gamma | y |_1
...
def main():
    ...
    if EXAMPLE == 'noisy':
        ...
        # CG = LCGsolver(BasicStopper(iiter=30))
        # CG.run(problemLS, verbose=True)
        # if PLOT:
        #     plt.figure(figsize=(5, 4))
        #     plt.plot(x.getNdArray(), 'k', lw=1, label='x')
        #     plt.plot(y.getNdArray(), '.k', label='y=x+N')
        #     plt.plot(problemLS.domain.getNdArray(), 'r', lw=2, label='x_inv')
        #     plt.legend()
        ...
        # LSQR = LSQRsolver(BasicStopper(iiter=30))
        # LSQR.run(problemLSQR, verbose=True)
        # if PLOT:
        #     plt.figure(figsize=(5, 4))
        #     plt.plot(x.getNdArray(), 'k', lw=1, label='x')
        #     plt.plot(y.getNdArray(), '.k', label='y=x+N')
        #     plt.plot(problemLSQR.domain.getNdArray(), 'r', lw=2, label='x_inv')
        #     plt.legend()
        ...
        #     plt.plot(x.getNdArray(), 'k', lw=1, label='x')
        #     plt.plot(y.getNdArray(), '.k', label='y=x+N')
        #     plt.plot(problemLSR.domain.getNdArray(), 'r', lw=2, label='x_inv')
        #     plt.legend()
        ...
        #     plt.plot(x.getNdArray(), 'k', lw=1, label='x')
        #     plt.plot(y.getNdArray(), '.k', label='y=x+N')
        #     plt.plot(problemLSR_1.domain.getNdArray(), 'r', lw=2, label='x_inv')
        #     plt.legend()
        ...
        # FISTA = ISTAsolver(BasicStopper(iiter=300), fast=True)
        # FISTA.run(problemFISTA, verbose=True)
        # if PLOT:
        #     plt.figure(figsize=(5, 4))
        #     plt.plot(x.getNdArray(), 'k', lw=1, label='x')
        #     plt.plot(y.getNdArray(), '.k', label='y=x+N')
        #     plt.plot(problemFISTA.domain.getNdArray(), 'r', lw=2, label='x_inv')
        #     plt.legend()
        ...
        #
        # ADMM = ADMMsolver(BasicStopper(iiter=30), niter_linear=10, niter_lasso=50)
        ...
        #     plt.plot(y.getNdArray(), '.k', label='y=x+N')
        #     plt.plot(derivative.getNdArray(), ':k', lw=1, label='∂x')
        #     plt.plot(problemADMM.domain.getNdArray(), 'r', lw=2, label='x_inv')
        #     plt.plot((TV * problemADMM.domain).getNdArray(), ':r', lw=2, label='∂(x_inv)')
        #     plt.legend()
    ...
    if EXAMPLE == 'gaussian1D':
        ...
        #     plt.show()
        # Blurring = pyNpOperator.ConvNDscipy(domain=x, kernel=pyVec.vectorIC(h))
        ...
        # FISTA = ISTAsolver(BasicStopper(iiter=100), fast=True)
        # FISTA.run(problemFISTA, verbose=True)
        # if PLOT:
        #     plt.figure(figsize=(5, 4))
        #     plt.imshow(problemFISTA.domain.getNdArray(), cmap='bone'), plt.colorbar()
        #     plt.title(r'FISTA, $\lambda$=%.2e, %d iter'
        #               % (problemFISTA.lambda_value, FISTA.stopper.iiter))
        #     plt.show()
        ...
        #                              regsL1=D, epsL1=.1)
        #
        # ADMM = ADMMsolver(BasicStopper(iiter=10), niter_linear=30, niter_lasso=10)
        # ADMM.setDefaults(save_obj=True, save_model=True)
        # ADMM.run(problemADMM, verbose=True, inner_verbose=True)
        # if PLOT:
        #     plt.figure(figsize=(5, 4))
        #     plt.imshow(problemADMM.domain.getNdArray(), cmap='bone'), plt.colorbar()
        #     plt.title(r'ADMM TV, $\varepsilon=%.2e$, %d iter'
        #               % (problemADMM.epsL1[0], ADMM.stopper.iiter))
        #     plt.show()
