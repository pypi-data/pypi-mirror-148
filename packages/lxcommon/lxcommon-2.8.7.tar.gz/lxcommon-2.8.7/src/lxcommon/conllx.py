...
class ConllxToken:
        ...
        if args:
            if len(args) == 1:
                if isinstance(args[0], str):
                    self._init_from_str(args[0], kwargs.pop("empty", "_"))
    ...
    def asstr(self, empty="_"):
        return "\t".join([empty if v is None else str(v) for v in self.astuple()])
    ...
    def _init_from_str(self, arg, empty):
        fields = arg.split("\t")
...
def readsent(f, empty="_"):
    ...
    for line in f:
        ...
        if line.startswith("#"):
            continue
        if line:
            sent.append(ConllxToken(line, empty=empty))
        elif len(sent) > 0:
            break
    return sent
