...
# 装饰器
@contextmanager
def error(错误提示:str='',抛出异常=False,递归=0,ignore=False)->None:
    '''
...
def zsq_try(func):
    ...
    def rt(*args, **keyargs):
        try:
            ...
            pcolor(msg, 'error')
    return rt
def zsq_try_Exception(error: str = None, 递归=0) -> None:
    '''自定义错误装饰器'''
    def rt(func):
...
def ASCII转字符串(列表: list,字符串=True) -> list or str:
    if 字符串:
...
def 创建目录(文件路径: str, 提示类型: int = 1, 编码: str = 'utf-8'):
    ...
    文件路径 = ' '.join(文件路径.split()) #去掉\x00 \x08 \r \t \n 等
    ...
    if not rt.可创建 and 提示类型:
        if 提示类型 == 1:
            ...
            pcolor('已存在 : {}'.format(fname_new), 'ok')
        else:
            pass
    return rt
...
class PYPP():
    ...
    async def 获取COOKIE(self):
        cookie_list=await self.page.cookies()
        lst=[]
        for cookie in cookie_list:
            lst.append('{}={};'.format(cookie.get('name'),cookie.get('value')))
        cookie=';'.join(lst)
        return cookie
    ...
    def 异步函数(self, func, *args, **kwargs):
        return asyncio.get_event_loop().run_until_complete(func(*args, **kwargs))
class FFMPEG():
    def __init__(self, ffmpeg_href="", fpath="",cmd_show=False):
    ...
    def 合并视频音频(self,文件路径,音频文件,视频文件):
        ...
        self.执行(cmd)
    def 合并视频(self,文件路径="out.mp4",文件名:str='file.txt'):
        #ffmpeg -i "concat:input1.mpg|input2.mpg|input3.mpg" -c copy output.mpg
        #视频列表=[os.path.abspath(x) for x in 视频列表]
        #  ffmpeg -f concat -i filelist.txt -c copy output.mkv
        cmd='{} -f concat -safe 0 -i "{}" -y -c copy "{}"'.format(self.ffmpeg_path, 文件名, 文件路径) #覆盖视频
        self.执行(cmd)
...
class 电脑信息():
    def __init__(self):
        self.网卡=psutil.net_if_addrs()
        self.主机名=socket.gethostname()
        self.内网IP=socket.gethostbyname_ex(self.主机名)[-1]
        self.硬盘分区= psutil.disk_partitions()
        self.硬盘空间=[]
        for disk in psutil.disk_partitions():
            try:
                self.硬盘空间.append(psutil.disk_usage(disk.device))
            except Exception as e:
                pass
        self.内存=psutil.virtual_memory()
        self.系统开机时间=datetime.datetime.fromtimestamp(psutil.boot_time ()).strftime("%Y-%m-%d %H: %M: %S")
        net=psutil.net_io_counters()
        self.接收流量='{0:.2f} Mb'.format(self.MB(net.bytes_sent))
        self.发送流量='{0:.2f} Mb'.format(self.MB(net.bytes_recv))
        self.用户=psutil.users()
    def MB(self,kb):
        return kb/1024/1024
    def CPU使用率(self):
        lst=[]
        for cpu in range(psutil.cpu_count()):
            lst.append(f'{psutil.cpu_percent(cpu)}%')
        return lst
class 弹窗():
    @staticmethod
    def 信息框(标题, 文本):
