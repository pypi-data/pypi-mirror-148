...
def parser_input():
    ...
    parser = argparse.ArgumentParser(
        description="Extend Simulation production/equilibration")
    parser.add_argument('-s', action="store", dest="tpr", help='Input tpr',
                        type=str, required=True)
    parser.add_argument('-time', action="store", dest="time",
                        help='Extend simulation time, default=10',
                        type=float, default=10)
    parser.add_argument('-dt', action="store", dest="dt",
                        help='integration time step, default=0.005',
