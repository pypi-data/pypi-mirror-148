...
def parser_input():
    ...
    parser = argparse.ArgumentParser(
        description="Equilibrate in 3 steps a system (coor+top), (i) first "
        "equilibration with heavy atoms position restraints, (ii) second "
        "equilibration with alpha carbon position restraints and (iii) finally"
        " equilibration with weak alpha carbon position restraints")
    parser.add_argument('-f', action="store", dest="f", help='Input PDB file',
...
if __name__ == "__main__":
    ...
    out_str = "EM steps : {}\nEqui HA time : {}ns \nEqui CA time : {}ns \n " \
              "Equi CA_LOW time :{}ns \n"
    print(out_str.format(args.min_steps,
                         args.HA_time,
                         args.CA_time,
                         args.CA_LOW_time,
                         args.prod_time))
    ...
    md_sys.em_2_steps(out_folder=em_dir, no_constr_nsteps=args.min_steps,
    ...
    md_sys.solvate_add_ions(out_folder=solv_dir, name=sys_name,
                            ion_C=args.Conc, create_box_flag=False)
    ...
    md_sys.em_equi_three_step_iter_error(out_folder=equi_dir, name=sys_name,
                                         nsteps_HA=HA_step, nsteps_CA=CA_step,
                                         nsteps_CA_LOW=CA_LOW_step, dt=dt,
                                         dt_HA=dt_HA)
    ...
    md_sys.production(out_folder=prod_dir, name=sys_name,
                      nsteps=prod_steps, dt=dt)
