...
class FinalDfController:
    ...
    def build_content_profile_final_df(df_content, df_country, df_actor, df_tag, df_homepage, df_content_core,
        ...
        if not pd.isna(df_content[CONTENT_ID].loc[0]):
            ...
            content_df, result_not_in_df, package_df = CommonUtils.fetch_prepare_product_package_1(content_df,
                                                                                                   df_content,
        ...
        return content_df, result_not_in_df, merged_with_country, actor_content_df, tags_content_df, homepage_content_df, content_core_content_df, package_df
    ...
    def get_final_df_content_profile(
        ...
        for row, val in df_content.iterrows():
            ...
            try:
                ...
                content_core_content_df, package_df = FinalDfController.build_content_profile_final_df(
                ...
                cls.content_creator_updater_network(result_df, merged_with_country, actor_content_df, tags_content_df,
                                                    homepage_content_df, content_core_content_df, package_df)
    ...
    @staticmethod
    def add_similarity_with_contents_based_on_homepage_id(
            df_content,
            df_homepage,
            connection_uri,
            connection: GraphDbConnection = None
        ...
        for row, val in df_content.iterrows():
            ...
            try:
                content_df = DataFrame()
                result_not_in_df = DataFrame()
                if not pd.isna(new_df[CONTENT_ID].loc[0]):
                    content_df, result_not_in_df, homepage_content_df = CommonUtils.fetch_prepare_homepages_1(
                        new_df, content_df, df_homepage, result_not_in_df)

                result = cls.content_creator_updater_similarity_network(homepage_content_df)
                if result:
                    print("Successfully dumped add similarity_with contents_based on homepage_id {0}".format(val[CONTENT_ID]))
                    final_df_content_profile = pd.concat([final_df_content_profile, ], axis=0)
                else:
                    print("Not able to add content similarity based on homepage_id".format(val[CONTENT_ID]))
                    final_df_content_profile = pd.concat([final_df_content_profile, content_df], axis=0)
    ...
    @staticmethod
    def add_similarity_with_contents_all(
            df_content,
            df_homepage,
            connection_uri,
            connection: GraphDbConnection = None
        ...
        try:
            for row, val in df_content.iterrows():
                ...
                if not pd.isna(new_df[CONTENT_ID].loc[0]):
                    ...
                    result =  cls.content_creator_updater_similarity_all_contents(homepage_content_df)
                    if result:
                        print("Successfully add similarities in  profile for {0}".format(val[CONTENT_ID]))
                        final_df_content_profile = pd.concat([final_df_content_profile, content_df], axis=0)

                    else:
                        print("Not able add similarities  with all content ".format(val[CONTENT_ID]))
                        result_not_correct_df = pd.concat([result_not_correct_df, result_not_in_df], axis=0)
