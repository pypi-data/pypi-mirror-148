...
def clean_run_results(*paths):
    ...
    for path in paths:
        ...
        try:
            ...
            print(f"Cannot delete file '{new_name}'")
            print(e)
            print("-"*50)
            pass
...
class SmAnalysisMixIn:
    def collect_opts_for_tests(self, sm_test_data):
        self.clos = list(sm_test_data["CLOs"])
    ...
    def make_ccs(self, missing_ok=False):
        ...
        proc = sp.Popen(
            ["ccs", bam, ccs_path], stdout=sp.PIPE, stderr=sp.PIPE
        )
...
class TestCaseHappyPathSmAnalysis(SmAnalysisMixIn):
    ...
    def partition_prefix(self):
        ...
        for arg in ("-P", "--partition"):
            if arg in self.clos:
                partition, partitions = self.clos[options.index(arg)+1].split(
                    ":")
    ...
    def check_ipdsummary_program_and_processes(
        ...
        expected_ipdsummary_processes = ipdsummary_instances*ipdsummary_workers
        if ipdsummary_workers > 1:
            expected_ipdsummary_processes += ipdsummary_instances
        if "--only-produce-methylation-report" not in options:
            assert (
                expected_ipdsummary_processes
                == cmd_result[1]["nprocs_ipdsummary"]
            )
    ...
    def make_expected_summary_report(self):
        ...
        overview_dict = {
            ...
            "Program options": " ".join(
                [str(self.bam), str(self.fasta)]+self.clos),
        ...
        bam_dict = {
            ...
            "MD5 checksum (full)": md5(open(self.bam, "rb").read()).hexdigest(),
        ...
        methylations_dict = {
            ...
            "Fully methylated": (
                "{fully_methylated_gatcs} ({fully_methylated_gatcs_wrt_meth} %)"
                ).format(**stats),
            "Fully unmethylated": (
                "{fully_unmethylated_gatcs} ({fully_unmethylated_gatcs_wrt_meth} %)"
                ).format(**stats),
            "Hemi-methylated...": (
                "{hemi_methylated_gatcs} ({hemi_methylated_gatcs_wrt_meth} %)"
                ).format(**stats),
            "...only in '+' strand": (
                "{hemi_plus_methylated_gatcs} ({hemi_plus_methylated_gatcs_wrt_meth} %)"
                ).format(**stats),
            "...only in '-' strand": (
                "{hemi_minus_methylated_gatcs} ({hemi_minus_methylated_gatcs_wrt_meth} %)"
                ).format(**stats),
        }
    ...
    def check_sm_analysis_with_bam_and_expected_results(self, *options):
        ...
        """
        bam = self.bam # this I do for convenience
        ...
        if "-v" in options or "--verbose" in options:
            ...
            verbose = False
        self.make_expected_summary_report()
        ...
        if self.unaligned_input:
            ...
            if self.pi_shifted_aligned_bam:
                assert (
                    "...but a possible pi-shifted aligned version of the input BAM"
                    f" was found: 'pi-shifted.blasr.{bam}'. It will be used."
                ) in output
            else:
                assert (
        ...
        if len(self.clos) == 0:  # want to run without options in this FT
            ...
            clean_run_results(
                *aligned_bams_to_rm, *ccs_files_to_remove,
                self.expected_gff, self.expected_csv, self.expected_meth_report
            )
            self.clos.append("--verbose")
    ...
    def test_different_combinations_of_aligned_files_unexpectedly_present(
        ...
        opts, clos = self.collect_opts_for_tests(sm_test_data)
        if sm_test_data["name"] == "unaligned input with one mol crossing ori":
            aligned_bam = opts[1]
            pi_shifted_aligned_bam = opts[2]

            # 1) ∃ aligned and ∃ pi-shifted
            self.check_sm_analysis_with_bam_and_expected_results(*opts)
            clean_run_results(*opts[4:7])
            self.check_sm_analysis_with_bam_and_expected_results(
                *opts, "--verbose")
            clean_run_results(*opts[4:7])
            ...
            with temporarily_rename_file(pi_shifted_aligned_bam):
                self.check_sm_analysis_with_bam_and_expected_results(
                    *(opts[0:2]+(None,)+opts[3:])
                )
                clean_run_results(pi_shifted_aligned_bam, *opts[4:7])
                self.check_sm_analysis_with_bam_and_expected_results(
                    *(opts[0:2]+(None,)+opts[3:]), "--verbose")
                clean_run_results(*opts[4:7])
            # 3) ∄ aligned and ∃ pi-shifted
            with temporarily_rename_file(aligned_bam):
                self.check_sm_analysis_with_bam_and_expected_results(
                    *(opts[0:1]+(None,)+opts[2:])
                )
                clean_run_results(aligned_bam, *opts[4:7])
                self.check_sm_analysis_with_bam_and_expected_results(
                    *(opts[0:1]+(None,)+opts[2:]), "--verbose")
                clean_run_results(*opts[4:7])
    ...
    def test_ipd_model(
        ...
        # it out:
        opts, clos = self.collect_opts_for_tests(sm_test_data)
        if ("-M" in clos) or ("--ipd-model" in clos):
            self.check_sm_analysis_with_bam_and_expected_results(
                *(opts+clos))
            clean_run_results(*opts[4:7])
            self.check_sm_analysis_with_bam_and_expected_results(
    ...
    def test_run_with_partition(
        ...
        if sm_test_data["name"] == "partition2of3":
            self.check_sm_analysis_with_bam_and_expected_results(*(opts+clos))
            clean_run_results(*opts[4:7])
            self.check_sm_analysis_with_bam_and_expected_results(
                *(opts+clos), "--verbose")
    ...
    def test_run_with_aligned_CCS_bam_file(
        ...
        # recycle it by using the -C command line option:
        if sm_test_data["name"] == "no clos":
            # he copies the aligned CCS file the he wants to use to
            # the current working dir:
            bam = opts[0]
            aligned_ccs_bam_file = Path("blasr.ccs."+bam.name)
            shutil.copy(DATA_DIR/aligned_ccs_bam_file, ".")
            clos = ("-C", f"{aligned_ccs_bam_file}")
            self.check_sm_analysis_with_bam_and_expected_results(*(opts+clos))
            clean_run_results(*opts[4:7])
            self.check_sm_analysis_with_bam_and_expected_results(
                *(opts+clos), "--verbose")
    ...
    def test_run_with_CCS_bam_file_without_aligned_one(
        ...
        if sm_test_data["name"] == "no clos":
            ...
            # the current working dir:
            bam = opts[0]
            ccs_bam_file = Path("ccs."+bam.name)
            shutil.copy(DATA_DIR/ccs_bam_file, ".")
            clos = ("-c", f"{ccs_bam_file}")
            self.check_sm_analysis_with_bam_and_expected_results(*(opts+clos))
            clean_run_results(*opts[4:7])
            self.check_sm_analysis_with_bam_and_expected_results(
                *(opts+clos), "--verbose")
    ...
    def test_can_produce_just_methylation_reports(
        ...
        if sm_test_data["name"] == "no clos":
            files = []
            files.append(list(os.listdir()))
            gff = sm_test_data["gff"]
            gff.rename(gff.name[9:])
            csv = sm_test_data["csv"]
            csv.rename(csv.name[9:])
            files.append(list(os.listdir()))
            opts = (
                opts[0], None, None, opts[3],
                None, # expected_gff -> None since has been renamed
                None # expected_csv -> None since has been renamed
            )+ opts[6:] + (
                "--only-produce-methylation-report",)
            self.check_sm_analysis_with_bam_and_expected_results(*opts)
            clean_run_results(opts[6])
            opts = opts + ("--verbose",)
            self.check_sm_analysis_with_bam_and_expected_results(*opts)
...
class TestCaseErrors:
    ...
    def check_error_msgs_in_ccs(
        ...
        if check_critical:
            assert "critical" in output.lower(), f"Actual output:\n{output}"
        if check_could_not:
            assert f"[ccs] Could not generate '{ccs_bam_file}'" in output
        if check_suspucious:
            assert (
                f"[ccs] Although the file '{ccs_bam_file}' has been generated,"
                " there was an error.") in output
            assert (
                "[ccs] It is advisable to check the generated ccs file, to "
                "see if it is correct."
            ) in output
            assert (
                "[ccs] The following command was used to produce it:"
            ) in output
            assert f"[ccs] 'ccs {bam} {ccs_bam_file}'" in output
        if check_stderr:
            assert f"[ccs] ...the error was: 'libchufa.so not found'" in output
        if check_fake:
            assert (
                "[ccs] ...but the ccs program did not report any error."
            ) in output
        if check_could_not:
            # and the return code of the program is not 0, which is a clear
            # sign that there was a problem:
