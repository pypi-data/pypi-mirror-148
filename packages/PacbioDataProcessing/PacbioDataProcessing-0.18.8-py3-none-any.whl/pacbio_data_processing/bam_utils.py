...
from .constants import INDEX_SUF, DNA_SEQ_COLUMN, ASCII_QUALS_COLUMN
...
def gen_index_single_molecule_bams(
    ...
    """It generates indices using program (the path to pbindex) in this
    way::
    ...
    Note for developers:
    Maybe it should check for errors and report them (since we are using
    an external tool) and do not yield the molecule if an error happens).
    """
    for (mol_id, molecule) in molecules:
        file_name = molecule.src_bam_path
        index_file = Path(str(file_name)+INDEX_SUF)
        if skip_if_present and index_file.is_file():
            # logging.warn(f"Index for '{molecule}' found. Skipping")
            pass
        else:
            subprocess.run((program, file_name))  # , check=True)
            # logging.debug(f"Index for '{molecule}' generated")
