...
def get_levelpassword(levelid):
    try:
        url = "http://c8o.altervista.org/password.php"
        r = f"LevelID={str(levelid)}".encode()
        data = requests.post(url=url, data=r, headers=head).content.decode()
        print(data)
    except:
...
def comment(username, password, levelid, comment, proxy):
    levelid = str(levelid)
    try:
        url = "http://www.boomlings.com/database/getGJUsers20.php"
        r = f"gameVersion=21&binaryVersion=35&gdw=0&str={username}&total=0&page=0&secret=Wmfd2893gb7".encode()
...
def send_message(username, password, ToUsername, title, message, proxy):
    try:
        url = "http://www.boomlings.com/database/getGJUsers20.php"
        r = f"gameVersion=21&binaryVersion=35&gdw=0&str={username}&total=0&page=0&secret=Wmfd2893gb7".encode()
...
def like_level(username, password, levelid, like, proxy):
    levelid = str(levelid)
    try:
        url = "http://www.boomlings.com/database/getGJUsers20.php"
        r = f"gameVersion=21&binaryVersion=35&gdw=0&str={username}&total=0&page=0&secret=Wmfd2893gb7".encode()
