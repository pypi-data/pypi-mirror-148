...
def default_return(results, default=None, columns:dict=None, page:int=None, full_count:int=None):
        ...
        if type(value) == set:
            ...
            for k, v in value.items():
                ...
                if Operators.EQUAL.equals(k) or Operators.ASIGN.equals(k):
                    ...
                    conditions.append(key.notin_(v if isinstance(v, Iterable) else [v]))
                elif Operators.IN.equals(k):
                    conditions.append(key.in_(v if isinstance(v, Iterable) else [v]))
...
class AlphaDatabase(AlphaDatabaseCore):
    ...
    def select_query(
        ...
        if count:
            ...
            return default_return(results, default=default, columns=columns, page=page,full_count=full_count)
        ...
        if unique:
            if type(unique) == str:
                if not first:
                    ...
                    ), default=default, columns=columns, page=page,full_count=full_count)
                else:
                    return default_return((
                        results_json[unique]
                    ), default=default, columns=columns, page=page,full_count=full_count)
        ...
        if dataclass is not None:
            if len(results_json) == 0:
                return default_return([] if not first else default, default=default, columns=columns, page=page,full_count=full_count)
            if not first:
                results_json = [dataclass.auto_map_from_dict(r) for r in results_json]
            else:
                results_json = dataclass.auto_map_from_dict(results_json)
        return default_return(results_json, default=default, columns=columns, page=page,full_count=full_count)
