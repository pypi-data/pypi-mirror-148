...
def get_version(type = "current"):
    if os.path.isfile(os.path.join(os.path.dirname(__file__),"version.py")):
        from wiliot.version import __version__
        version = __version__
    else:
        version = scm_get_version(get_git_root(__file__),git_describe_command = "git describe --dirty --tags --long --match [0-9]*.[0-9]*.[0-9]*")
