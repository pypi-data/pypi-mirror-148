    ...
    def validate(oebuild_data):
        ...
        if oebuild_data.get("sources"):
            ...
            if oebuild_data["sources"].get("repo") and oebuild_data["sources"].get(
                ...
                raise (
                    TuxbakeParsingError(
                        "repo or git_trees may be specified, but not both"
                    )
                )
        ...
        if oebuild_data.get("target"):
            ...
            raise (TuxbakeParsingError("Please specify the target in build definition"))
