...
def calc_contrast_loss(features,assignment_orig,device,args):
    ...
    for ph in range(assignment_orig.shape[1]):
        ...
        # Feature distance to top-k
        feat_dist = torch.matmul(features[top_assignment], features[top_assignment].T).sum(1)
        
        # Select most similar patches
        top_elem = (feat_dist.topk(pos_batch,0)[1])
        ...
        # Patches with highest confidence assignment          
        bottom_val = feat_dist.topk(pos_batch*9,0,largest=False)[1]
        ...
        # print('Positives: '+str((torch.log(positives).mean()*temp).item()*100)[:5]+' feat_pos: '+str(feat_dist[top_elem[0]].item())[:6]+'   -   Negative: '+str((torch.log(negatives).mean()*temp).item()*100)[:5]+' feat_neg: '+str(feat_dist[bottom_val[0]].item())[:6])
