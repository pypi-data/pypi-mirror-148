...
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def get_revision_or_version_by_key(self,key:str)->Union[Revision,Version]:
  revision=self._deserialize(key_serializer={key:"revision"},local_location=self.revision_store_path)[0]
  if revision:
   return revision
  version=self._deserialize(key_serializer={key:"version"},local_location=self.version_store_path)[0]
  if version:
   return version
