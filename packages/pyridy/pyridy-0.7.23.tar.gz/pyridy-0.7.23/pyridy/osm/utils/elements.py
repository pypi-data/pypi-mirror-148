...
class OSMResultNode:
    def __init__(self, lat: float, lon: float, value=None, f=None, proc = None, dir: str = ""):
...
class OSMRelation:
    def __init__(self, relation: overpy.Relation, ways=None, color=None):
        ...
        self.lat_sw = min([float(n.lon) for n in self.nodes])
        self.lat_ne = max([float(n.lon) for n in self.nodes])
