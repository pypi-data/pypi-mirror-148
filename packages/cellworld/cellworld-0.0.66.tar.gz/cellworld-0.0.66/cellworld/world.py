...
class World:
    ...
    def get_centrality_derivative(self, depth: int = 1) -> JsonList:
        centrality = self.get_centrality(depth=depth)
        cell_map = Cell_map(self.configuration.cell_coordinates)
        centrality_derivative = [0.0 for x in self.cells]
        for cell_index, cell in enumerate(self.cells):
            if cell.occluded:
                continue
            for connection in self.configuration.connection_pattern:
                ...
                if opposite_index == -1 or self.cells[opposite_index].occluded:
                    centrality_derivative[cell_index] += abs(centrality[connection_index])
                    continue
                centrality_derivative[cell_index] += abs(centrality[connection_index]-centrality[opposite_index])
        return [d/2 for d in centrality_derivative]
