...
from ..common.constants import HANDLER_MODEL_MANIFEST_FILE, MODEL_VERSION, C_PRETRAINED_FILE_OR_URL
...
def get_pretrained_file_or_url():
    ...
    if not validators.url(file_or_url) and not os.path.isfile(file_or_url):
        failed(
            f'Invalid file path or url in: {file_or_url} key:{C_PRETRAINED_FILE_OR_URL} in {HANDLER_MODEL_MANIFEST_FILE}')
        exit(-1)
    return file_or_url
