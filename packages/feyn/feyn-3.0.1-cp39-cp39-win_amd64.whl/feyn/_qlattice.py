...
class QLattice(QLatticeNotebookMixin if detect_notebook() else object):
    ...
    def sample_models(
        ...
        res = []
        ar0_codes = self._context.get_codes(0, input_names)
        ar1_codes = self._context.get_codes(1, function_names)
        ar2_codes = self._context.get_codes(2, function_names)
        output_code = self._context.lookup_by_fname(output_name, 0)

        query_string = query_string or "_"
        try:
            query_complexity, query_codes = self._context.query_to_codes(output_name, query_string)
