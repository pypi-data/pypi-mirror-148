...
def job_send_clients(job_config: dict) -> None:
    ...
    try:
        ...
        if len(db_clients) <= 0:
            logger.warning(job_config.get('job_name') + ' | Nenhum cliente retornado para integracao no momento')
            return

        clients_inserted = insert_out_clients(db_config, db_clients)
        if not clients_inserted:
            logger.error(job_config.get('job_name') + ' | Nao foi possivel inserir os clientes no banco semaforo')
        ...
        clients = [Cliente(
            ...
            cpf=c.cpf or "",
            cnpj=c.cnpj,
            endereco=Endereco(True, c.address_type, c.address, c.zipcode, c.number, c.complement, c.neighbourhood, c.city, c.state, c.residential_phone or c.mobile_phone, str(), c.reference, 'BR', str()),
            email=c.email,
            codigo_referencia=c.client_erp,
            telefone_residencial=c.residential_phone,
            telefone_celular=c.mobile_phone) for c in db_clients]
        ...
        while limit <= total:
            ...
            if len(results) < 1:
                logger.error(job_config.get('job_name') + f' | Falha ao integrar clientes: {results[0].message}')
            ...
            if len(failed_results) > 0:
                for fr in failed_results:
                    for c in partial_clients:
                        if c.cpf or c.cnpj == fr.identifiers[0]:
                            time.sleep(0.3)
                            logger.error(job_config.get('job_name') + f' | Falha ao integrar cliente cod.erp {c.codigo_referencia}: {fr.message}')
                            api_okvendas.post_log(Log(f'Falha ao integrar cliente cod.erp {c.codigo_referencia}: {fr.message}', datetime.now().isoformat(), c.codigo_referencia, 'CLIENTE'))
        ...
        logger.error(job_config.get('job_name') + f' | Erro durante execucao do job: {str(e)}')
        api_okvendas.post_log(Log(f'Erro nao esperado na integracao de clientes: {str(e)}', datetime.now().isoformat(),'', 'CLIENTE'))
