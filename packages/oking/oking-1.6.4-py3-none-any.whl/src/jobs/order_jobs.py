...
def job_orders(job_config: dict, start_at_pending: bool = False) -> None:
    try:
        ...
        if not current_job.__contains__('b2b'):
            ...
            if start_at_pending:
                process_b2b_order_queue(queue_status.get('pending'), db_config, True)

            process_b2b_order_queue(queue_status.get('paid'), db_config, True)

            process_b2b_order_queue(queue_status.get('canceled'), db_config)
...
def process_b2b_order_queue(status: str, db_config: DatabaseConfig, status_to_insert: bool = False) -> None:
    try:
        logger.info(current_job + f' | Consultando fila de pedidos no status {status}')
        queue = api_okvendas.get_order_queue(
            url=src.client_data.get('url_api') + '/pedido/fila/{0}',
            ...
            try:
                ...
                if order.erp_code is not None and order.erp_code != '':  # Pedido integrado anteriormente
                    ...
                    if check_order_existence(db_config, order.order_id):  # Pedido existente no banco semaforo
                        ...
                        logger.info(current_job + ' | Inserindo novo pedido no banco semaforo')
                        inserted = insert_temp_b2b_order(order, db_config)
                        if inserted:
                            logger.info(current_job + ' | Pedido inserido com sucesso, chamando procedures...')
                            sp_success, client_erp_id, order_erp_id = call_order_procedures(db_config, q_order.order_id)
                            if sp_success:
...
def protocol_order(db_config: DatabaseConfig, order, queue_order: Queue, order_erp_id: str = '', client_erp_id: str = '') -> None:
    ...
    try:
        ...
        if client_erp_id != '':
            logger.info(current_job + f' | Salvando codigo do erp do cliente no banco semaforo {client_erp_id}')
            conn = db.get_conect()
            cursor = conn.cursor()
            ...
            logger.info(current_job + f' | Protocolando cliente via api okvendas com codigo_referencia {client_erp_id}')
...
def insert_temp_b2b_order(order: OrderB2B, db_config: DatabaseConfig) -> bool:
    step = ''
    db = database.Connection(db_config)
    conn = db.get_conect()
    try:
...
def call_update_order_procedure(db_config: DatabaseConfig, order) -> bool:
    success = True
    db = database.Connection(db_config)
    conn = db.get_conect()
    try:
