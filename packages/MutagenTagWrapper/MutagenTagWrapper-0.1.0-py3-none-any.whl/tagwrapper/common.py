...
from .util import verify_seekable
...
class TagWrapper(metaclass=ABCMeta):
    ...
    def save(self,
             filething: str | bytes | PathLike | IO[bytes] = None, **kwargs
             ) -> None:
        if filething is None:
            if self.filething is None:
                self._raw_tag.save(**kwargs)
            else:
    ...
    def getter_hook(self, value: Iterable | None, getter_name: str) -> list[str] | None:
        ...
        if value is not None:
            pending: list[str | bytes] = list(value)
            ret = []
            for i in pending:
