...
class QueryResult:
    ...
    @staticmethod
    async def read_response(graph, response):
        result = QueryResult(graph, response)
        if len(response) == 1:
            result.parse_statistics(response[0])
        else:
            # start by parsing statistics, matches the one we have
            result.parse_statistics(response[-1])  # Last element.
            await result.parse_results(response)
        return result
    ...
    @staticmethod
    def parse_header(raw_result_set):
