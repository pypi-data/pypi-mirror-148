...
def get_parent_run(run: Run) -> Run:
    ...
    # Pipeline runs follow the below types:
    #   Parent (top level Pipeline Run) type: "azureml.PipelineRun"
    #   Parent AutoML Run (child of the above Pipeline Run) type: "azureml.StepRun"
    #   Child AutoML Run type: "azureml.scriptrun"
    # Stop once we are at the Parent AutoML Run (either at the top level, or one below the top for Pipeline runs)
    if run.parent is None or run.type == "azureml.StepRun":
        return run
