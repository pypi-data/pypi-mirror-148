...
@click.pass_context
@coro
async def materialize(ctx, filename, prefix, push_deps, workspace_map, workspace, no_versions, verbose, target_datasource=None):
    ...
    click.echo(FeedbackManager.info_before_push_materialize(name=pipe_name))
    try:
        ...
        try:
            ...
            pipe_name, _ = filename.rsplit('.', 1)
            if click.confirm(FeedbackManager.info_pipe_exists(name=pipe_name)):
                to_run = await folder_push(
                    cl,
                    tag=prefix,
                    filenames=[filename],
        ...
        click.echo(FeedbackManager.info_before_materialize(name=pipe_name))
        ...
        pipe_file_name = f"{pipe['name']}.pipe"
        ...
        if click.confirm(FeedbackManager.info_materialize_push_datasource_override(name=f)):
            try:
        ...
        if click.confirm(FeedbackManager.warning_populate(file=f_pipe)):
            ...
            if populate:
                response = await cl.populate_node(pipe_name, node_name, populate_subset=populate_subset)
