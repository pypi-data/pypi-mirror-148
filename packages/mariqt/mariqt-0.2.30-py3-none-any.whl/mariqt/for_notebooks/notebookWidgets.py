...
class FileParserWidget(ItemWidgetBase):
    ...
    def __init__(self,defaultFields,on_change_fct,requiredFields=[],iFDO=None,ignoreDateTime=True,allowConvert=True,converters:list=[],minFields=1,fixedItemWidthPixels:int=200,allowCustomFields:bool=True):
        """
        ...
        borderColor='#C0C0C0'
        self.auxFile_widget = widgets.GridBox([ widgets.HBox([widgets.Label("File:"),widgets.VBox([self.fileValid])],layout=widgets.Layout(justify_content='space-between')),
...
class FileParserDateTimeWidgetWithMasterTime(FileParserWidget):
    ...
    def __init__(self,defaultFields:dict,on_change_fct,dateTimeParser=[],iFDO=None,allowCustomFields=True,instancesList=None):
        converters = [ e() for e in ConverterBase.getAllConverters(ConverterBase)]
        FileParserWidget.__init__(self,defaultFields,on_change_fct,requiredFields=[],iFDO=None,ignoreDateTime=False,allowConvert=True,converters=converters,allowCustomFields=allowCustomFields)
    ...
    def copy(self):
        ret = FileParserDateTimeWidgetWithMasterTime(self.defaultFields,self.on_change_fct,allowCustomFields=self.allowCustomFields,instancesList=self.instancesList)
        ret.setParams(self.getParams())
