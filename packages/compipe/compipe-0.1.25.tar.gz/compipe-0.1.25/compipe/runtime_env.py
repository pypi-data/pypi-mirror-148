...
class Environment(metaclass=ThreadSafeSingleton):
    ...
    def get_path(self, keys: list):
        """
        Get path from environment
        :param keys: list of keys
        :return:
        """
        matched = self.param.get(keys.pop(0), None)
        for key in keys:
            if not (matched := matched.get(key, None)):
                return None
            elif isinstance(matched, str):
                return os.path.normpath(matched)
            elif isinstance(matched, dict):
                matched = matched.get(key, None)
            else:
                raise ValueError(f'Invalid type of matched value: {type(matched)}')
