    ...
    def format_for_output(self, value):
        ...
        if isinstance(value, pd.DataFrame):
            if self.graph is None and len(value.columns) >= 2:
                format_dict["@ui"] = {"type": "graph", "key_x": value.columns[0], "key_y": value.columns[1]}
...
class Agent:
    ...
    def process_negotiation(self, msg):
        ...
        if msg == "ok" and self.interface.has_func("charge_func"):
            response["charge"] = int(self.interface.func_dict["charge_func"](msg["request"]))
    ...
    def header(self, basic_auth=False):
    ...
    def _automatic_run(cls):
        ...
        for job_func in cls._automatic_built_agents.values():
            agent = job_func(_automatic=True)
