...
class Number_Convert():
    def __init__(self, test_strings):
        ...
        self.number_map = {'零': 0, '一': 1, '二': 2, '两': 2, '三': 3, '四': 4, '五': 5, '六': 6, '七': 7, '八': 8, '九': 9}  # 1-9数字
        self.unit_map = {'十': 10, '百': 100, '千': 1000, '万': 10000, '亿': 100000000}  # 数字单位
        self.NumList = []
        
    def operate(self, num_str):  # 这个和下面呢个operate1都是处理字符串的函数，别调用
        Num = 0
        unit = False
    ...
    def operate1(self, strings): #处理字符串的，分成了三种情况，有“亿”，无“亿”有“万”， 无“亿”无“万”
        ...
        if '亿' in strings:
            ...
            for index0, i in enumerate(strings2):
                ...
                if len(i.split('万')) != 1:
                    for index, j in enumerate(i.split('万')):
                        Num += self.operate(j) * (10000 if index == 0 else 1)
                else:
                    Num += self.operate(i.split('万')[0])
            ...
            for index, j in enumerate(strings.split('万')):
                Num += self.operate(j) * (self.unit_map['万'] if index == 0 else 1)
            return Num
        else:
            return self.operate(strings)

    def num_convert3(self):
            ...
            if (Str.isnumeric() and not Str.isdigit()) or (
                ...
                for index1, Str2 in enumerate(self.test_strings[index0:]):
                    if (not Str2.isnumeric()) and Str2 != '两':  # 如果遍历到不是数字的字符，表示这一串数字遍历完了，那就开始将这一串中文数字转化成阿拉伯数字
                        Num = self.operate1(self.test_strings[index0:index0 + index1])
