...
try:
    from myscaledb._types import what_py_converter, empty_convertor
except ImportError:
    from myscaledb.types import what_py_converter, empty_convertor
...
class Record(Mapping):
    ...
    def decode_again(self, key: str, binary: bytes):
        items = list(self._row)
        order = self._names[key]
        self._row = tuple(items[:order] + [binary] + items[order + 1 :])
...
class RecordsFabric:
    ...
    def newBin(self, row: bytes) -> Record:
        return Record(
            row=row,  # because of delimiter
            names=self.names,
            converters=self.converters,
        )
