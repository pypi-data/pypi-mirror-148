...
try:
    ...
    from myscaledb.types import rows2ch, json2ch, py2ch, list2ch
...
class Client:
    ...
    async def get_objects_async(self, records: List[Record]):
        ...
        for credential_string in boto_clients.keys():
            ...
            async with get_session().create_client(
                    ...
                    aws_session_token=credential["SessionToken"], ) as client:
                tasks = []
                for i in id_list[credential_string]:
                    tasks.append(asyncio.create_task(
                        self.getObject(records[i], get_object_name, client)))
    ...
    async def getObject(self, row: Record, get_object_name: str, client):
        retries = 3
        get = False
        while retries > 0 and not get:
            try:
                ...
                object_url = row[get_object_name][1]
