...
"""
    print(title)    
    log.warning("Interactive mode not supported at this time; see documentation.  Call solution.deploy() instead.")
...
def import_and_merge_configs(current_dir_path: str, base_config_file_path: str, child_config_file_path: str = None):
    ...
    if child_config_file_path is not None:
        log.info(f'client config {child_config_file_path} specified...')
        client_config_json = import_local_json(f'{current_dir_path}/{child_config_file_path}')

        log.info(f'attempting to merge client config: {base_config_file_path} with child config: {child_config_file_path}')
        merged_config = merge_child_config(client_config_json, default_config_json)
        
        log.info(f'merge successful')
