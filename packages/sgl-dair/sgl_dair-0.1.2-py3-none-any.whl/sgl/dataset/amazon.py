...
class Amazon(NodeDataset):
    def __init__(self, name="photo", root="./", split="random"):
        if name not in ['computers', 'photo']:
            raise ValueError("Dataset name not supported!")
        super(Amazon, self).__init__(root + "amazon/", name)
    ...
    def raw_file_paths(self):
        return osp.join(self._raw_dir, f'amazon_electronics_{self.name.lower()}.npz')
