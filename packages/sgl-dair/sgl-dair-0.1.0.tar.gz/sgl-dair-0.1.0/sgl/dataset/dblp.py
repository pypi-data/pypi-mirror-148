...
class Dblp(HeteroNodeDataset):
    ...
    TYPE_OF_NODE_TO_PREDICT = 'author'
    ...
    # Download the zip at https://www.dropbox.com/s/yh4grpeks87ugr2/DBLP_processed.zip?dl=1
    # and put the path here
    def __init__(self, root: str, path_of_zip: str, split: str = "official"):
        self.src_dataset = DBLP(
            root=root, path_of_zip=path_of_zip).data

        super(Dblp, self).__init__(root + "dblp/", "dblp")
    ...
    def __generate_split(self, split):
        if split == "official":
            # since 'paper' is the first item in TYPE_OF_NODE_TO_PREDICT,
            # there is no need to add an offset to the following indices
            train_idx = self.src_dataset[self.TYPE_OF_NODE_TO_PREDICT]['train_mask'].nonzero(
            ).flatten()
            val_idx = self.src_dataset[self.TYPE_OF_NODE_TO_PREDICT]['val_mask'].nonzero(
            ).flatten()
