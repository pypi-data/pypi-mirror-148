...
class SecretsManagerStorage(ExactOnlineConfig, ConfigParser):
    ...
    def save(self):
        current_secret = {s: dict(self.items(s)) for s in self.sections()}
        self.read_dict(current_secret)
        if time() + 10 > self.get_access_expiry():
            self.secretsmanager_client.put_value(secret_value=json.dumps(current_secret))
