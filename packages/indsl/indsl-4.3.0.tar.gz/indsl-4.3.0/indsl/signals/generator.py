...
def const_value(
    ...
    This function generates a horizontal line. The assumptions when generating the straight line
    is that the start date is set as "1970-01-01", the end date is set as "now" and the sampling is "1 minute".
    If the number of data points generated exceeds 1e8 then the start date is moved forward, such that the
    number of data points generated is not greater than 1e8 with "1 minute" sampling resolution.
    ...
    """
    sample_freq = pd.Timedelta(minutes=1)
    slope = 0
    idx = _make_index(start="1970-01-01", end="now", freq=sample_freq)
    if len(idx) > 1e8:
        idx = idx[len(idx) - 100000000 :]
