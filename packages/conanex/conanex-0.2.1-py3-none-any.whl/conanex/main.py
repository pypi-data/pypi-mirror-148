...
def parse_args():
    ...
    install_parser.add_argument('-m', '--manifests', type=str, nargs='+', help='MANIFESTS')
    install_parser.add_argument('-mi', '--manifests-interactive', type=str, nargs='+', help='MANIFESTS_INTERACTIVE')
    install_parser.add_argument('-v', '--verify', type=str, nargs='+', help='VERIFY')
    ...
    install_parser.add_argument('-b', '--build', type=str, nargs='+', help='BUILD')
    install_parser.add_argument('-r', '--remote', type=str, help='REMOTE')
    install_parser.add_argument('-u', '--update', type=str)
    ...
    install_parser.add_argument('-s', '--settings', type=str, help='SETTINGS_HOST')
    install_parser.add_argument('-s:b', '--settings:build', type=str, help='SETTINGS_BUILD')
    install_parser.add_argument('-s:h', '--settings:host', type=str, help='SETTINGS_HOST')
...
def build_create_args(args, tmpdirname, package):
    ...
    if args.manifests:
        new_args.append('-m')
        new_args.extend(args.manifests)
    if args.manifests_interactive:
        new_args.append('-mi')
        new_args.extend(args.manifests_interactive)
    if args.verify:
        new_args.append('-v')
        new_args.extend(args.verify)
    if args.build:
        new_args.append('-b')
        new_args.extend(args.build)
    if args.remote:
        new_args.append('-r')
