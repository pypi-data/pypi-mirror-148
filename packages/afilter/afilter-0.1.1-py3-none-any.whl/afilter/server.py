...
def get_defects(protocol, triton_client, model_name, requests):
    set_async = True
    ...
    try:
        if protocol =='grpc':
            inputs = [grpcclient.InferInput(f'input', [1, 4, 600, 600], "FP32")]
            inputs[0].set_data_from_numpy(inputs_data)
            outputs = [grpcclient.InferRequestedOutput(f'output')]
            if set_async:
                outputs_data = []
                ...
                sent_count = 1
                outputs_data = triton_client.infer(model_name=model_name, request_id=str(sent_count),
            ...
            if set_async:
                outputs_data = [triton_client.async_infer(model_name=model_name,
                                                        inputs=inputs[i],
                                                        outputs=outputs[i]) for i in range(REQ_NUM)]
                results = [output_data.get_result().as_numpy('output') for output_data in outputs_data]
            else:
    ...
    def FilterFunc(self, requests, context):
        ...
        try:
            if self.protocol.lower() == "grpc":
                ...
                self.triton_client = httpclient.InferenceServerClient(url=self.url, concurrency=REQ_NUM)
        except Exception as e:
            print("context creation failed: " + str(e))
            sys.exit()

        defects = get_defects(self.protocol, self.triton_client, 
                              self.model_name, requests.request)
...
def aivs_serve(protocol, url, model_name):
    ...
    afilter_pb2_grpc.add_FilterServicer_to_server(
        FilterServicer(protocol, url, model_name), server)
