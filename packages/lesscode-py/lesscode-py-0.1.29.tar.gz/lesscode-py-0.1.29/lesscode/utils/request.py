...
def eureka_get(path="", data=None, return_type="json", app_name="DATA_SERVICE", pack=False, **kwargs):
    ...
    if tornado.options.options.running_env != "local":
        ...
        res = sync_get(path=path, params=data, base_url=base_url if base_url else tornado.options.options.data_server,
...
def eureka_post(path="", data=None, return_type="json", app_name="DATA_SERVICE", pack=False, **kwargs):
    ...
    if tornado.options.options.running_env != "local":
        ...
        res = sync_post(path=path, data=data, base_url=base_url if base_url else tornado.options.options.data_server,
                        result_type=return_type,
                        pack=pack, **kwargs)
    return res
