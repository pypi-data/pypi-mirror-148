...
- The `coiled` package now supports Python 3.10. Note that 3.10 is still not recommended, because we recommend using `coiled-runtime` (which uses dask 2022.1.0, which does not support Python 3.10).

0.0.78
======

- The CLI command ``coiled env create`` used to create software environments,
  now accepts a ``--account`` option to specify the account to use for the
