...
class Comando:
    ...
    def runCMD_cached(self, command, stdin_str=None, ignore_return_code=False, show_exp_redis=False):
        ...
        if self.redis_client:
            ...
            if show_exp_redis:
                delta_seconds = int(datetime.datetime.now().timestamp()) - int(stored_timestamp)
        ...
        if self.redis_client:
            ...
            self.redis_client.set(f"timestamp:{command}", int(datetime.datetime.now().timestamp()), ex=self.redis_exp_time)
        return out
