...
def generate_psf_filter_catalogues():
    ...
    for filt in conf['filters']:
        ...
        catalog_name = f'{field}_{filt}_psf_withIDs.cat'
...
def combine_vac_features():
    ...
    if not os.path.exists(save_file):
        ...
        valuesN = f"PHOT_ID_dual"

        cmd += f"in1={catalog} ifmt1=fits values1={valuesN} "
        cmd += f"join1=always "
