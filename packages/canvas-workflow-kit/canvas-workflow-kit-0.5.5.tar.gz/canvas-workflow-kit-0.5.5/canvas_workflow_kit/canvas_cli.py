...
from canvas_workflow_kit.protocol import CONTEXT_REPORT
...
def test_fixture(
    ...
    if subdirectories:
        for i, fixture_folder in enumerate(subdirectories):
            if i >= 1:
                click.echo('-' * 80)
            test_fixture_directory(fixture_folder, Class, date, start_date, end_date, report_mode, full)

    else:
        test_fixture_directory(path, Class, date, start_date, end_date, report_mode, full)
