...
class Keycloak:
    @staticmethod
    def instantiate_with_refresh_token(url=None, refresh_token=None):
    ...
    @staticmethod
    def instantiate_with_raw_json(url=None, json_string=None):
        well_known = get_well_known_info(url, MASTER_REALM)
        token = Token(well_known=well_known, raw_json_str=json_string)
        return Keycloak(token=token, url=url)
