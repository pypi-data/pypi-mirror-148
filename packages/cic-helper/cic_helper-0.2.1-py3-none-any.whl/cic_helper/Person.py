...
from cic_helper.constants import (CHAIN_SPEC, CSV_HEADER, RPC_PROVIDER,
                                  CSV_Column)
...
class Person:
    ...
    def get_balance(self, contract_address: str, fee_limit=800000):
        if (contract_address):
            self.contract_address = contract_address
        cmd = ['erc20-balance', '--fee-limit', str(fee_limit), '-p', RPC_PROVIDER,
               '-i', CHAIN_SPEC, '-u', '-e', self.contract_address, self.user_address]
        log.debug(">" + " ".join(cmd))
    ...
    def send(self, contract_address, signer_keyfile, fee_limit=800000):
        self.contract_address = contract_address
        log.info(
            f"  Sending {self.send_amount} to {self.phone_number} ({self.user_address})")
        cmd = ["erc20-transfer", "-p", RPC_PROVIDER, "-i", CHAIN_SPEC, "-u", "-e", contract_address, "--fee-limit",
               str(fee_limit), "-y", signer_keyfile, "-a", self.user_address, str(self.send_amount), "-s", "-w"]
        log.debug("> " + " ".join(cmd))
        try:
    ...
    def to_row(self) -> List[str]:
        return [self.phone_number, self.user_address, self.contract_address, self.current_balance, str(self.send_amount), self.timestamp]
