...
class ViewRPC(object):
    ...
    def visualize_data_button(self):
        ...
        if unit_axes:
            # Create figure with as many axes as there are different measurement units
            fig, axs = plt.subplots(len(unit_axes), 1)
            # Adjust the subplots to leave some space on the right side
        ...
        # Button test
        ax = fig.add_axes([0.85, 0.83, 0.1, 0.05])
        _button = plt.Button(ax, '[Button Test]]', color='#d1d1d1')
        _button.hovercolor = "#adadad"
        _button.on_clicked(self.on_click)
