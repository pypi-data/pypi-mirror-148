...
import string
import shutil
from importlib import import_module  # to import the Connector class dynamically
...
def pytest_addoption(parser):
    ...
    """
    parser.addoption("--file", action="store", default="absolute path to the connector script to run")
    parser.addoption("--env", action="store", default="absolute path to the env that holds the variables used to test")
    parser.addoption(
...
def copy_ini_to_root():
    shutil.copy(os.path.join(os.path.join(os.path.dirname(__file__), "../"), "pytest.txt"),
...
@pytest.fixture(scope="session",
                autouse=True)  # FIXME: make this pull the class from the session, not just the session itself
def connector_class():
    ...
    try:
        ...
        print("Error: cannot locate Connector class in session <session name>")
        exit(1)
