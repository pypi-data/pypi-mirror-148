...
def api_key_components() -> Tuple[str, str]:
    decoded_api_key = base64.b64decode(jetpack.api_key).decode("utf-8")
    version, runtime_address, api_key_secret = decoded_api_key.split("::")
    if version != "v2":
        raise SystemError(f"expected version v2, but got {version}")

    return runtime_address, api_key_secret
...
class Client:
    ...
    def _address(self) -> str:
        def host() -> str:
            if jetpack.api_key != "":
                runtime_address, _ = api_key_components()
