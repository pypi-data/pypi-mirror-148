...
class init_anim:
    ...
    Arguments:
        :param t: :class:`~np.ndarray', contains the time information that corresponds with the six degrees of freedom data 
        :param speed_up: float, the "real time speed up" or the ratio of the duration of the data to the duration of the animation 
        :param scale: float, global physical scale factor of the animation, i.e., .1 will reduce everything to be 1/10th its original size
    ...
    def __init__(self, t, speed_up, scale):
        ...
        self.t = t
        """:class:`~np.ndarray': contains the time information that corresponds with the six degrees of freedom data"""
        ...
        """:class:`~np.ndarray': the frames that Blender will animate and have corresponding data for"""
...
class ref_frame:
    ...
    Arguments:
        :param name: str, the Blender object name of the reference frame
        :param parent: :class:`~bpy.data.object', the Blender object parent of this reference frame 
        :param anim: :class:`~bpsci.core.init_anim`), class object that was used to initialize the animation
    ...
    def __init__(self, name, parent, anim):
        ...
        self.parent = parent
        """:class:`~bpy.data.object': the Blender object parent of this reference frame"""
        
        # Blender portion
        self.ob = bpy.data.objects.new(name, None)
        """:class:`~bpy.data.object': the Blender object that represents this reference frame"""
    ...
    def static_6DOF(self, quat, x, y, z):
        ...
        Arguments:
            :param quat: :class:`~np.ndarray', an numpy array of one quaternion
    ...
    def dynamic_6DOF(self, quat, x_list, y_list, z_list):
        ...
        Arguments:
            :param quat: :class:`~np.ndarray', an numpy array of quaternions over time
            :param x: :class:`~np.ndarray', the x position over time
            :param y: :class:`~np.ndarray', the y position over time
            :param z: :class:`~np.ndarray', the z position over time
...
class dyn_obj:
    ...
    Arguments:
        :param obj: :class:`~bpy.data.object', the Blender object that will be animated
        :param pa: :class:`~np.ndarray', the offset of the principal axes specified by the :param euler_type:
        :param euler_type: str. the Euler angle order (i.e. 'xyz' for 1,2,3 or 'zxz' for 3,1,3)
        :param parent: :class:`~bpy.data.object', the Blender object parent of the original object 
        :param anim: :class:`~bpsci.core.init_anim`, class object that was used to initialize the animation
    ...
    def __init__(self, obj, pa, euler_type, parent, anim):
        ...
        self.ob = obj
        """:class:`~bpy.data.object': the Blender object that will be animated"""
        
        self.euler_pa = pa
        """:class:`~np.ndarray': the offset of the principal axes specified by the :param euler_type:"""
        
        self.quat = R.from_euler(euler_type, pa).as_quat()
        """:class:`~np.ndarray': a numpy array of one quaternion that represents the principal axes offset"""
    ...
    def apply_animation(self, x_list, y_list, z_list, quat_list):
        ...
        Arguments:
            :param x_list: :class:`~np.ndarray', a numpy array of the x position over time
            :param y_list: :class:`~np.ndarray', a numpy array of the y position over time
            :param z_list: :class:`~np.ndarray'. a numpy array of the z position over time
            :param quat_list: :class:`~np.ndarray' or None, a numpy array of the rotational quaternion over time. Can be passed None if rotation is ignored.
            """
    ...
    def apply_streamline(self, staticity, int_x, int_y, int_z, thickness):
        ...
        Arguments:
            :param staticity (str): a string (either 'static' or 'dynamic') that specifies whether the the streamline is animated over time
            :param int_x: :class:`~np.ndarray', a numpy array of the x position over time
            :param int_y: :class:`~np.ndarray', a numpy array of the y position over time
            :param int_z: :class:`~np.ndarray', a numpy array of the z position over time
        """


        name = self.name+'_streamline'
        """str: the Blender object name of the streamline"""
        frames = self.frames
        """:class:`~np.ndarray': the frames that Blender will animate and have corresponding data for"""
...
class dyn_vec:
    ...
    Arguments:
        :param parent: :class:`~bpy.data.object', the Blender object the vector is associated with
        :param name: str, the Blender object name of the vector ('_vector' will be appended to this name)
        :param scale_mag: float, the scaling factor of the vector's magnitude axis (purely aesthetic)
        :param scale_off: float, the scaling factor of the vector's off-axes (purely aesthetic)
        :param offset: tuple[float], offset of vector from the parent object (purely aesthetic)
        :param anim: :class:`~bpsci.core.init_anim`, class object that was used to initialize the animation
    """
    def __init__(self, parent, name, scale_mag, scale_off, offset, anim):
        ...
        """:class:`~bpy.data.object': holds the vector"""
        ...
        self.vec = vec
        """:class:`~bpy.data.object': the actual arrow asset"""
    ...
    def animate(self, x, y, z):
        ...
        Arguments:
        :param x: :class:`~np.ndarray', a numpy array of the x component over time
        :param y: :class:`~np.ndarray',  a numpy array of the y component over time
        :param z: :class:`~np.ndarray', a numpy array of the z component over time
        """
        ...
        """:class:`~bpy.data.object': an empty that provides calculates the directionality of the vector"""
...
class anim_text:
    ...
    Arguments:
        :param name: str, the Blender object name of the text ('_text' will be appended to this name)
        :param parent: :class:`~bpy.data.object', the Blender object parent of this text 
        :param anim: :class:`~bpsci.core.init_anim`, class object that was used to initialize the animation
        :param data: :class:`~np.ndarray', the data to be displayed (same length as the number of animated frames)
        :param label: str, a label to append to the text (if no label is desired, pass '')
        :param fix_place: int, the number of decimal places to fix the text to
        :param if_str: bool, whether or not the data is a numerical or string vector. Currently only numerical vectors are supported.
    """
