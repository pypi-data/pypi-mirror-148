...
To initialize any animation, call the :class:`init_anim` object to setup the global parameters of the simulation.

Then call the :class:`dyn_obj` object to create an :mod:'bpsci' object that is capable of animation; this is the basic building block for any moving object. 
...
class init_anim:
    def __init__(self, t, speed_up, scale):
        ...
        Arguments:
            :param t (nd.array): contains the time information that corresponds with the six degrees of freedom data 
        ...
        self.t = t
        """iterable[float]: contains the time information that corresponds with the six degrees of freedom data"""
        ...
        """iterable[float]: the frames that Blender will animate and have corresponding data for"""
...
class ref_frame:
    def __init__(self, name, parent, anim):
        ...
        Arguments:
            ...
            :param parent (bpy.data.object): the Blender object parent of this reference frame 
    ...
    def static_6DOF(self, quat, x, y, z):
        """
        INTERNAL METHOD Places the reference frame in one specified static location.

        This method will be properly exposed in future updates.
        May be useful for offsetting the reference frame from the parent object (i.e. for center of mass of principal axes offset)


        Arguments:
            :param quat (nd.array): an numpy array of one quaternion
    ...
    def dynamic_6DOF(self, quat, x_list, y_list, z_list):
        """
        INTERNAL METHOD Animates the reference frame over time.
        ...
        Arguments:
            :param quat (nd.array): an numpy array of quaternions over time
            :param x (nd.array): the x position over time
            :param y (nd.array): the y position over time
            :param z (nd.array): the z position over time
...
class dyn_obj:
    def __init__(self, obj, pa, euler_type, parent, anim):
        ...
        Arguments:
            :param obj (bpy.data.object): the Blender object that will be animated
            :param pa (iterable[float]): the offset of the principal axes specified by the :param euler_type:
            :param euler_type (str): the Euler angle order (i.e. 'xyz' for 1,2,3 or 'zxz' for 3,1,3)
            :param parent (bpy.data.object): the Blender object parent of the original object 
            :param anim (:class:`~bpsci.core.init_anim`): class object that was used to initialize the animation
        ...
        self.euler_pa = pa
        """iterable[float]: the offset of the principal axes specified by the :param euler_type:"""
        
        self.quat = R.from_euler(euler_type, pa).as_quat()
        """nd.array: a numpy array of one quaternion that represents the principal axes offset"""
    ...
    def apply_animation(self, x_list, y_list, z_list, quat_list):
        ...
        Arguments:
        :param x_list (nd.array): a numpy array of the x position over time
        :param y_list (nd.array): a numpy array of the y position over time
        :param z_list (nd.array): a numpy array of the z position over time
        :param quat_list (nd.array or None): a numpy array of the rotational quaternion over time. Can be passed None if rotation is ignored.
        """
    ...
    def apply_streamline(self, staticity, int_x, int_y, int_z, thickness):
        ...
        Arguments:
            ...
            :param int_x (nd.array): a numpy array of the x position over time
            :param int_y (nd.array): a numpy array of the y position over time
            :param int_z (nd.array): a numpy array of the z position over time
        """
...
class dyn_vec:
    def __init__(self, parent, name, scale_mag, scale_off, offset, anim):
        ...
        Arguments:
            :param parent (bpy.data.object): the Blender object the vector is associated with
    ...
    def animate(self, x, y, z):
        ...
        Arguments:
        :param x (nd.array): a numpy array of the x component over time
        :param y (nd.array): a numpy array of the y component over time
        :param z (nd.array): a numpy array of the z component over time
        """
        ...
        """bpy.data.object: an empty that provides calculates the directionality of the vector"""
...
class anim_text:
    def __init__(self, name, parent, anim, data, label, fix_place, if_str):
        ...
        :param parent (bpy.data.object): the Blender object parent of this text 
        :param anim (:class:`~bpsci.core.init_anim`): class object that was used to initialize the animation
        :param data (nd.array): the data to be displayed (same length as the number of animated frames)
