...
def exploration(csv_path, name_date_column, name_target_column, dict_columns_aggregate={}, window_start='',
                ...
                media_split=True, show_graph_media=True, show_graph_no_media=True, method_corr='pearson', min_periods_corr=1, plot_for_line=5, show_only_one='',
                constant_ols=True):
    # create csv
    ...
    display(df.corr(method=method_corr, min_periods=min_periods_corr))

    display(Markdown(f"## Show through a scatterplot the relationship between each input variable and {name_target_column}"))
