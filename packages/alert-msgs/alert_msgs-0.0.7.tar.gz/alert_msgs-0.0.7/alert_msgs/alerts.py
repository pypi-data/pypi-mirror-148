...
from .settings import EmailSettings, SlackSettings, logger
...
def send_slack_message(
    ...
    for _ in range(n_attempts):
        ...
        logger.info(f"[{resp.status_code}] {settings.webhook}")
...
def send_alert(
    method: Union["email", "slack"], components: List[AlertComponent], **kwargs
):
    methods = {"email": send_email, "slack": send_slack_message}
    if not (func := methods.get(method)):
        raise ValueError(
            f"Unknown method '{method}'. Valid choices {', '.join(methods.keys())}"
        )
    return func(components=components, **kwargs)
