...
class twoPointCrossover(basicCrossover):
    ...
    def cross_parents(self, parents=None, children=None, **kargs):
        ...
        if random.random() < kargs.get('xov_rate',self.xov_rate):
            ...
            # Decide on split points
            split_pt1, split_pt2 = \
                random.randint(0, len(p1)), random.randint(0, len(p1))

            if split_pt2 < split_pt1:
                split_pt1, split_pt2 = split_pt2, split_pt1


            # Create chromosomes and inherit information from parents

            children[0].inherit(p1c[:split_pt1] + p2c[split_pt1:split_pt2] + \
                                p1c[split_pt2:],parents[0],parents[1])

            children[1].inherit(p2c[:split_pt1] + p1c[split_pt1:split_pt2] + \
                                p2c[split_pt2:], parents[0],parents[1])

            if track_xov:
                children[0].set_attr('xov', True)
                children[1].set_attr('xov', True)
            if track_split_pt:
                children[0].set_attr('split_pt1', split_pt1/len(p1))
                children[1].set_attr('split_pt1', split_pt1/len(p1))
                children[0].set_attr('split_pt2', split_pt2/len(p1))
                children[1].set_attr('split_pt2', split_pt2/len(p1))
            ...
            if track_split_pt:
                children[0].set_attr('split_pt1', None)
                children[1].set_attr('split_pt1', None)
                children[0].set_attr('split_pt2', None)
                children[1].set_attr('split_pt2', None)
    ...
    def cross_batch(self, parents=None, children=None, **kargs):
        ...
        for p1, p2, c1, c2 in zip(parents[::2], parents[1::2], \
            ...
            if rand_chance() < xov_rate:
                ...
                c1.inherit(p1c[:split_pt1] + p2c[split_pt1:split_pt2] + \
                                    p1c[split_pt2:], p1, p2)
                c2.inherit(p2c[:split_pt1] + p1c[split_pt1:split_pt2] + \
                                    p2c[split_pt2:], p1, p2)
                ...
                if track_split_pt:
                    c1.set_attr('split_pt1', None)
                    c2.set_attr('split_pt1', None)
                    c1.set_attr('split_pt2', None)
                    c2.set_attr('split_pt2', None)
