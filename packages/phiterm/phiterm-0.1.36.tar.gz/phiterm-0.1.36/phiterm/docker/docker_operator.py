...
def run_workflows_docker(
    ...
    # run single Workflow with a task_id param
    if workflow_name is not None or task_name is not None:
        workflow_to_run: Optional[Workflow] = None

        # workflow_name != "" means run a workflow
        if workflow_name is not None and workflow_name != "":
            workflow_to_run = workflows.get(workflow_name, None)

        # workflow_name == "" and task_name != "" means run a task
        elif workflow_name == "" and task_name != "":
            if len(workflows) > 1:
                logger.error(
                    "Running a task without workflow_name when file has multiple workflows is not yet supported"
                )
                return False
            wf_name = list(workflows.keys())[0]
            workflow_to_run = workflows[wf_name]
