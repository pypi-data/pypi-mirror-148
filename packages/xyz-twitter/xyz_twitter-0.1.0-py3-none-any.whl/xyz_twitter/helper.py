...
def update_or_create_user(screen_name):
    ...
    d = api.serialize_user(u)
    user, created = User.objects.update_or_create(screen_name=screen_name, defaults=dict(
        tid=d['id'], name=d['name'], url=d['url'], avatar=d['profile_image_url_https'], description=d['description'],
        created_at=d['created_at'], detail=d
    ))
    return user
...
def sync_user_tweets(user):
    ...
    api = Api()
    u = api.get_user(user.tid)
    pages = api.get_tweets(u.id_str)
    for p in pages:
        for a in p:
            d = api.serialize_tweet(a)
            user.tweets.update_or_create(
                tid=a.id, defaults=dict(
                    tid=d['id'], full_text=d['full_text'], created_at=d['created_at']
                )
            )
