...
def get_default_vars():
    return {
        ...
        'old_do_auth' : '#auth_basic       "Restricted Area"',
        'new_do_auth' : 'auth_basic       "Restricted Area"',
        "old_auth_file" : "#auth_basic_user_file  htpasswd/authfile",
        "new_auth_file" : "auth_basic_user_file  htpasswd/"
    }
...
def execute_commands(config_template, domain, ip, cert_name, port, pollport, redirect_domain, auth_file):
    ...
    old_do_auth = default_vars["old_do_auth"]
    new_do_auth = default_vars["new_do_auth"]
    old_auth_file = default_vars["old_auth_file"]
    new_auth_file = default_vars["new_auth_file"]
    ...
    if "odoo" in config_template and pollport:
        ...
        if auth_file != "":
            eq_display_message = "Set auth file to " + auth_file
            eq_set_do_auth_cmd = "sed -i s/" + old_do_auth + "/" + new_do_auth + "/g " + server_path + "/" + domain + ".conf"
            eq_set_auth_file_cmd = "sed -i s/" + old_auth_file + "/" + new_auth_file + auth_file + "/g " + server_path + "/" + domain + ".conf"
            #os.system(eq_set_do_auth_cmd)
            os.system(eq_set_auth_file_cmd)
            print(eq_display_message.rstrip("\n"))
        print(eq_display_message.rstrip("\n"))
        os.system(eq_set_port_cmd)
