...
def execute_commands(config_template, domain, ip, cert_name, port, pollport, redirect_domain, auth_file):
    ...
    if "odoo" in config_template and pollport:
        ...
        if auth_file != "":
            eq_set_do_auth_cmd = "sed -i s/" + old_do_auth + "/" + 'auth_basic       "Restricted Area"' + "/g " + server_path + "/" + domain + ".conf"
            eq_set_auth_file_cmd = "sed -i s/" + old_auth_file + "/" + "auth_basic_user_file  htpasswd/" + auth_file + "/g " + server_path + "/" + domain + ".conf"
