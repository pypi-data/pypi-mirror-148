...
@fixture()
def farmland_problem(domain: Domain) -> Problem:
    return ProblemParser(problem_path=FARMLAND_NUMERIC_PROBLEM, domain=domain).parse_problem()

@fixture()
def farmland_trajectory_parser(domain: Domain, problem: Problem) -> TrajectoryParser:
    return TrajectoryParser(domain, problem)
...
def test_parse_farmland_trajectory(trajectory_parser: TrajectoryParser):
    observation = trajectory_parser.parse_trajectory(FARMLAND_NUMERIC_TRAJECTORY)
    # assert len(observation.components) == 19