...
class Database:
    ...
    def get_basic_hk_stock_info(self):
        ...
        df['market_capital'] = df['share_issued'] * df['price']
        df['market_capital'] = df['market_capital'].map(lambda x:round(x,0))
        df['market_capital'] = df['market_capital'].astype('int64')

        df = df[['code','stock_name','price','PE','EPS','dividend','volume','turnover','share_issued','lot_size','market_capital','ipo_date','year_end_date']]
