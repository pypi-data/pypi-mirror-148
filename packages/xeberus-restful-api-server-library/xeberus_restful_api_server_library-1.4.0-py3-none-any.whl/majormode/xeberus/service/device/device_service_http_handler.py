...
class DeviceServiceHttpRequestHandler(HttpRequestHandler):
    @http_request(r'^/device/handshake$',
                  http_method=HttpMethod.POST,
                  authentication_required=False,
                  signature_required=True)
    def shake_hands(self, request: HttpRequest, device_id):
    ...
    @http_request(r'^/device/(device_id)/activation/(activation_code)$',
                  http_method=HttpMethod.POST,
                  authentication_required=False,
                  signature_required=True)
    def activate_device(self, request, device_id, activation_code):
        ...
        return DeviceService().activate_device(
            request.app_id,
            activation_code,
            device_id,
            serial_number,
    ...
    def report_battery_events(self, request, device_id):
        if not isinstance(request.body, list):
            raise DeviceService.InvalidArgumentException("The message body MUST be a list of battery events")
    ...
    @http_request(r'^/device/(device_id)/keepalive$',
                  http_method=HttpMethod.POST,
                  authentication_required=False,
                  signature_required=True)
    def report_keepalive(self, request, device_id):
        seed = request.get_argument(
            's',
            data_type=HttpRequest.ArgumentDataType.string,
            argument_passing=HttpRequest.ArgumentPassing.query_string,
        ...
        return DeviceService().report_keepalive(
            request.app_id,
            device_id,
            seed,
            account_id=request.session and request.session.account_id,
            ...
            location=location,
            mobile_exchanged_byte_count=mobile_exchanged_byte_count,
    ...
    def report_location_updates(self, request, device_id):
        if not isinstance(request.body, list):
            raise DeviceService.InvalidArgumentException("The message body MUST be a list of location updates")
        ...
        return DeviceService().report_location_updates(
            request.app_id,
            device_id,
            events,
            account_id=request.session and request.session.account_id,
    ...
    @http_request(r'^/device/activation/request$',
                  http_method=HttpMethod.POST,
                  authentication_required=True,
                  signature_required=True)
    def request_activation_code(self, request):
        ...
        return DeviceService().request_activation_code(
            request.app_id,
            request.account_id,
            team_id,
