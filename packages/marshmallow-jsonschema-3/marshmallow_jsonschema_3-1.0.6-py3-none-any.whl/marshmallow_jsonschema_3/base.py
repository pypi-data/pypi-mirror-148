...
class JSONSchema(Schema):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.use_new_enums = kwargs.pop("use_new_enums", False)
        self.use_repr_enums = kwargs.pop("use_repr_enums", False)
        self.use_nullable = kwargs.pop("use_nullable", False)
        ...
        for validator in field.validators:
            if validator.__class__ in FIELD_VALIDATORS:
                schema = FIELD_VALIDATORS[validator.__class__](
                    ...
                    validator,
                    obj,
                    use_new_enums=self.use_new_enums,
                    use_repr_enums=self.use_repr_enums,
                )
            else:
                base_class = getattr(
    ...
    def _from_nested_schema(self, obj, field):
        ...
        schema = {"type": "object", "$ref": f"#/components/schemas/{name}"}
    ...
    def wrap(self, data, **_) -> typing.Dict[str, typing.Any]:
        ...
        if self.sort_keys:
            for schema, definition in self._nested_schema_classes.items():
                properties = dict(sorted(definition["properties"].items()))
                self._nested_schema_classes[schema]["properties"] = properties
