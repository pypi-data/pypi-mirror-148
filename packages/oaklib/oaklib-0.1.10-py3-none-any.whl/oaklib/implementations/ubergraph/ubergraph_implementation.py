...
class UbergraphImplementation(SparqlImplementation, RelationGraphInterface, SearchInterface, OboGraphInterface,
    ...
    def pairwise_similarity(self, subject: CURIE, object: CURIE = None,
        ...
        if predicates:
            ...
            where.append(_sparql_values('sp', pred_uris))
            where.append(_sparql_values('op', pred_uris))
        query = SparqlQuery(select=['?a', '?ic'],
                            distinct=True,
                            where=where)
        bindings = self._query(query.query_str())
        ics = {self.uri_to_curie(row['a']['value']): float(self.uri_to_curie(row['ic']['value'])) for row in bindings}
        max_ic = max(list(ics.values()))
        best_mrcas = [a for a in ics if ics[a] == max_ic]
        mrca = best_mrcas[0]
        sim = TermPairwiseSimilarity(subject_id=subject,
                                     object_id=object,
                                     ancestor_id=mrca)
