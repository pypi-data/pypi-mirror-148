...
class RatsParser:
    ...
    def _create_partitioned_dataframe(self):
        ...
        regex_match_for_validity = re.compile('[^A-F 0-9-]')
        series_stream = []
        ...
        def __operation(df):
            ...
            for field in self.packet_format:
                if field == 'DATA':
                    ...
                    active_edb_flags = f"{packet_dictionary['ACTIVE_EDBS']:0<b}"
                    ...
                    # at this point, I should have parsed the active edbs, and should be able to group the data accordingly
    ...
    def _assign_samples_to_edbs(self):
        ...
        - why not do this per llc rather than per packet?...
        """
    ...
    def _scale_and_rename_rats_inputs(self):
        ...
        try:
            instanceName = self.filename.split('_')[-1].split('.')[0].split('(')[0].replace('-',
                                                                                            '_')
