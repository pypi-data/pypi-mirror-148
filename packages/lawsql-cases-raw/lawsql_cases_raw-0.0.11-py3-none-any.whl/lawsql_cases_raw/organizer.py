...
from .config import MAX_CASE_TITLE, MAX_FALLO, MAX_RAW_PONENTE
...
def is_per_curiam(d: dict) -> Optional[int]:
    if (x := get_raw_ponente_string(d)) and re.compile(r"per\s+curiam", re.I).search(x):
        return 1
    return None
...
def add_content(d: dict) -> Optional[dict]:
    from .utils import decisions_path
