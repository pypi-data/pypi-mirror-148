...
class ProfileVersionFile(VersionFile):
    ...
    def set_version(self, version, **extra):
        ...
        # This tricky logic is for handling nested keys like examples.examples_version
        for key in PROFILE_VERSION_KEYS:
            sub_keys = key.split('.')
            if len(sub_keys) == 1:
                data[key] = version
            else:
                data_field = data[sub_keys[0]]
                for sub_key in sub_keys[1:-1]:
                    data_field = data_field[sub_key]
                data_field[sub_keys[-1]] = version
