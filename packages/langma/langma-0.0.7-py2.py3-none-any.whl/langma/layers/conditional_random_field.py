...
class KConditionalRandomField(tf.keras.layers.Layer):
    """
    K is to mark langma version of CRF
    ...
    def __init__(self, sparse_target=True, **kwargs):
        if LooseVersion(tf.__version__) < '2.2.0':
            raise ImportError(
                "The KConditionalRandomField requires TensorFlow 2.2.x version or higher."
            )
    ...
    def call(self, inputs, mask=None, **kwargs):
        if mask is not None:
            ...
            self.sequence_lengths = K.sum(K.ones_like(inputs[:, :, 0],
                                                      dtype='int32'),
                                          axis=-1)
        viterbi_sequence, _ = tfa.text.crf_decode(inputs, self.transitions,
                                                  self.sequence_lengths)
        output = K.cast(K.one_hot(viterbi_sequence, inputs.shape[-1]),
                        inputs.dtype)
