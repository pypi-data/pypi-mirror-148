    ...
    def inverse_transform(
        ...
        if self.multi_label:
            return self.multi_label_binarizer.inverse_transform(
                labels, threshold=threshold, keep_prob=keep_prob)
        else:
            res = []

            for label in labels:
                argmax_label_id = label.argmax(-1)
                argmax_label_prob = label[argmax_label_id]
                vocab_label = self.idx2vocab[argmax_label_id]
                if keep_prob:
                    res.append({vocab_label: argmax_label_prob})
                else:
                    res.append(vocab_label)
            return res
