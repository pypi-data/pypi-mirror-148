...
class Tft(Config):
    ...
    def fill_dates(self, data1, value=0, method=None):
        ...
        if value!=0:
            data_train=data_train.where(data_train.bfill().notna(), 0)
            data_train.iloc[-(self.forecast_horizon*self.backtests+self.input_window):]=data_train.iloc[-(self.forecast_horizon*self.backtests+self.input_window):].fillna(0)
        scalers=[]
        if self.normalization:
