...
class Well(BaseModel):
    ...
    def well_head(self,to_crs:int=None,c:float=0.3028):
        ...
        cols = ['x','y','z','crs','geometry','lon','lat','td_md','td_tvd']
        _w.index.name='well'
        #_w = _w.round(1)
...
class Wells(BaseModel):
    ...
    def sample_wells(self, n:int, seed:Optional[int] = None):
        if seed is not None:
            np.random.seed(seed)
        return np.random.choice(list(self.wells.keys()), n, replace=False)
    ...
    def from_dataonboarding(
        ...
        if wh_dates_cols is not None:
            for c in wh_dates_cols['cols']:
                wellheads_df[c] = pd.to_datetime(wellheads_df[c],format=wh_dates_cols['format'])
        ...
        if formations is not None:
            ...
            if fm_dates_cols is not None:
                for c in fm_dates_cols['cols']:
                    formations_df[c] = pd.to_datetime(formations_df[c],format=fm_dates_cols['format'])
        ...
        if units is not None:
            ...
            if un_dates_cols is not None:
                for c in un_dates_cols['cols']:
                    units_df[c] = pd.to_datetime(units_df[c],format=un_dates_cols['format'])
        ...
        if perforations is not None:
            ...
            if perf_dates_cols is not None:
                for c in perf_dates_cols['cols']:
                    perforations_df[c] = pd.to_datetime(perforations_df[c],format=perf_dates_cols['format'])
        ...
        if tools is not None:
            ...
            if to_dates_cols is not None:
                for c in to_dates_cols['cols']:
                    tools_df[c] = pd.to_datetime(tools_df[c],format=to_dates_cols['format'])
        ...
        if correlations is not None:
            ...
            if cor_dates_cols is not None:
                for c in cor_dates_cols['cols']:
                    correlations_df[c] = pd.to_datetime(correlations_df[c],format=cor_dates_cols['format'])
