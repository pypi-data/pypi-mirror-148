...
class HistoricalReturnOperator(IndicatorOperator):
    @staticmethod
    def get_column_names():
        return ["Returns-Hist"]
...
# Todo: needs test, I change the logic
class ForwardReturnOperator(IndicatorOperator):
    @staticmethod
    def get_column_names():
        return ["Return"]

    def __init__(self, prefix, period, magnifier_multiplier):
        self.prefix = prefix
        self.period = period
        self.magnifier_multiplier = magnifier_multiplier
    ...
    def apply(self, data: IndicatorData) -> IndicatorData:
        ...
        close_prices = data["Close"]
        return_col = self.get_column_names()[0]

        fwd_col = self.calculate_final_column_name(prefix=self.prefix, col_name=self.get_column_names()[0],
                                                    period=self.period)

        data[fwd_col] = close_prices \
