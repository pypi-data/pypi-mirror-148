...
class RequestFilesAction(flow.Action):
    ...
    def _process_file(self, sequence_name, shot_name, preset_name, file_data, target_folder, rel_path=None):
        synced = self._sync_file(sequence_name, shot_name, preset_name, file_data)

        if synced:
            self._copy_file(file_data['revision_path'], target_folder, rel_path)
