...
'''.format(
    ...
    def test_latency(self, exclude_country):
        ...
        self.fast_proxy = [_[0] for _ in json.loads(open(self.bench_file, "rb").read().decode()).items() if _[1][0] and _[1][1] < 2.6]
        self.gse_proxy = json.loads(open(self.gse_file, "rb").read().decode())
        self.fast_proxy = [_ for _ in self.proxies if "{}:{}".format(_["server"], _["port"]) in self.fast_proxy]
        self.gse_proxy = [_ for _ in self.proxies if "{}:{}".format(_["server"], _["port"]) in self.gse_proxy]
        print("\r", end="", flush=True)
