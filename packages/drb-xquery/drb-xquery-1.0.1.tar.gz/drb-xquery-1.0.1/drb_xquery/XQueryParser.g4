...
boundarySpaceDecl: KW_DECLARE KW_BOUNDARY_SPACE (KW_PRESERVE | KW_STRIP) ;
defaultCollationDecl: KW_DECLARE KW_DEFAULT KW_COLLATION uriLiteral ;
baseURIDecl: KW_DECLARE KW_BASE_URI uriLiteral ;
constructionDecl: KW_DECLARE KW_CONSTRUCTION (KW_STRIP | KW_PRESERVE) ;
orderingModeDecl: KW_DECLARE KW_ORDERING (KW_ORDERED | KW_UNORDERED) ;
emptyOrderDecl: KW_DECLARE KW_DEFAULT KW_ORDER KW_EMPTY (KW_GREATEST | KW_LEAST) ;
...
functionParam: DOLLAR qName typeDeclaration? ;
...
forClause: KW_FOR forBinding ;

forBinding: DOLLAR name=varName typeDeclaration? allowingEmpty? positionalVar?
        KW_IN exprSingle ;
...
tumblingWindowClause: KW_TUMBLING KW_WINDOW DOLLAR name=qName
                          typeDeclaration? KW_IN exprSingle
...
groupingSpec: DOLLAR name=varName
                    (typeDeclaration? COLON_EQ exprSingle)?
...
caseClause: KW_CASE (DOLLAR varName KW_AS)? sequenceUnionType KW_RETURN
            exprSingle ;
...
attributeTest: KW_ATTRIBUTE LPAREN (attributeNameOrWildcard (COMMA typeName)?)? RPAREN ;
