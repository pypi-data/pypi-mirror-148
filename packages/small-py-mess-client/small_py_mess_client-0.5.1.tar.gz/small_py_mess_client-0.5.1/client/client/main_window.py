...
class ClientMainWindow(QMainWindow):
    ...
    def set_disabled_input(self):
        self.ui.label_new_message.setText(
            'To select a recipient, double-click on it in the contact window.')
    ...
    def history_list_update(self):
        ...
        for i in range(start_index, length):
            ...
            if item[1] == 'in':
                mess = QStandardItem(
                    f'Incoming from {item[3].replace(microsecond=0)}:\n {item[2]}')
                ...
                mess = QStandardItem(
                    f'Coming from {item[3].replace(microsecond=0)}:\n {item[2]}')
                mess.setEditable(False)
    ...
    def set_active_user(self):
        try:
            self.current_chat_key = self.transport.key_request(
                self.current_chat)
            logger.debug(f'Loaded open key {self.current_chat}')
        ...
        try:
            ...
            if err.errno:
                self.messages.critical(
                    self, 'Error', 'Server connection lost!')
                self.close()
            self.messages.critical(self, 'Error', 'Timout!')
