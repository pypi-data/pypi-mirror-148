...
class PoserFile(object):
  ...
  def __init__(self, fn, createLinks=False):
    ...
    #  Poser File Version
    self._version = None
    self._floatVersion = 9.0 # Default version is set to Poser 9
...
#    return False if self._versionStr==None else (self._versionStr==cmpVers)
  ...
  def addAttribut(self, po):
    ...
    # Manage other Props : hairProp, controlProp, Magnets
         ...
         pot==PoserToken.E_coneForceFieldProp or \
         pot==PoserToken.E_controlHandleProp :
      pp = po
      pp.setPoserFile(self)
  ...
  def writeFile(self, fn):
    ...
    try:
      ...
      self.write(prn)
      prn.close()
      logging.info(f"File[{fn}]: {os.path.getsize(fn)} bytes")
    except OSError:
      ret = C_ERROR
    return ret
