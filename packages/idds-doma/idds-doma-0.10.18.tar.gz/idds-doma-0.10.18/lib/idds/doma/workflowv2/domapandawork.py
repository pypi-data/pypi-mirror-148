...
class DomaPanDAWork(Work):
    ...
    def poll_panda_jobs_from_messages(self, job_ids):
        job_ids = list(job_ids)
        self.logger.debug("poll_panda_jobs_from_messages, job_ids[:10]: %s" % (str(job_ids[:10])))
    ...
    def poll_panda_jobs(self, job_ids):
        ...
        # job_ids_left, inputname_jobid_map_msg = poll_panda_jobs_from_messages(job_ids)
        ...
        try:
            ...
            if processing:
                ...
                if task_id:
                    ...
                    inputname_jobid_map = self.poll_panda_jobs(unterminated_jobs, processing)
