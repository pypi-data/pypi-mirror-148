...
def save_object(
    ...
    **kwargs,
    ...
    if file_format == '.json':
        ...
        obj.to_file(file_name=local_file_path, driver='GeoJSON', **kwargs)
    elif file_format == '.shp':
        file_formats = ['.cpg', '.dbf', '.prj', '.shp', '.shx']
        obj.to_file(file_name=local_file_path, driver='ESRI Shapefile', **kwargs)
...
def get_csv_as_gpd(
    ...
    bucket: str. S3 bucket name to upload to.
    dtype: dictionary. Dictionary of column types that is passed into pd.read_csv.
    ...
    df = get_csv(bucket=bucket, local_file_path=local_file_path, dtype=dtype)
