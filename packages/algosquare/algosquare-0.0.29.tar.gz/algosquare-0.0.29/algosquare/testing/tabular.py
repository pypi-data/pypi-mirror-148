...
from ..base.tabular import TabularClassifier, TabularRegressor, is_input_metatype
from ..base.types import Dimensionality, Metatype, is_dimensionality, is_classification_metatype, is_delimiter_metatype, get_delimiter
from ..metrics.metrics import get_metric

{Metatype.NUMERICAL, Metatype.CATEGORICAL, Metatype.BINARY,
...
def load_tabular_data(algo, nan_fraction = 0.1):
    ...
    dimensionality = algo.dimensionality()
    if not is_dimensionality(dimensionality, strict = True):
        raise TestError('invalid dimensionality')

    n_cols = 1 if dimensionality == Dimensionality.UNIVARIATE else 3

    if isinstance(algo, TabularClassifier):
        target_metatype = algo.target_metatype()
        categories = ['bad', 'good'] if target_metatype == Metatype.BINARY else ['cat', 'dog', 'mouse', 'bird']
        y = create_classification_target(X.index, n_cols, categories)
        y_metatypes = [target_metatype for x in range(n_cols)]
    elif isinstance(algo, TabularRegressor):
...
def test_tabular(algo, output_dir, timeout, nan_fraction):
    ...
    output['input_metatypes'] = sorted(list(input_metatypes))
    ...
    if isinstance(algo, TabularClassifier):
        for prediction_method in ('predict_proba', 'decision_function'):
            try:
                ...
                if algo.dimensionality() == Dimensionality.UNIVARIATE:
