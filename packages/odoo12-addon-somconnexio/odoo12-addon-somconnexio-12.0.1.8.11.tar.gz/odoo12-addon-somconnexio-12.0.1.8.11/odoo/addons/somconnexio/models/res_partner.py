...
class ResPartner(models.Model):
    ...
    def _compute_lead_in_provisioning(self):
        for record in self:
            lead_ticket_numbers = {
                ll.ticket_number
                for cl in record.opportunity_ids
            ...
            record.has_lead_in_provisioning = bool(
                lead_ticket_numbers - contract_ticket_numbers
            )
    ...
    def active_sponsees_number(self):
        ...
        active_sponsees = self.sponsee_ids.filtered(
            lambda x: x.has_active_contract or x.has_lead_in_provisioning
        )
