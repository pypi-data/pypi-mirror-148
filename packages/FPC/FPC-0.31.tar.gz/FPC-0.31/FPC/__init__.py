...
def now(x=None):#x can be int means convert timestamp to string,or sting means convert string to timestamp
	if not x:return int(datetime.datetime.now().timestamp()) if x is None else datetime.datetime.now().timestamp()
...
def process(target,is_thread=True,join=False,num=1,sleep=None,**args):#args[['',''],]
