...
def restart():
    ...
    :return:
    """
    global start, state, end, on_step, on_end, _prepare, _groups, _so
    start = None
    state = None
    end = None
...
def step(*ex):
    ...
    :param *ex: external factor interference. Before formally exec step, call these exfunc(sm)->None. If any exfunc return a non-None value will immediate stop 'step' in this time, and 'step' will return the value from that exfunc.
    ...
    _state = _state[1](state, _state[2], _so)
