...
class Efsm(EfsmBase):
    ...
    def add(self, *state, fn=None, data=None):
        """
        have the basic add() function in StateMachine.
        Other higher function:
        1. could accept a function with decorated by ss
        .add(update1, update2, ...)
        2. could accept a callable with doc had '@state/@sta/@s'
        .add(update1, update2, ...)

        auto choose the start if start is None and this is the first time calling add.
        auto choose the end if end is None and this is the first time calling add.
    ...
    def step(self, *ex):
        ...
        code = state[1].__code__

        if code.co_argcount >= 3 or len(code.co_varnames) > code.co_argcount:
            state = state[1](self.state, state[2], self._so)
        elif code.co_argcount >= 2:
            state = state[1](self.state, state[2])
        elif code.co_argcount >= 1:
            state = state[1](self.state)
        else:
            state = state[1]()
...
if __name__ == '__main__':
    ...
    fsm.restart()  # clear for example2
    ...
    fsm.restart()  # clear for example3

    class Test:
        def update(self, state, o, s):
            """
            @state idle move stop  # ',' is not a matter
            :param state:
            :param o:
    ...
    fsm.restart()  # clear for example5
    ...
    fsm.restart()  # clear for example6

    def update(state, *args):  # in shell-mode, you caould pass 0-3 params, it will auto choose how to call it.
        """
        @sta idle move stop
        :param state:
        :return:
