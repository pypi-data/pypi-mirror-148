...
class Efsm(EfsmBase):
    ...
    def add(self, *state, fn=None, data=None):
        # print(state, fn, data)
        if len(state) == 1 and fn is None:
            if isinstance(state[0], StateSet):
                state = state[0].tolist()
                self._groups[state[0]] = state[1:]
            elif callable(state[0]):  # type(state[0]) == _function:
                get = Efsm._parse(state[0].__doc__)
                if get:
                    state = super(Efsm, self).add(*get, fn=state[0], data=data)
                else:
                    raise Exception(f"fn:{state[0].__name__} does not have doc for state.")
            else:
                raise Exception("Unexpected get{fn:None, state:{" + str(state[0]) + "}}. Only could recv StateSet/doced-function when fn is None.")
        else:
...
if __name__ == '__main__':
    ...
    class PhyNote(metaclass=StateMeta):  # use StateMeta to auto add PhyNote._efsm = Efsm()
        """
