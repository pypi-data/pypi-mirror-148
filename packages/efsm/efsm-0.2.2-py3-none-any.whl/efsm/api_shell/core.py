...
class EfsmBase(StateMachine):
    ...
    # def _redirect(self, efsm, state):
    #     """
    #     get the target to other efsm
    #     :return:
    #     """
    #     self._try_update()
    #     if state is not None: efsm.state = state  # redirect efsm.state to state
    #     get, to = efsm.links.get(state, (None, None))
    #     if get is not None:
...
class Efsm(EfsmBase):
    ...
    def add(self, *state, fn=None, data=None):
        # print(state, fn, data)
        if len(state) == 1 and fn is None and isinstance(state[0], StateSet):
...
if __name__ == '__main__':
    ...
    @fsm.ss.idle.move.stop  # careful: It's not proper to use for class method. Because it will record it immediately.
