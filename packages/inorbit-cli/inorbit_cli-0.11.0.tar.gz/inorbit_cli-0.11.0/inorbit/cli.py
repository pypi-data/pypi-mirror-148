...
def delete(ctx, filename):
    ...
    with open(file=filename, mode="r") as fd:
        ...
        # If the file is a JSON, the above function could return a list with the first element being a list of config objects inside
...
def get_config(kind, scope, dump, config_id, include_system_defaults):
    ...
    response = [
        ...
        for r in response["items"] if include_system_defaults or r["scope"][:5] != "root/"
    ]
    click.echo(tabulate(response, headers="keys"))
...
def get_kinds():
    ...
    click.echo(
        "\nLearn more about kinds at: https://developer.inorbit.ai/docs?hsLang=en#getting-started-with-the-cli")
