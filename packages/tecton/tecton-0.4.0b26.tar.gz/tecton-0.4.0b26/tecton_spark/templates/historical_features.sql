...
{%- macro join_window(name, spine, spine_timestamp_key, feature_timestamp_key, feature_columns, join_keys, time_aggregation, last_join) %}
    ...
    {%- for feature in time_aggregation.features %}
        {{ aggregate_cte_name(name, spine, time_aggregation.features, loop.index0) }} AS (
            ...
            GROUP BY {{ all_join_keys }}, {{ name|upper }}{{ separator }}{{ feature.output_feature_name|upper }}
    ...
    {%- if include_feature_view_timestamp_columns -%}
        {%- for item in feature_set_items -%}
            {%- if item.aggregation -%}
                ...
                TO_TIMESTAMP(DATE_PART(EPOCH_SECOND, _TT_SPINE_TABLE.{{ spine_timestamp_key }}) - MOD(DATE_PART(EPOCH_SECOND, _TT_SPINE_TABLE.{{ spine_timestamp_key }}), {{ item.aggregation.slide_interval.ToSeconds() }})) AS {{ final_select_name(item.name + separator + item.timestamp_key, false, true) }}
            {%- else %}
