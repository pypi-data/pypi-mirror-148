...
class KafkaConfig(BaseStreamDSConfig):
    ...
    This class used as an input to a :class:`StreamDataSource`'s parameter ``stream_config``. This class is not
    ...
    def __init__(
        ...
        :param watermark_delay_threshold: (Optional) Watermark time interval, e.g: timedelta(hours=24), used by Spark Structured Streaming to account for late-arriving data. See: https://docs.tecton.ai/v2/overviews/framework/feature_views/stream_feature_view.html#productionizing-a-stream
        ...
        .. code-block:: python
            ...
            # Declare Kafka Config instance object that can be used as an argument in StreamDataSource
            click_stream_kafka_ds = KafkaConfig(watermark_delay_threshold=datetime.timedelta(days=7),
