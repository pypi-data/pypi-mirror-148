...
class Regbot:
  ...
  @classmethod
  def enterSignalGenerator(cls,opening,closing,thr):
  ...
  @classmethod
  def exitSignalGenerator(cls,opening,closing,thr):
    scalledInput = cls.prepareInput(opening,closing)
    return (cls.loadmodel().predict_proba(scalledInput)[:,1] > thr).astype(int)[0]
    #return (cls.loadmodel().predict(scalledInput) > 0).astype(int)[0]
...
def signal(opening,closing,thr,sig):
  if sig.lower() == 'enter':
    try:
      return Regbot.enterSignalGenerator(opening,closing,thr)
    except Exception as e:
      print(e)
  if sig.lower() == 'exit':
    try:
      return Regbot.exitSignalGenerator(opening,closing,thr)
    except Exception as e:
      print(e)
  else:
