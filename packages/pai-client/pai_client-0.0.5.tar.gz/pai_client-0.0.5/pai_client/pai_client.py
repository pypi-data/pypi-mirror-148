...
class PaiClient:
    ...
    def get_job_name(self,job_config_yaml: str =""):
        job_config_yaml = self.parse_job_config(job_config_yaml)

        if 'name' in job_config_yaml:
            return job_config_yaml['name']
        return ''

    def parse_job_config(self,job_config_yaml: str =""):
        if os.path.isfile(job_config_yaml):
            with open(job_config_yaml,'r') as f:
               job_config_yaml = yaml.safe_load(f)
        else:
            job_config_yaml = yaml.safe_load(job_config_yaml)

        return job_config_yaml
    ...
    def get_job_status(self,job_name="",job_config_yaml: str =""):
        if job_name=="" and job_config_yaml!="":
            job_name=self.get_job_name(job_config_yaml)
    ...
    def wait_for_job_complete(self,job_name="",job_config_yaml: str =""):
        if job_name == "" and job_config_yaml != "":
            job_name = self.get_job_name(job_config_yaml)
        interval_sec = 60
        while True:
