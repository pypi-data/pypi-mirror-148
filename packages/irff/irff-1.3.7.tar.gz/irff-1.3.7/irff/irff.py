...
class IRFF(Calculator):
  ...
  def get_bondorder_uc(self):
      ...
      # self.N    = torch.sum(torch.where(self.bop<self.p['boc2'],0.0,1.0),1)
      # N         = self.P['val'] - self.P['valboc'] - self.N 
      # # S         = torch.where(N>-0.00001,-0.5,0.5)
      # # Si        = torch.unsqueeze(S,0)
      # # Sj        = torch.unsqueeze(S,1) 
      # # self.S    = (Si + Sj)*self.eye
      # Ni       = torch.unsqueeze(N,0)*self.eye
      # Nj       = torch.unsqueeze(N,1)*self.eye
      # cond1    = self.bop<self.p['boc2']
      # cond2    = self.bop>=self.p['boc2']
      # self.S   = torch.where(torch.logical_or(torch.logical_and(cond1,torch.logical_and(Ni>0,Nj>0)),
      #                                         torch.logical_and(cond2,torch.logical_or(Ni>=0,Nj>=0))),
      #                         -0.10,0.10)
  ...
  def message_passing(self):
      ...
      for t in range(1,self.messages+1):
          ...
          # V         = self.P['val']
          # Ni        = torch.unsqueeze(V,0)*self.eye - Dbi
          # Nj        = torch.unsqueeze(V,1)*self.eye - Dbj
          # S       = np.where(N>=0.0,-0.125,0.125)
          # Si        = torch.where(Ni>0.9,-0.125,0.125)
          # Sj        = torch.where(Nj>0.9,-0.125,0.125)
          # self.S    = (Si + Sj)*self.eye

          # if self.MessageFunction==3:
          #    Dbi = Di - self.H[t-1]
          #    Dbj = Dj - self.H[t-1]
          #    Fi  = self.f_nn('f'+str(t),[self.S,Dbj,Dbi,self.H[t-1]],layer=self.mf_layer[1])
          ...
          if self.MessageFunction==1:
             ...
             Fi  = self.f_nn('f'+str(t),[Dsi_j,Dsi_i,self.Hsi[t-1],
             ...
             Dbi = Di - self.H[t-1]
             Dbj = Dj - self.H[t-1]
             Fi  = self.f_nn('f'+str(t),[Dbj,Dbi,self.Hsi[t-1],self.Hpi[t-1],self.Hpp[t-1]],
                             layer=self.mf_layer[1])
             Fj  = torch.transpose(Fi,1,0)
             F   = Fi*Fj
             Fsi = F[:,:,0]
