...
class ReaxFF(object):
  def __init__(self,libfile='ffield',dataset={},
      ...
      self.p_['acut']    = self.atol        
      self.p_['hbtol']   = self.hbtol    
  ...
  def clip_parameters(self):
      ...
      for k in self.v:
          ...
          if key == 'n.u.':
             ...
             if self.nn:
                if key =='bo1':
                   self.p[k] = tf.clip_by_value(self.v[k],-0.1,-0.005)
                elif key =='bo3':
                   self.p[k] = tf.clip_by_value(self.v[k],-0.2,-0.005)
                elif key =='bo5':
             ...
             if self.nn:
                if key =='bo2':
                   self.p[k] = tf.clip_by_value(self.v[k],3.0,10.0)
                elif key =='bo4':
                   self.p[k] = tf.clip_by_value(self.v[k],3.0,10.0)
                elif key =='bo6':
                   self.p[k] = tf.clip_by_value(self.v[k],3.0,10.0)
             ...
             self.p[k] = tf.clip_by_value(self.v[k],0.87*self.p_['rosi_'+vn[1]],0.93*self.p_['rosi_'+vn[1]])
          elif key == 'ropp':
             self.p[k] = tf.clip_by_value(self.v[k],0.87*self.p_['ropi_'+vn[1]],0.93*self.p_['ropi_'+vn[1]])
          elif key == 'rohb':
             self.p[k] = tf.clip_by_value(self.v[k],1.5,3.6)
          elif key == 'rvdw':
             rvdw_ =  self.p_['rosi_'+vn[1]]
             self.p[k] = tf.clip_by_value(self.v[k],rvdw_,3.0*self.p_['rosi_'+vn[1]])
          elif key=='val5':
  ...
  def get_pentalty(self):
      ...
      for bd in self.bonds:
          if bd in penalty_bop:
             print('Unc-Bond-Order Penalty of                          {:5s}: {:f}'.format(bd,penalty_bop[bd]))
          if bd in penalty_bo:
