...
class IRFF_NP(object):
  ...
  def message_passing(self):
      ...
      for t in range(1,self.messages+1):
          ...
          # self.N  = np.sum(np.ceil(np.where(self.bop<self.botol,0.0,self.bop)),axis=1)
          # V         = self.P['val'] #-1.0
          # self.Ni   = np.expand_dims(V,axis=0)*self.eye - Dbi
          # self.Nj   = np.expand_dims(V,axis=1)*self.eye - Dbj
          # S       = np.where(N>=0.0,-0.125,0.125)
          # Si        = np.where(self.Ni>0.9,-0.125,0.125)
          # Sj        = np.where(self.Nj>0.9,-0.125,0.125)
