...
def find_video_of_file(video_dir, filename):
    ...
    if return_path == '':
        print('SMOOTHING FAILED: Smoothing requires the video to be imported into the project. SimBA could not find a video file resporesenting {} in the project_folder/videos directory'.format(str(filename)))
    else:
        return return_path
...
def smooth_data_gaussian(config=None, file_path=None, time_window_parameter=None):
    ...
    print('Smoothing complete for file {}'.format(filename), '...')
