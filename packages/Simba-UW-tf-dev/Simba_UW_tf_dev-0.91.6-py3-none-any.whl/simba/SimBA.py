...
class project_config:
    ...
    def __init__(self):
        ...
        self.smooth_dropdown = DropDownMenu(self.smooth_pose_win_lbl, 'Smoothing', ['None', 'Gaussian'], '12', com=self.smoothing_selected)
        self.smooth_dropdown.setChoices('None')
        self.smoothing_gaussian_time_label = Entry_Box(self.smooth_pose_win_lbl, 'Period (ms):', labelwidth='12', width=10)
    ...
    def smoothing_selected(self, choice):
        if choice == 'None':
            self.smoothing_gaussian_time_label.grid_forget()
        if choice == 'Gaussian':
            self.smoothing_gaussian_time_label.grid(row=1, column=1, sticky=E)
    ...
    def importh5(self):
        ...
        if self.smooth_dropdown.getChoices() == 'Gaussian':
            ...
            smooth_settings_dict['Parameters']['Time_window'] = self.smoothing_gaussian_time_label.entry_get
    ...
    def import_singlecsv(self):
        ...
        if self.smooth_dropdown.getChoices() == 'Gaussian':
            time_window = self.smoothing_gaussian_time_label.entry_get
    ...
    def import_multicsv(self):
        try:
            ...
            if self.smooth_dropdown.getChoices() == 'Gaussian':
                time_window = self.smoothing_gaussian_time_label.entry_get
                print('Smoothing pose-estimation data (Time window: ' + str(time_window))
                for file in csvfile:
...
class loadprojectini:
    ...
    def smoothing_selected(self, choice):
        if choice == 'None':
            self.smoothing_gaussian_time_label.grid_forget()
        if choice == 'Gaussian':
            self.smoothing_gaussian_time_label.grid(row=0, column=1, sticky=E)
