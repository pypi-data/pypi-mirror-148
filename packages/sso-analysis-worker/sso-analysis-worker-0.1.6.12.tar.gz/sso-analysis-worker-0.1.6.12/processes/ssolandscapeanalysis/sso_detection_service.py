...
class SSODetectionService:
    ...
    def automatic_sso_detection(self, chromedriver, latest_login_info, progress_callback=None, run_google=True,
        ...
        if run_google:
            ...
            google_locate_result = self.locators['google'].locate_login(chromedriver, latest_login_info, special_check)
            ...
            if google_locate_result[0]:
                ids.append((self.provider_ids['google'], google_locate_result[1], google_locate_result[2],
                            latest_login_info.loginPath, google_locate_result[3]))
        ...
        if run_facebook:
            ...
            if facebook_locate_result[0]:
                ids.append((self.provider_ids['facebook'], facebook_locate_result[1], facebook_locate_result[2],
                            latest_login_info.loginPath, facebook_locate_result[3]))
        ...
        if run_apple:
            ...
            logger.info("--- Apple ---")
            start_time = time.time()
            apple_locate_result = self.locators['apple'].locate_login(chromedriver, latest_login_info, special_check)
            ...
            if apple_locate_result[0]:
                ids.append((self.provider_ids['apple'], apple_locate_result[1], apple_locate_result[2],
                            latest_login_info.loginPath, apple_locate_result[3]))
        if len(ids) == 0:
            ids.append((self.provider_ids['none'], None, None, None, SSODetectionStrategy.ELEMENT_SEARCH))
        return ids
