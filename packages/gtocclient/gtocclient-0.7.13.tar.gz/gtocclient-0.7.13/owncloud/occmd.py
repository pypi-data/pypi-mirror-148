...
def get_client(progress=True):
    ...
    if progress:
        client = Client(cfg.url, progress=cfg.progress_meter, progress_class=cfg.progress_class, oc_path_format=True, proxies=proxies)
    else:
        client = Client(cfg.url, oc_path_format=True, proxies=proxies)
    client.login(cfg.user_id, cfg.password)
    return client
