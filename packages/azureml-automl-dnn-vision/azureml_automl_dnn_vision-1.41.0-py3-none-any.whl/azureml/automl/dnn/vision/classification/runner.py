...
def run(automl_settings, mltable_data_json: Optional[str] = None, multilabel=False, **kwargs: Any):
    ...
    if distributed and device_count > 1:
        ...
        torch.multiprocessing.spawn(train_worker, args=(
            settings, mltable_data_json, multilabel), nprocs=device_count, join=True)
    else:
        if distributed:
        ...
        train_worker(0, settings, mltable_data_json, multilabel)
...
def train_worker(rank, settings, mltable_data_json, multilabel):
    ...
    if train_ds is not None:
        train_dataset_wrapper, valid_dataset_wrapper = read_aml_dataset(
            dataset=train_ds, validation_dataset=validation_ds, split_ratio=split_ratio,
