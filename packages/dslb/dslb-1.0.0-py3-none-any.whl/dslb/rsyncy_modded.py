...
"""This is a modded version of rsyncy python module."""
...
class CLI:
    ...
    def printline(*text, sep=" ", end="\r\n"):
        CLI.write("\r", sep.join([str(t)
                  for t in text]), CLI.esc.clear_line(), end)
...
class Rsyncy:
    ...
    def draw_stat(self):
        ...
        if self.chk_finished:
            ...
            spin = self.spinner[round(
                elapsed.total_seconds()) % len(self.spinner)]
        ...
        # reduce to fit
        def plen(): return sum(len(s) for s in parts) + len(parts)
        ...
        status = delim.join(parts).replace(
            "\xff", f"{self.cspin}{spin}{self.ctext}")
...
def run_rsync(args, write_pipe, rc, as_sudo=False):
    # prefix rsync and add args required for progress
    args = ["rsync", "--info=progress2", "-hv", "--no-v"] + args
    if as_sudo:
        args = ["sudo"] + args
    try:
...
def main(args: list, as_sudo: bool = False):
    ...
    try:
        if len(args) == 1:
            ...
            rc = queue.Queue()
            t = Thread(target=run_rsync, args=(args, write_pipe, rc, as_sudo))
            t.start()
