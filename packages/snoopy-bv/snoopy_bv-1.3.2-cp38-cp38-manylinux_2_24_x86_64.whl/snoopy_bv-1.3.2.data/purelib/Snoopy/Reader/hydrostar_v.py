...
def json_to_rao( db_path, output_path ):
    ...
    ds = read_appHarmonicFlow_json( db_path )
...
def read_hydrostarV_database(inputdata):
    ...
    harmonicFlow_db_path    = os.path.join(folderPath,outputDir,"HarmonicFlow_DB")
    kwargs = {}
    ...
    hflowDBs = read_appHarmonicFlow_json(harmonicFlow_db_path)
    ...
    return outputList
def read_appHarmonicFlow_json(db_path,selectForce=1):
    """ Read appHarmonicFlow json output files and return a
    ...
    for file in files:
        ...
        hFlowParams = data.get("hFlowParams",{})

        forwardSpeed.append(hFlowParams.get("forwardSpeed"))
        waveHeading.append(hFlowParams.get("waveHeadingDeg"))
        waveFreq.append(hFlowParams.get("waveFreqRad"))
        encFreqVal  = hFlowParams.get("encFreqRad")
        encFreq.append(encFreqVal)
        waveLength.append(hFlowParams.get("waveLength"))
...
if __name__ == "__main__":
    ...
    if test:
        ...
        harmonicFlowOutput = read_appHarmonicFlow_json(hydroCoeff_db_path)

        meshBaseFlowOutput = read_appMeshBaseFlow_json(hstat_db_path)

        data = build_HydroStarV_db(baseFlow_db_path = hstat_db_path,
                                   harmonicFlow_db_path = hydroCoeff_db_path)
