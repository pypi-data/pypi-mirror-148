...
def vectran3(a, origin, destination):
    """
    Move modal amplitudes by a vect v 
    ...
    return  vectran(a, origin - destination )


def vectran_freq_head( a4 , origin, destination) :
    moved = np.zeros(a4.shape, dtype=a4.dtype)
    for ihead in range(a4.shape[0]):
        for ifreq in range(a4.shape[1]):
            moved[ihead, ifreq , :] =  vectran3(a4[ihead, ifreq , :], origin, destination)
    return moved
