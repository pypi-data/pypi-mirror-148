...
"""
    ...
    def integrate_spreading(self, variant = "ww3", wmin = _wmin, wmax = _wmax, n_theta = 72, method = "dblquad" ) :
        ...
        if variant.lower() in [ "ww3" , "swan"]:
            a = self._cossin(np.cos, wmin = wmin, wmax = wmax, method=method, n_theta = n_theta)
            b = self._cossin(np.sin, wmin = wmin, wmax = wmax, method=method, n_theta = n_theta)
            m0 = self.integrate_moment( 0 )
            return ( 2 * ( 1 - ( (a**2 + b**2) / m0**2 )**0.5)   )**0.5
        elif variant.lower() in ["era5", "ecmwf", "wam"]:
            db = 2*np.pi / n_theta
            t = np.arange( 0, 2*np.pi - db, db)
            freq = np.arange( wmin, wmax , 0.005 )
            I1 = 0.0
