...
class DataFrameBuilder:
    ...
    def compute(self, synapse, MTOC, perforin):
        self.data[self.index_property.name].append(self.index_property.compute(synapse, MTOC, perforin))
    ...
    def show_radar(self):
        ...
        # Ind1
        df = self.interpolate()
        values = df[df[self.group.name] == "NP"].drop(columns=[self.index_property.name, self.group.name]).mean(
        ...
        # Ind2
        values = df[df[self.group.name] == "P"].drop(columns=[self.index_property.name, self.group.name]).mean(
            axis=0).values.flatten().tolist()
        values += values[:1]
