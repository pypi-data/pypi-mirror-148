...
class PulsarWorker(object):
    ...
    async def _health_check(self):
        ...
        if not self._producer.is_connected() or not self._consumer.is_connected():
            self._fail_count += 1
            logger.error(f"pulsar 连接第{self._fail_count}失败")
            print(f"pulsar 连接第{self._fail_count}失败")
        ...
        if self._fail_count >= 30:
            # 通过退出程序来让k8s 重启
            logger.error("pulsar 连接检测失败，将退出由k8s实现重启")
            print("pulsar 连接检测失败，将退出由k8s实现重启")
