...
class DummyCMP(CUModelParams):
    def __init__(self):
        super().__init__(1, 2, 1, 1, 10)

        # This is here to mock the debug output of CU model parameters
        fake_params = np.random.random((1, 1, 2, 52))  # (1, X, Y, num_params)
        self.hdu_list = fits.HDUList([fits.PrimaryHDU(fake_params)])
    ...
    with InstrumentPolarizationCalibration(
        ...
        try:  # This try... block is here to make sure the dbs get cleaned up if there's a failure in the fixture
            ...
            mocker.patch(
                ...
                new_callable=PropertyMock,
                return_value=(DummyCMP(), object(), object(), object()),
            )
