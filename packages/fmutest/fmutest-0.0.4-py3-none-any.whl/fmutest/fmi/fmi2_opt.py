...
class FMU_ME(FMU2Model):
    def __init__(self, fmu, step_size):
        ...
        super(FMU_ME, self).__init__(guid=self.model_description.guid, unzipDirectory=uzd, modelIdentifier=self.model_description.modelExchange.modelIdentifier, instanceName = 'ME1')
        self.instantiate()
    ...
    def _euler_(self, start_values, start_time=0, stop_time=1):

        # check step size
        output_interval = self.step_size
        if not np.isclose(round(output_interval / self.step_size) * self.step_size, output_interval):
        ...
        while time < stop_time:
            t_next = start_time + n_fixed_steps * self.step_size            
            if t_next > stop_time:
                break
            n_fixed_steps += 1
            ...
            if abs(time - round(time/output_interval)*output_interval) < eps and time > self.recorder.lastSampleTime + eps:
