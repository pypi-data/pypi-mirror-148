...
class SSML:
    ...
    def dump(self):
        ...
        if "</s>" not in xml:
            return xml
        pending_text = re.search(r"<s>(.*)</s>", xml)
        pending_text = pending_text and pending_text.group(1)
        processed_text = pending_text.replace("<s>", "").replace("</s>", "")
        return xml.replace(pending_text, processed_text)
