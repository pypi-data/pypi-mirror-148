...
def to_spec(gps_type, media, entry):
    ...
    return {
        ...
        'Heading': entry['direction'],
        'Datecode': entry['locTime'],
        'Position': [lon, lat],
    }
...
def find_gps(args, api, lookup, gps):
    ...
    for media_id, media in lookup.items():
        ...
        print(f"MEDIA START: {media['start']}")
        print(f"MEDIA STOP: {media['stop']}")
        spec += entries
        print(f"Found {len(entries)} GPS entries for media {media_id}...")
