...
class HttpSession(AbstractContextManager):
    ...
    def submit(self,
        ...
        self.__logger.debug(f'HTTP/{method} to {url}')
        self.__logger.debug(f'authenticator_index => {authenticator_index}')
        self.__logger.debug(f'retry_with_reauthentication => {retry_with_reauthentication}')
        self.__logger.debug(f'retry_with_next_authenticator => {retry_with_next_authenticator}')
