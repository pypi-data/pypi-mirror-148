...
def create_coco_json_from_yolo_set(yolo_root_dir: str, coco_root_dir: str, ds_source_set: str, target_save_file: str,
    ...
    if not os.path.isdir(coco_root_dir):
        logging.info("Coco root ds does not exist. Creating...")
    ...
    if materials_dict is None:
        ...
        logging.info(f"No materials dict provided. Created {materials_dict}")
    ...
    img_target_paths = [os.path.join(coco_root_dir, "coco", ds_source_set + "2017", fname) for fname in sorted(os.listdir(img_source_dir))]
    ...
    for i, (img_src_path, label_path, img_target_path) in enumerate(zip(img_src_paths, labels_src_paths, img_target_paths)):
        ...
        coco_dict["images"].append({"file_name": img_target_path,  # "file_name": os.path.split(img_path)[-1],
...
# def cvt_yolo_to_detectron(yolo_root_dir: str, coco_target_dir: str = None, materials_dict: dict = None) -> None:
#     """
#
...
#     :return:
#     """
#     if isinstance(yolo_root_dir, str) and yolo_root_dir.endswith("/"):
#         yolo_root_dir = yolo_root_dir[:-1]
#     if coco_target_dir is None:
#         coco_target_dir = yolo_root_dir + "_coco"
#
#     # Make sure there are no slashes at the end of the paths:
#     yolo_root_dir = yolo_root_dir.rstrip()
#     coco_target_dir = coco_target_dir.rstrip()
#
...
#     # Create json files:
#     create_coco_json_from_yolo_set(yolo_root_dir, coco_target_dir, "train", train_json_name, materials_dict)
#     create_coco_json_from_yolo_set(yolo_root_dir, coco_target_dir, "valid", valid_json_name, materials_dict)
#     create_coco_json_from_yolo_set(yolo_root_dir, coco_target_dir, "test", test_json_name, materials_dict)
#
...
def cvt_yolo_to_detectron(yolo_root_dir: str, coco_target_dir: str = None, materials_dict: dict = None) -> None:
    ...
    if materials_dict is None:
        materials_dict = ds_annotations.create_fallback_yolo_materials_dict(yolo_root_dir)
    # Create json target paths
    train_json_name = os.path.join(coco_annotations_dir, "instances_train2017.json")
    val_json_name = os.path.join(coco_annotations_dir, "instances_val2017.json")
    test_json_name = os.path.join(coco_annotations_dir, "instances_test2017.json")
    # Create json files:
    create_coco_json_from_yolo_set(yolo_root_dir, coco_target_dir, "train", train_json_name, materials_dict)
    create_coco_json_from_yolo_set(yolo_root_dir, coco_target_dir, "valid", val_json_name, materials_dict)
    create_coco_json_from_yolo_set(yolo_root_dir, coco_target_dir, "test", test_json_name, materials_dict)
...
if __name__ == "__main__":
    ...
    cvt_yolo_to_detectron(fdir_gnejna_full, materials_dict={0: "trash"})