...
def create_coco_json_from_copied_yolo_set(yolo_root_dir: str, coco_root_dir: str, ds_source_set: str,
    ...
    img_target_paths = [os.path.join(coco_root_dir, ds_source_set + "2017", fname) for fname in sorted(os.listdir(img_source_dir))]
    ...
    coco_dict = {"info": {"year": int(datetime.date.today().year),
                          "version": None,
                          "description": f"dataset created from yolo {yolo_root_dir}",
...
def create_coco_json_only_from_yolo_set(yolo_root_dir: str, ds_source_set: str,
...
def cvt_yolo_to_coco(yolo_root_dir: str, coco_target_dir: str = None, materials_dict: dict = None) -> None:
    """
    Converts a yolo dataset to a fake coco dataset. Probably not advisable.
    :param yolo_root_dir: yolo root ds dir
    :param coco_target_dir: optional. target root location of coco target dir.
    :param materials_dict: e.g. {0: "trash"}
    :return:
    """
    if coco_target_dir is None:
        home = os.path.expanduser("~")
        coco_target_dir = os.path.join(home, "coco")
    logging.info(f"COCO target dir: {coco_target_dir}")

    print("Creating coco directories and paths...")
    coco_annotations_dir = os.path.join(coco_target_dir, "annotations")
    coco_train_dir = os.path.join(coco_target_dir, "train2017")
    coco_valid_dir = os.path.join(coco_target_dir, "val2017")
    coco_test_dir = os.path.join(coco_target_dir, "test2017")
    print(f"Saving coco dataset in: {os.path.abspath(coco_target_dir)}")

    if os.path.isdir(coco_target_dir):
        print(f"[ WARNING ] Dataset conversion skipped. Target dir already exists: {coco_target_dir}")
        logging.info(f"[ WARNING ] Dataset conversion skipped. Target dir already exists: {coco_target_dir}")
        return None
    else:
    ...
    train_json_fpath = os.path.join(coco_annotations_dir, "instances_train2017.json")
    val_json_fpath = os.path.join(coco_annotations_dir, "instances_val2017.json")
    test_json_fpath = os.path.join(coco_annotations_dir, "instances_test2017.json")
    # Create json files:
    create_coco_json_from_copied_yolo_set(yolo_root_dir, coco_target_dir, "train", train_json_fpath, materials_dict)
    create_coco_json_from_copied_yolo_set(yolo_root_dir, coco_target_dir, "valid", val_json_fpath, materials_dict)
    create_coco_json_from_copied_yolo_set(yolo_root_dir, coco_target_dir, "test", test_json_fpath, materials_dict)
...
def cvt_yolo_to_detectron(yolo_root_dir: str, target_dir: str = None, dataset_name: str = "detectron", materials_dict: dict = None) -> None:
    ...
    print(f"Saving dataset in: {os.path.abspath(target_dir)}")
    ...
    if os.path.isdir(target_dir):
        print(f"[ WARNING ] Dataset conversion skipped. Target dir already exists: {target_dir}")
        logging.info(f"[ WARNING ] Dataset conversion skipped. Target dir already exists: {target_dir}")
        return None
    else:
...
def create_jsons_only_from_yolo(yolo_root_dir: str, json_target_dir: str = None, dataset_name: str = None,
    ...
    # Create json files:
    create_coco_json_only_from_yolo_set(yolo_root_dir=yolo_root_dir, ds_source_set="train", target_save_file=fpath_json_train, materials_dict=materials_dict)
    create_coco_json_only_from_yolo_set(yolo_root_dir=yolo_root_dir, ds_source_set="valid", target_save_file=fpath_json_val, materials_dict=materials_dict)
    create_coco_json_only_from_yolo_set(yolo_root_dir=yolo_root_dir, ds_source_set="test", target_save_file=fpath_json_test, materials_dict=materials_dict)
...
if __name__ == "__main__":
    ...
    cvt_yolo_to_detectron(yolo_root_dir=fdir_gnejna_full, materials_dict={0: "trash"})
